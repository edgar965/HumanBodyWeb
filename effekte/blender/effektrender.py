# -*- coding: utf-8 -*-
"""Effektrender — Kamera, Boden, Workbench/Eevee, MP4.
...
class Effektrender:
    ...
    def blickrichtung(self):
        """Wohin die Figur im ersten Bild schaut: aufwaerts x (rechts - links).
    ...
    def faerben(werkstoff, farbe):
        """Grundfarbe fuer beide Renderer: Knoten (Eevee) und diffuse_color (Workbench)."""
    ...
    def rendern(self, gesamt_schritte, versatz):
        ...
        def geschrieben(sz, _tiefe=None):
            ...
            if time.perf_counter() >= zustand["naechste"] or bild == ende:
                self.melden(
                    "Effekte: Rendern Bild %d von %d — %d / %d"
    ...
    def geschriebene_datei(ausgabe):
        """Blender haengt bei Filmen Start-Ende an, wenn die Endung fehlt."""
