# -*- coding: utf-8 -*-
"""Stoffnewton — Kleidung auf der bewegten HumanBody-Figur, gerechnet mit Newton.
...
class Stoffnewton:
    ...
    def aufbauen(self):
        ...
        shape = builder.add_shape_mesh(-1, mesh=netz, xform=wp.transform(wp.vec3(0.0), wp.quat_identity()))  # pyright: ignore[reportCallIssue]
        ...
        print(
            ...
            % (
                ...
                int(self.fest.sum()),
                float(self.masse[~self.fest].sum()),
                len(self.koerper_dreiecke),
            ),
            flush=True,
        )

    def _panels(self):
        """Jedes Dreieck einzeln flach gelegt (drei eigene Panel-Punkte je
    ...
    def _flaechen_vorbereiten(self):
        """Flaeche je Punkt (ein Drittel jeder Nachbarflaeche) fuer den Wind."""
        t = self.stoff_dreiecke
        ...
        for ecke in range(3):
            self.flaeche_punkt += np.bincount(t[:, ecke], weights=flaeche / 3.0, minlength=self.n)[: self.n]
    ...
    def _teilschritt(self, zeit, mit_wind=True):
        ...
        if mit_wind:
            kraft = self._wind(
                self.s0.particle_q.numpy().astype(np.float64),
                self.s0.particle_qd.numpy().astype(np.float64),
                zeit,
            )
    ...
    def _bild_fahren(self, k0, k1, f0, f1, zeit, mit_wind):
        """Ein Bild: Koerper und feste Punkte laufen linear von 0 nach 1,
        `sub` Teilschritte."""
    ...
    def _einlauf(self):
        """Von der Ruhelage (A-Haltung) ueber die Zwischenposen der Rampe
    ...
    def laufen(self):
        ...
        for bild in range(self.bilder - 1):
            ...
            self._bild_fahren(
                self.koerper[bild],
                self.koerper[bild + 1],
                self.stoff_folge[bild],
                self.stoff_folge[bild + 1],
                bild / self.fps,
                True,
            )
            wp.synchronize()
