# -*- coding: utf-8 -*-
"""Figurfilm — der Unterprozess der Pipeline „HumanBody-Figur (DEF-Skelett)".
...
class Figurfilm:
    ...
    def melden(self, phase, anteil):
        """Bruchform `n / 100` — `Logbeobachter.BRUCH` liest sie."""
        print("%s%s — %d / 100" % (self.PRAEFIX, phase, int(round(anteil * 100))), flush=True)
    ...
    def laufen(self):
        ...
        self.melden("Modell %s" % modell.name, 0.0)
        ...
        if code != 0 or not os.path.isfile(self.ausgabe):
            stand = lauf.stand()
            raise SystemExit("Film abgebrochen: %s" % (stand.get("fehler") or stand))
        self._bericht(modell, lauf)
        self.melden("Fertig", 1.0)
        return 0

    def _auftrag(self, modell):
        """Der Auftrag im Aufbau von `filmlauf.py`, aus Modell und Parametern."""
        return {
            ...
            "breite": int(self.p.breite),
            "hoehe": int(self.p.hoehe),
            "ziel": self.ausgabe,
