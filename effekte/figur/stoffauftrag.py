# -*- coding: utf-8 -*-
"""Stoffauftrag — die Kleidung des Films an Newton uebergeben und zurueckholen.
...
class Stoffauftrag:
    ...
    def __init__(self, film, ablage, parameter, melder, stuecke):
        """`film` ist der gerechnete `Hbfilm`; `stuecke` die Liste aus
        ...
        self.teile = [
            t
            for t in film.teile[1:]
            if not t['name'].startswith(self.FRISUR) and self.rig_pfade.get(t['name'], '').endswith('.json')
        ]
    ...
    def rechnen(self, python, skript, cache):
        if not self.teile:
            ...
            print('Stoff: kein GarmentCode-Stueck am Modell — nichts zu rechnen', flush=True)
            return None
        self.melder('Stoff vorbereiten', 0.3)
        ...
        return {
            'stuecke': [t['name'] for t in self.teile],
    ...
    def _schreiben(self, pfad):
        ...
        parameter = {
            'fps': self.p.fps,
            'teilschritte': self.p.teilschritte,
            'iterationen': self.p.iterationen,
            'wind': self.p.wind,
            'richtung': self._windrichtung().tolist(),
            'turbulenz': self.p.turbulenz,
            'dichte': self.p.dichte,
            'steifigkeit': self.p.steifigkeit,
            'biegung': self.p.biegung,
            'einlauf': self.p.einlauf,
        }
        ...
        np.savez(
            ...
            koerper_rampe=koerper_rampe,
            stoff_rampe=stoff_rampe,
    ...
    def _rampe(self, koerper):
        """Zwischenposen A-Haltung -> Bild 0 (`einlauf` Sekunden bei `fps`),
    ...
    def _fest(self, ruhe):
        """Das obere Band des Stuecks — `bund` cm unter dem hoechsten Punkt."""
        oben = float(ruhe[:, 2].max())
        return ruhe[:, 2] >= oben - float(self.p.bund) / 100.0

    def _windrichtung(self):
        """Aus der Blickrichtung des Rigs (Zehe gegen Fuss, Ruhelage, z oben)."""
    ...
    def _laufen(self, python, skript, auftrag, ergebnis, cache):
        if not os.path.isfile(python):
            raise ValueError('Newton-Umgebung fehlt: %s' % python)
        ...
        for roh in prozess.stdout or []:
            ...
            if zeile.startswith('Effekte: '):
                ...
                try:
                    n, m = [int(x) for x in teil.rsplit('—', 1)[1].split('/')]
                    self.melder(teil.rsplit('—', 1)[0].strip(), 0.3 + 0.15 * n / max(m, 1))
