...
export class Testfallansicht {
    ...
    zeigen(z) {
        ...
        const stand = JSON.stringify([figur, t ? t.rms_mm : null, z.updated_at]);
        ...
        const teile = Object.entries(t.je_teil || {}).sort((a, b) => b[1] - a[1]).slice(0, 5)
        ...
        this.zahlen.innerHTML = `<div class="bildmodell-testfallkopf">`
            ...
            + `<b>RMS ${t.rms_mm.toFixed(2)} mm</b>, max ${t.max_mm.toFixed(1)} mm · `
            + `Höhe Referenz ${t.hoehe_cm.referenz} cm, Modell ${t.hoehe_cm.modell} cm`
            + (teile ? ` · schlechteste Teile (RMS mm): ${teile}` : '') + `</div>`
