...
/**
 * UmaModell — eine UMA-Figur aus dem Figurkatalog, als `Modell` für jede Seite.
 *
 ...
 *
 * Seit 13.09.2026 in `gemeinsam/` (Edgar: „eine globale Klasse … Von der
 * sollen HumanBody, UMA, MakeHuman ableiten und die Bauen-Methode
 * implementieren"); die Szene erbt als `UmaModell` nur das Speichern.
 */
const LADER = new GLTFLoader();
export class UmaModell extends Modell {
    ...
    async bauen() {
        const gltf = await LADER.loadAsync(UmaModell.ADRESSE + encodeURIComponent(this.datei) + '/');
        ...
        const lage = new THREE.Matrix4()
            .makeRotationFromQuaternion(UmaModell.ausrichtung(boneByName))
        ...
        this.hoehe = UmaModell.hoehe(netze);
        this.huelle = new THREE.Group();
        this.huelle.name = 'uma-huelle';
        this.huelle.scale.setScalar(this.hoehe > 0 ? UmaModell.ZIELHOEHE / this.hoehe : 1);
        this.huelle.add(wurzel, ...netze);
        this.group.add(this.huelle);
        this.group.updateMatrixWorld(true);

        this.netze = netze;
        this.bodyMesh = UmaModell.haut(netze);
    ...
    static ausrichtung(boneByName) {
        ...
        quer.y = 0;
        const vorn = quer.cross(UmaModell.OBEN).normalize();
        return drehung.setFromAxisAngle(UmaModell.OBEN, -Math.atan2(vorn.x, vorn.z));
    }
