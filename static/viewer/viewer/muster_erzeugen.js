...
export async function peRegionGenerate() {
    ...
    const zMin = (sliderVal('pe-region-zmin') / 100).toFixed(3);
        const zMax = (sliderVal('pe-region-zmax') / 100).toFixed(3);
    ...
    try {
        ...
        if (data.error) { if (statusEl) statusEl.textContent = `Error: ${data.error}`;
            if (genBtn) genBtn.disabled = false; return; }
...
export async function peGenerate3D() {
    ...
    try {
        ...
        const wrapOffset = (sliderVal('pe-wrap-offset') || 6) / 1000;
            const wrapStiffness = (sliderVal('pe-wrap-stiffness') ?? 50) / 100;
...
export async function peSaveToLibrary() {
    ...
    const cr = parseInt(colorHex.slice(1, 3), 16) / 255; const cg = parseInt(colorHex.slice(3, 5), 16) / 255;
...
export async function peLoadFromGarment(garmentId) {
    try {
        ...
        const nameEl = document.getElementById('pe-save-name'); if (nameEl) { const parts = garmentId.split('/');
            nameEl.value = parts[parts.length - 1]; }
        const catEl = document.getElementById('pe-save-category');
        if (catEl) { const parts = garmentId.split('/'); if (parts.length > 1) { const cat = parts[0];
            for (const opt of catEl.options) { if (opt.value === cat) { catEl.value = cat; break; } } } }
