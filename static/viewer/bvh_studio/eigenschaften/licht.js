...
export class Lichteigenschaften {
    static maske(track) {
        ...
        return `<div class="prop-group">
            <div class="prop-row"><label>Licht-Typ:</label>${auswahl}${track._sceneLight ? '<span style="margin-left:6px;font-size:0.7rem;color:var(--text-muted);">(Szene)</span>' : ''}</div>
            <div class="prop-row"><label>Farbe:</label><input type="color" value="${L ? '#' + L.color.getHexString() : '#ffffff'}" id="prop-light-color"></div>
            ...
            ${ambient ? '' : `<div class="prop-row"><label>Winkel:</label><input type="number" value="${((L?.angle ?? Math.PI / 6) * 180 / Math.PI).toFixed(1)}" id="prop-light-angle" min="1" max="170" step="1"> °</div>
