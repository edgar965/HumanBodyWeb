# -*- coding: utf-8 -*-
"""Effektlauf — den Blender-Prozess eines Effektauftrags fahren und beobachten.
...
class Effektlauf:
    ...
    def verwaiste_aufraeumen(cls):
        """Nach einem Serverneustart: lebt der Blender-Prozess noch, wird er
        ...
        for auftrag in Effektauftrag.objects.filter(status__in=Effektauftrag.LAEUFT):
            ...
            if auftrag.pid and Prozesspruefung.lebt(auftrag.pid):
                if str(auftrag.id) not in cls._beobachtet:
                    ...
                    threading.Thread(target=cls._sicher_wieder, args=(auftrag.id,), daemon=True).start()
                continue
    ...
    def wieder_verfolgen(self):
        """Ohne Prozessgriff: Ende ueber die PID, Erfolg ueber die Dateien."""
        Effektbeobachter(
            self.auftrag, str(self.logdatei), Effektbefehl(self.auftrag).bilder(), pid=self.auftrag.pid
        ).verfolgen()
