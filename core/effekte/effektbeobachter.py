# -*- coding: utf-8 -*-
"""Effektbeobachter — der Logbeobachter, der nur die Effekte-Zeilen liest.
...
class Effektbeobachter(Logbeobachter):
    ...
    def auswerten(self, text, jetzt=None):
        zeilen = [
            z.strip()[len(self.PRAEFIX) :] for z in text.splitlines() if z.strip().startswith(self.PRAEFIX)
        ]
        if not zeilen:
            return False
