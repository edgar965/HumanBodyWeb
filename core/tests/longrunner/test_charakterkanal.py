# -*- coding: utf-8 -*-
"""Was wirklich über den Morph-Kanal geht — Punkte UND Knochen.
...
PUNKTE_JE_STUFE = {
    1: 74128,  # 70.851 + 3.277 Nahtkopien (15.09.2026)
    2: 286376,
    3: 1125784,
}
...
class Morphkanal:
    """Ein Gespräch mit dem Consumer — als Klasse, nicht als freie Funktion.
    ...
    async def alles(self, nachricht):
        """Sendet und sammelt, bis nichts mehr kommt.
...
class DerMorphKanal(SimpleTestCase):
    ...
    async def test_die_ruhelage_schickt_keine_knochen(self):
        """Sonst bindet der Browser die Haut bei jedem Regleranschlag neu,
    ...
    async def test_erst_die_punkte_dann_die_knochen(self):
        """Die Reihenfolge ist Drahtformat: Der Browser bindet die Haut an
    ...
    async def test_der_regler_zurueck_auf_null_holt_das_skelett_zurueck(self):
        """Die LEERE Nachricht — ohne sie bliebe das Rig in der letzten
    ...
    async def test_auch_ein_meta_regler_bewegt_das_skelett(self):
        """`meta` ist ein eigener Zweig im Consumer. Genau so eine Stelle
    ...
    async def test_reset_setzt_koerper_und_skelett_zurueck(self):
        """DER BEFUND. Vor dem 05.09.2026 kam hier 174 statt 0 — und zwar
    ...
    async def test_und_schickt_das_netz_dabei_nur_einmal(self):
        """Vorher gingen 840 kB zweimal raus — erst der ALTE Körper, dann
