# -*- coding: utf-8 -*-
u"""Genesis 9 gegen die installierte Daz-Bibliothek (18.09.2026, „Forsche wie
...
class HdUndDetailTest(SimpleTestCase):
    ...
    def test_hd_beitrag_allein(self):
        u"""Der HD-Anteil ohne die Kaefigdeltas: Nabel HD3 auf Stufe 1 — einige
    ...
    def test_hd_morph_bewegt_stufe_1_und_2(self):
        ...
        mit = self._netz('basis', {'anhaenge': False,
                                   'regler': {'body_bs_Navel_HD3': 1.0}})
        ...
        hoch = self._netz('basis', {'anhaenge': False,
                                    'regler': {'body_bs_Navel_HD3': 1.0}}, hoch=True)
    ...
    def test_detailnormalen_nur_mit_hoher_stufe(self):
        ...
        self.assertIn('skin_details/genesis9_head_nm_1001',
                      kopf['bilder']['detailnormalen'])
...
class EirgridUndPropsTest(SimpleTestCase):
    ...
    def _stueck(self, kennung, rumpf):
        antwort = self.client.post(
            ...
            data=json.dumps(rumpf), content_type='application/json')
        self.assertEqual(antwort.status_code, 200, antwort.content[:300])
        return antwort.json()
    ...
    def test_eirgrid_stilarten_und_zopfknochen(self):
        ...
        gedreht = self._stueck('eirgrid_hair_g9', {
            'stil': ['eirgrid_51_pose_01_back', 'eirgrid_52_length_03_long']})
        d = np.linalg.norm(_punkte(gedreht['teile'][0]) - _punkte(ruhe['teile'][0]),
                           axis=1)
    ...
    def test_zopfknochen_im_browser_skelett(self):
        u"""Der Koerper-Endpunkt haengt die eigenen Knochen der getragenen
        Stuecke ins Skelett — in der Stellung des Stils (Pose Back: die
        Zopfspitze wandert nach hinten)."""
        client = self.client
        ohne = client.post('/api/character/genesis9-figur/basis/netz/',
                           data=json.dumps({'anhaenge': False}),
                           content_type='application/json').json()
        self.assertNotIn('eigene', ohne['skelett'])
        adresse = '/api/character/genesis9-figur/basis/netz/'
        ruhe = client.post(adresse, data=json.dumps({
            'anhaenge': False, 'kleidung': [{'kennung': 'eirgrid_hair_g9'}]}),
            content_type='application/json').json()
        back = client.post(adresse, data=json.dumps({
            'anhaenge': False,
            'kleidung': [{'kennung': 'eirgrid_hair_g9',
                          'stil': ['eirgrid_51_pose_01_back']}]}),
            content_type='application/json').json()
        self.assertEqual(len(ruhe['skelett']['eigene']), 14)
        # 138 Knochen + 14 Zoepfe, je Blatt ein Endknochen.
        self.assertEqual(len(back['skelett']['knochen'])
                         - len(ohne['skelett']['knochen']), 28)
        ...
        self.assertLess(b['BackRight3']['schwanz'][2],
                        r['BackRight3']['schwanz'][2] - 0.1)
    ...
    def test_prop_haengt_an_der_hand(self):
        ...
        self.assertEqual((eintrag['art'], eintrag['knochen'], eintrag['griff']),
                         ('requisit', 'r_hand', True))
        ...
        client = self.client
        offen = client.post('/api/character/genesis9-figur/feminine/netz/',
                            data=json.dumps({'anhaenge': False}),
                            content_type='application/json').json()
        griff = client.post('/api/character/genesis9-figur/feminine/netz/',
                            data=json.dumps({'anhaenge': False,
                                             'griffe': ['tubal_dagger_feminine_rt']}),
                            content_type='application/json').json()
        daumen = {k['name']: np.asarray(k['kopf']) for k in offen['skelett']['knochen']}
        daumen_griff = {k['name']: np.asarray(k['kopf'])
                        for k in griff['skelett']['knochen']}
        bewegt = np.linalg.norm(daumen['r_index3'] - daumen_griff['r_index3'])
        self.assertGreater(bewegt, 0.01)
        self.assertLess(np.linalg.norm(daumen['l_index3'] - daumen_griff['l_index3']),
                        1e-6)
    ...
    def test_makeup_system_aus_script_loads(self):
        ...
        self.assertEqual((ebene['klarlackdeckkraft'], ebene['klarlackrauheit']),
                         (0.35, 0.0))
    ...
    def test_glanzbild_und_hd_der_anhaenge(self):
        u"""Lipgloss im Netz: `schminke.glanz` (PNG) am Kopf; Mund HD3 bewegt
        ...
        adresse = '/api/character/genesis9-figur/basis/netz/'
        netz = self.client.post(adresse, data=json.dumps({
            'praesets': {'lipgloss': glossy['id']}}),
            content_type='application/json').json()
        ...
        hd = self.client.post(adresse, data=json.dumps({
            'regler': {'head_bs_MouthRealism_HD3': 1.0}}),
            content_type='application/json').json()
