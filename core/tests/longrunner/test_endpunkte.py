...
from djangobase.endpunkttests import LESEN, WIRKUNG, EndpunktProbe

logger = logging.getLogger('core')

#: Ein Auftrag mit fester Kennung — die Adresse stand achtmal da und
#: war allein schon zu lang fuer eine Zeile (30.08.2026).
FOTOAUFTRAG = '/api/character/photo-job/00000000-0000-0000-0000-000000000001/'
...
class EndpunkteTest(EndpunktProbe):
    ...
    ENDPUNKTE = [
        ...
        (LESEN, 'daten', FOTOAUFTRAG),
        (WIRKUNG, 'loeschen', FOTOAUFTRAG + 'delete/'),
        (LESEN, 'erneut_analysieren', FOTOAUFTRAG + 'reprocess/'),
        (WIRKUNG, 'ausrichtung_sichern', FOTOAUFTRAG + 'save-alignment/'),
        (WIRKUNG, 'projektion_sichern', FOTOAUFTRAG + 'save-projection/'),
        (WIRKUNG, 'bild_sichern', FOTOAUFTRAG + 'screenshot/'),
        (LESEN, 'silhouette', FOTOAUFTRAG + 'silhouette/'),
    ...
    def test_der_methodenschutz_bleibt(self):
        ...
        for art, ziel, muster in self.ENDPUNKTE:
            ...
            if antwort.status_code != 405:
                offen.append('%s (%s): HTTP %s statt 405' % (ziel, muster, antwort.status_code))
