# -*- coding: utf-8 -*-
"""Ein Slipper sieht aus wie ein Slipper — gemessen am simulierten Netz.
...
class Schuhmass:
    """Masse eines simulierten Schuhs gegen den Fuss der Figur."""

    def __init__(self, netz_obj, figurnetz):
        """`netz_obj`: das rohe `*_sim.obj` (GarmentCode-Raum, cm);
    ...
    def ueberstand(self):
        """[(Höhe cm, Schuhbreite minus Fussbreite cm)] je 1-cm-Scheibe ab
        ...
        for y in np.arange(1.0, self.schuh[:, 1].max(), 1.0):
            ...
            f = self.fuss[(self.fuss[:, 1] >= self.boden + y) & (self.fuss[:, 1] < self.boden + y + 1)]
            if len(s) > 3 and len(f) > 3:
                aus.append(
                    (float(y), float((s[:, 0].max() - s[:, 0].min()) - (f[:, 0].max() - f[:, 0].min())))
                )
        return aus

    def rand(self):
        """Die Randpunkte des linken Schuhs — Kanten mit nur einem
    ...
    def randabstand_mm(self):
        """(median, 90 %) des Abstands Einstiegsrand -> Haut, Millimeter."""
