# -*- coding: utf-8 -*-
"""UMAs Form-Regler lesen: Unity-YAML, Wirkungen, Spiegelung, echter Bestand.
...
class UnityYamlTest(unittest.TestCase):
    ...
    def test_kennung_aus_ziffern_bleibt_text(self):
        """`guid: 0000…0001` las YAML als Zahl 1 — und `1234e5…` als
...
class FormreglerTest(unittest.TestCase):
    ...
    def test_gruppen_je_geschlecht(self):
        self.assertEqual([g["name"] for g in self.leser.gruppen("weiblich")], ["Körper", "Gesicht", "Pose"])
    ...
    def test_verschieben_wird_gespiegelt(self):
        """Unity: links −0,02 auf X. glTF ist an X gespiegelt: links +0,02."""
        wirkungen = {
            w["knochen"]: w for w in self._regler("weiblich", "Körper", "legSeparation")["wirkungen"]
        }
    ...
    def test_drehung_spiegeln_ist_selbstinvers(self):
        q = [0.1, 0.2, 0.3, 0.9]
        self.assertEqual(self.leser.spiegeln_drehung(self.leser.spiegeln_drehung(q)), q)

    def test_ohne_spiegelung_bleibt_alles_wie_in_unity(self):
        """Seit dem 08.09.2026 ist die Spiegelung abschaltbar.
        ...
        self.assertEqual(roh.spiegeln_drehung([0.1, 0.2, 0.3, 0.9]), [0.1, 0.2, 0.3, 0.9])
...
class EchterBestandTest(unittest.TestCase):
    """Gegen das UMA-Projekt auf der Platte — Pflicht, kein Skip: fehlt der
    ...
    def test_die_frauenfigur_hat_62_regler(self):
        ...
        self.assertEqual(
            [(g["name"], len(g["regler"])) for g in gruppen], [("Körper", 22), ("Gesicht", 39), ("Pose", 1)]
        )
        ...
        self.assertLessEqual(
            set(self.leser.uebergangen),
            {"DNAEffect_SharedColor", "DNAEffect_BlendShape", "DNAEffect_MeshModifier"},
        )
    ...
    def test_beinabstand_geht_links_nach_plus_x(self):
        ...
        seiten = {
            w["knochen"]: w["versatz"][0]
            for w in regler["wirkungen"]
