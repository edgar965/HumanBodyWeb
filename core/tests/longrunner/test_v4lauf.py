...
class V4Basis(TestCase):
    ...
    def lauf(self, zeilen=(), code=0, params=None):
        ...
        umgebung = override_settings(
            MOCAPNET_V4_SCRIPT=str(skript), MOCAPNET_V4_ROOT=str(self.ordner), PIPELINE_PYTHON=sys.executable
        )
        umgebung.enable()
    ...
    def bvh_ablegen(self, name="fertig.bvh", gross=True):
        ...
        pfad.write_text("HIERARCHY\n" + ("x" * (200 if gross else 5)), encoding="utf-8")
        return pfad


class BefehlTest(V4Basis):
    def test_auftrag_schlaegt_die_vorgabe(self):
        _, lauf = self.lauf(params={"face": True, "hands": False, "hcd_iterations": 0})
        befehl = lauf._befehl()
    ...
    def test_stoppmarke_und_ausgabe_stehen_im_befehl(self):
        ...
        self.assertTrue(befehl[befehl.index("--output") + 1].endswith("v4_tanz.bvh"))
...
class AusgabeTest(V4Basis):
    ...
    def test_fortschritt_zaehlt_bis_98_prozent(self):
        auftrag, lauf = self.lauf(zeilen=["TOTAL:100", "PROGRESS:100/100", "DONE:" + str(self.bvh_ablegen())])
        ergebnis = lauf.fahren()
        self.assertTrue(ergebnis.endswith("fertig.bvh"))
        self.assertEqual(auftrag.progress, 98, "v4 macht Erkennung UND BVH — Anteil 98, nicht 45")
