# -*- coding: utf-8 -*-
"""Hilfe → Kleidung → Kleiderphysik: die Seite steht, ihre Zahlen kommen aus Python.
...
class SeitePhysik(SimpleTestCase):
    ...
    def test_die_adresse_ist_die_erwartete(self):
        self.assertEqual(reverse("hilfe_kleidung_physik"), "/hilfe/kleidung/physik/")

    def test_die_kernaussagen_stehen_darin(self):
        """Die Zahlen und Namen, um die es geht — nicht der Fließtext."""
        for probe in (
            "Style3D",
            "Newton",
            "GarmentCode",
...
class KleiderphysikDatenStehenInPython(unittest.TestCase):
    ...
    def test_die_messung_hat_gute_und_schlechte_zeilen(self):
        """Eine Messtabelle, in der alles hält, hat nichts gemessen."""
    ...
    def test_blender_ist_messzeile_aber_keine_empfehlung(self):
        """Edgar: „blenderCloth nutze ich nicht" — die Zeile bleibt, das
        Urteil sagt nein."""
        blender = [k for k in Kleiderphysik.kandidaten() if k["name"].startswith("Blender")]
