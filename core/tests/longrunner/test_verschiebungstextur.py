# -*- coding: utf-8 -*-
u"""Verschiebungstextur: MB-Labs Displacement-Textur kommt als PNG mit
...
class Textur(SimpleTestCase):
    ...
    def test_dateiname_rundet_und_kennt_das_geschlecht(self):
        self.assertEqual(Verschiebungstextur.dateiname('Male_Caucasian', 0.123, -2, 1),
    ...
    def test_endpunkt_liefert_png_mit_cache_und_schreibt_in_die_ablage(self):
        with Pruefablage.ordner('verschiebung_') as ordner, \
                mock.patch.object(Verschiebungstextur, 'ORDNER', Path(ordner)):
            antwort = Client().get(
                '/api/character/textur/verschiebung/female/?age=0.5&tone=0.2&mass=0')
