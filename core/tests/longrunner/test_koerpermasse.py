# -*- coding: utf-8 -*-
"""`Koerperprofil` und `Koerpermasse`: Masse, deren Antwort bekannt ist.
...
def kunstkoerper(punkte_je_ring=48):
    """Ein 1,66 m grosser Kunstkoerper mit ausrechenbaren Massen.
    ...
    for z in np.arange(1.00, 1.4001, 0.01):
        for seite in (-1, 1):
            arme.extend([[seite * (0.30 + 0.04 * np.cos(p)), 0.04 * np.sin(p), z] for p in phi[::4]])
    for z in np.arange(0.00, 0.7901, 0.01):
        for seite in (-1, 1):
            beine.extend([[seite * 0.08 + 0.07 * np.cos(p), 0.07 * np.sin(p), z] for p in phi[::4]])
...
class DuenneScheibeTest(SimpleTestCase):
    """Eine Scheibe mit wenigen Punkten darf keine Messhoehe werden.
    ...
    def test_die_ausduennung_wirkt_ueberhaupt(self):
        """Gegenprobe: Ohne die Mindestbesetzung MUSS der duenne Ring
...
class ArmlochtiefeTest(SimpleTestCase):
    """`armscye_depth` haengt am Oberkoerper, nicht an der Koerperhoehe.
...
def armkoerper(winkel_grad, hand_punkte=0, punkte_je_ring=16):
    """Ein Kunstkörper, dessen Arme um einen BEKANNTEN Winkel hängen.
    ...
    for seite in (1, -1):
        ...
        for t in np.arange(0.0, 0.6001, 0.01):
            mitte = start + achse * t
            arme.extend([mitte + 0.045 * (np.cos(p) * quer1 + np.sin(p) * quer2) for p in phi])
        if hand_punkte:
...
class ArmhaltungTest(SimpleTestCase):
    """`arm_pose_angle` wird gemessen, nicht aus der Vorlage genommen.
    ...
    def test_sehr_steiler_arm_ist_die_bekannte_grenze(self):
        """Ueber 45 Grad laesst die Genauigkeit nach — gemessen, nicht
...
class SchulterneigungTest(SimpleTestCase):
    """`shoulder_incl` kommt aus dem Schlüsselbein, nicht aus der Vorlage.
    ...
    def test_bekannter_winkel(self):
        """Ein Schlüsselbein, das auf 0,10 m Länge um 0,10 m fällt,
    ...
    def test_fehlendes_skelett_gibt_none(self):
        """Kein Skelett heißt: Vorlagenwert behalten, nicht raten."""
        with Pruefablage.ordner() as ordner:
            self.assertIsNone(Schulterneigung.aus_datenordner(ordner))

    def test_echtes_skelett_liegt_bei_elf_grad(self):
        """Die Figur selbst — im Browser über `matrixWorld` gegengeprüft
