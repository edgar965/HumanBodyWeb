...
class AutofitUndStoffTest(SimpleTestCase):
    ...
    def test_2_bardot_rock_sitzt_auf_dem_clone(self):
        u"""Bund (y > 1,10 m in Ruhe) zum Clone-Koerper: gemessen 6,2 mm median."""
    ...
    def test_5_kleid_bauplan_und_freiheit(self):
        antwort = self.client.get(
            '/api/character/genesis9-figur/garderobe/dancing_queen_dress/stoff/0/'
            '?stufen=1')
        self.assertEqual(antwort.status_code, 200, antwort.content[:300])
        plan = antwort.json()
        ...
        kein = self.client.get(
            '/api/character/genesis9-figur/garderobe/g9_base_shirt/stoff/0/')
    ...
    def test_6_makeup_reste_blendmodi_glitzer_klarlack(self):
        ...
        self.assertEqual([round(w, 2) for w in glossy['klarlackfarbe']],
                         [0.95, 0.77, 0.67])
