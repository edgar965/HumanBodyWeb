# -*- coding: utf-8 -*-
"""MakeHuman-Figur: Basisnetz, Garderobe und der Sitz der Kleidung.
...
class MhfigurTest(SimpleTestCase):
    ...
    def test_netz_in_metern_y_oben(self):
        daten = self._netz()
        punkte = MhfigurTest._feld(daten["vertices"], np.float32, 3)
        self.assertEqual(len(punkte), 13380)
        self.assertAlmostEqual(float(punkte[:, 1].min()), 0.0, delta=0.001, msg="Fuesse nicht am Boden")
        self.assertAlmostEqual(daten["hoehe"], 1.666, delta=0.01)

    def test_umlaufsinn_zeigt_nach_aussen(self):
        """Positives Volumen = Normalen nach aussen (siehe Mhbasisnetz)."""
        daten = self._netz()
    ...
    def test_kleid_sitzt_am_koerper(self):
        ...
        stoff = MhfigurTest._feld(stueck["vertices"], np.float32, 3).astype(np.float64)
        haut = MhfigurTest._feld(self._netz()["vertices"], np.float32, 3).astype(np.float64)
        abstand = self._naechster_abstand(stoff, haut)
        self.assertLess(float(np.median(abstand)), 0.02, "Median-Abstand zur Haut zu gross")
        self.assertLess(float(abstand.max()), 0.04, "Ein Stoffpunkt schwebt weit neben dem Koerper")

    def test_uvs_kommen_je_ecke(self):
        """Punkte werden je (Punkt, Texturpunkt) aufgeteilt — sonst Nahtfehler."""
        stueck = self._kleid(self.ANZUG)
        self.assertIn("uvs", stueck)
        uvs = MhfigurTest._feld(stueck["uvs"], np.float32, 2)
        self.assertEqual(len(uvs), stueck["vertex_count"])

    def test_lfs_zeiger_ist_keine_textur(self):
        stueck = self._kleid(self.ANZUG)
        self.assertFalse(stueck.get("has_texture"), "Ein 135-Byte-LFS-Zeiger wurde als Textur gemeldet")
        self.assertIn("mat_color", stueck)

    def test_unbekanntes_kleid_ist_404(self):
        antwort = self.client.get("/api/character/mh-figur/garderobe/tops/quatsch/netz/")
        self.assertEqual(antwort.status_code, 404)
    ...
    def test_kleidung_blendet_die_haut_darunter_aus(self):
        ...
        self.assertLess(mit["face_count"], ohne["face_count"], "delete_verts wirkt nicht")
        self.assertGreater(
            mit["face_count"], ohne["face_count"] * 0.5, "Es wurde deutlich zu viel weggenommen"
        )

    def test_hoehe_bleibt_trotz_schuhen(self):
        """Schuhe nehmen die Fuesse weg — die Figur darf nicht schrumpfen."""
        schuhe = "shoes/toigo_mj_cloth_shoes"
        if not self._kennt(schuhe):
            self.skipTest("%s nicht in der Bibliothek" % schuhe)
        self.assertAlmostEqual(
            self._netz("?verdeckt=%s" % schuhe)["hoehe"], self._netz()["hoehe"], delta=0.001
        )
    ...
    def _netz(self, abfrage=""):
        antwort = self.client.get("/api/character/mh-figur/basis/netz/%s" % abfrage)
        self.assertEqual(antwort.status_code, 200)
        return json.loads(antwort.content)

    def _kleid(self, kennung):
        antwort = self.client.get("/api/character/mh-figur/garderobe/%s/netz/" % kennung)
    ...
    def _kennt(self, kennung):
        antwort = self.client.get("/api/character/mh-figur/garderobe/")
        return kennung in {s["id"] for s in json.loads(antwort.content)["stuecke"]}

    @staticmethod
    def _naechster_abstand(stoff, haut):
        """Je Stoffpunkt der Abstand zur naechsten Hautstelle, in Metern."""
