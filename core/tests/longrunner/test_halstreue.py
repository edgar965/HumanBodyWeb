# -*- coding: utf-8 -*-
"""Der Retarget beugt den Hals um genau die vorgegebenen Grad.
...
class HalstreueTest(SimpleTestCase):
    ...
    def test_die_quelle_folgt_den_vorgaben(self):
        """Erst die Probe auf den Eichfall selbst.
    ...
    def test_auch_bei_gebeugtem_rumpf(self):
        """Der Hals erbt die Korrektur des Rumpfes.
    ...
    def test_der_hals_ist_in_jedem_format_zugeordnet(self):
        """Ein Zielknochen ohne Zuordnung bleibt stumm in der Ruhelage.
    ...
    def _beugeachse(bvh, nummer):
        kinder = [k for k in range(len(bvh.names)) if int(bvh.parents[k]) == nummer]
        if not kinder:
            return None
    ...
    def _ziel_lagen(skel, quats, bild):
        ...
        for name in skel.bone_order:
            ...
            spur = quats.get(name)
            lokal = (
                np.asarray(spur[bild * 4 : bild * 4 + 4], dtype=float)
                if spur is not None
                else bone.rest_local_quat
            )
    ...
    def eichlauf(pfad, skip=None, rumpfgrad=0.0):
        """``(quellwinkel, zielwinkel)`` je Vorgabe, als Aenderung zu Bild 0.
        ...
        spuren = Retargetlauf(
            bvh,
            skel,
            mapping=bauart.BONE_MAP_TO_RIGIFY,
            skip_bones=liste,
            body_height=1.68,
        ...
        return [(quelle(i) - q0, ziel(i) - z0) for i in range(1, anzahl)]

    @staticmethod
    def _achsdrehung(achse, grad):
        """Quaternion [x, y, z, w] fuer `grad` um `achse`."""
    ...
    def _rumpf_vorbeugen(bvh, quats, q_ra, rumpfgrad):
        """Alle Bilder um `rumpfgrad` am Rumpfknochen vorbeugen — falls
    ...
    @staticmethod
    def fehlende():
        """Eingetragene Dateien, die es nicht gibt.
