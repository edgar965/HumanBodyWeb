# -*- coding: utf-8 -*-
"""UMA gegen UMA Python — vier Gegenproben gegen Unitys eigenen Bau.
...
class Unityreferenz:
    """Die Unity-GLB der Gegenprobe und die in Python gebaute Figur."""
...
class NetzGegenUnity(unittest.TestCase):
    """Fall 1 — dieselbe Rasse, nackt: Netz gegen Netz."""
    ...
    def test_dreieckszahl_ist_dieselbe(self):
        """Der Fall, der `lodRanges` ans Licht brachte: 58.538 statt 29.430.
    ...
    def test_die_huelle_stimmt_ueberein(self):
        ...
        for achse in range(3):
            self.assertAlmostEqual(float(eigen[:, achse].min()), float(unten[achse]), places=2)
            self.assertAlmostEqual(float(eigen[:, achse].max()), float(oben[achse]), places=2)
...
class SkelettGegenUnity(unittest.TestCase):
    """Fall 2 — dasselbe Skelett auf beiden Wegen."""
    ...
    def _weltlagen(self):
        ...
        return {k.name: np.linalg.inv(posen[i])[:3, 3] for i, k in enumerate(self.gebaut.netz.knochen)}
