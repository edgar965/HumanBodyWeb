# -*- coding: utf-8 -*-
u"""Mimik auf SMPL-X: der Übertrag der MB-Lab-Einheiten auf den SMPL-X-Kopf.
...
class SmplxMimikTest(unittest.TestCase):
    ...
    def test_lidschluss_links_trifft_das_linke_oberlid_und_sonst_nichts(self):
        ...
        for seite in ('L', 'R'):
            self.assertEqual(float(np.abs(d[self.ziel.augen[seite]]).max()), 0.0,
                             'Augapfel %s still' % seite)
    ...
    def test_felder_ohne_zunge_und_datei_auf_dem_stand_des_codes(self):
        ...
        for einheit in ('eyeClosedL', 'mouthSmile', 'browsMidVert'):
            self.assertEqual(datei_felder[einheit]['plus']['i'],
                             felder[einheit]['plus']['i'])
            np.testing.assert_allclose(datei_felder[einheit]['plus']['d'],
                                       felder[einheit]['plus']['d'], atol=1e-4)
