# -*- coding: utf-8 -*-
"""UMA-Figur aus dem Figurkatalog: Liste, Datei, Zettel, Regler.
...
class UmafigurTest(TestCase):
    ...
    def _figur(self, name='probe.glb', rasse='Human Female 3.0'):
        ...
        (self.katalog / 'uma' / name[:-4]).with_suffix('.json').write_text(
            json.dumps(
                {'name': name[:-4], 'ruhelage': 'A-Pose', 'hinweis': 'Rasse %s, 3 Kleidungsstücke' % rasse}
            ),
            encoding='utf-8',
        )
    ...
    def test_datei_mit_last_modified_und_304(self):
        ...
        erneut = self.client.get(
            reverse('uma_figur', args=['probe.glb']), HTTP_IF_MODIFIED_SINCE=antwort['Last-Modified']
        )
        self.assertEqual(erneut.status_code, 304)

    def test_falscher_name_400_fehlende_datei_404(self):
        self.assertEqual(
            self.client.get('/api/character/uma-figur/..%2Fx.glb/').status_code, 404
        )  # Django löst `..` nicht auf eine Route auf
    ...
    def test_regler_fehler_in_klartext(self):
        ...
        self.assertEqual(self.client.get(reverse('uma_regler'), {'figur': 'nix.glb'}).status_code, 404)
