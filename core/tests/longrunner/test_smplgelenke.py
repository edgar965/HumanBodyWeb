# -*- coding: utf-8 -*-
"""Smplgelenke — die 24 SMPL-Gelenke aus einem Netz.
...
class SmplgelenkeTest(unittest.TestCase):
    ...
    def test_namen_kommen_aus_der_skelettdefinition(self):
        """Keine zweite Namensliste — sonst driften BVH und Anzeige."""
    ...
    def test_posierte_haltung_bleibt_in_den_gemessenen_schranken(self):
        """A40 gegen die echte Vorwaertskinematik.
    ...
    def _abweichung(self, modell, gelenke, stoerung=0.0):
        """mm je Gelenk: Regressor auf dem posierten Netz gegen Kinematik."""
        ...
        for i in range(len(j_rest)):
            ...
            lokal[:3, 3] = j_rest[i] - (j_rest[eltern] if eltern >= 0 else np.zeros(3))
    ...
    def rodrigues(rv):
        ...
        return np.eye(3) + np.sin(laenge) * kreuz + (1 - np.cos(laenge)) * (kreuz @ kreuz)
