# -*- coding: utf-8 -*-
u"""Filmhaut — Materialgruppen des Körpers im pyrender-Film.
...
class FilmhautTest(SimpleTestCase):
    ...
    @staticmethod
    def kunstnetz():
        u"""Sechs Punkte, vier Dreiecke: Haut, Haut, Iris, Hornhaut."""
        uvs = np.array([[0.1, 0.1], [0.2, 0.1], [0.2, 0.2], [0.1, 0.2],
                        [0.5, 0.5], [0.6, 0.5]], dtype=np.float32)
        dreiecke = np.array([[0, 1, 2], [0, 2, 3], [2, 3, 4], [3, 4, 5]])
        return uvs, dreiecke, np.array([0, 0, 6, 5])

    def test_gruppen_wie_im_browser(self):
        quelle = (VIEWER / 'gemeinsam' / 'koerpermaterialien.js'
                  ).read_text(encoding='utf-8')
        zeilen = re.findall(r'\{ color: 0x([0-9a-f]{6}), roughness: ([0-9.]+)([^}]*)\}',
                            quelle)
    ...
    def test_netze_je_gruppe_haut_mit_textur_hornhaut_durchsichtig(self):
        ...
        haut = self.Filmhaut(uvs, dreiecke, material,
                             {'iris': '#ff0000', 'haut_glanz': 0.8}, bild)
        punkte = np.zeros((6, 3))
