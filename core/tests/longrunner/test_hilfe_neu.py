# -*- coding: utf-8 -*-
"""Hilfe → Kleidung → Neu: die Seite steht, und ihre Zahlen kommen aus Python.
...
class SeiteNeu(SimpleTestCase):
    ...
    def test_die_adresse_ist_die_erwartete(self):
        self.assertEqual(reverse("hilfe_kleidung_neu"), "/hilfe/kleidung/neu/")

    def test_die_kernzahlen_stehen_darin(self):
        """Nicht der Fließtext, sondern die Zahlen: Sie sind der Grund,
        warum es die Seite gibt."""
        for probe in ("16.277", "18.210", "6.890", "347", "274", "Konformer", "GarmentCode", "Fünf Welten"):
    ...
    def test_der_balken_ist_zur_skala_gezeichnet(self):
        """Die Breite MUSS die Zahl sein, nicht nur neben ihr stehen.
    ...
    def test_das_menue_fuehrt_den_punkt(self):
        """Eine Seite ohne Menüpunkt findet niemand."""
        gruppen = settings.DJANGOBASE.get("hilfe_extra") or []
        eintraege = [e for gruppe in gruppen for e in gruppe.get("untermenu", [])]
    ...
    def test_die_mitgelieferten_hilfeseiten_bleiben(self):
        """Der eigene Präfix darf djangoBases Seiten nicht schlucken."""
        for pfad in (
            "/hilfe/versionen/",
            "/hilfe/logs/",
            "/hilfe/tests/",
...
class NeuigkeitenStehenInPython(unittest.TestCase):
    """Die Stammdaten — geprüft ohne Django, weil sie ohne auskommen."""

    databases = set()

    def test_jede_welt_ist_vollstaendig(self):
        felder = ("kuerzel", "name", "formt", "regler", "netz", "kleidung", "bestand")
    ...
    def test_die_sieben_wege_sind_sieben(self):
        ...
        self.assertEqual(
            [w["name"] for w in stark],
            ["UMA-Konformer", "GarmentCode"],
            "Die beiden Wege, die auf JEDEN Körper gehen",
        )
    ...
    def test_es_gibt_drei_stufen_mit_aufwand(self):
        ...
        for stufe in stufen:
            ...
            self.assertTrue(stufe["aufwand"], "Eine Stufe ohne Aufwandsangabe ist ein Wunsch")

    def test_abgrenzung_und_unsicherheit_sind_benannt(self):
        """Ein Vorschlag ohne „was ich NICHT vorschlage" und ohne „was
