# -*- coding: utf-8 -*-
"""Hilfetexte, Wertenamen und Voreinstellungen der GarmentCode-Regler.
...
def _pfade(entwurf):
    """Alle Reglerpfade eines Entwurfs, flach."""
    return _pfade_der_bloecke(Regler.fuer(entwurf))


def _alle_pfade():
    """Reglerpfade über ALLE Kleidungsstücke des Katalogs.
...
class ReglertexteTest(SimpleTestCase):
    """Die Erklärungen gehören zu Reglern, die es gibt."""
...
class ReglerhilfeTest(SimpleTestCase):
    """Wertenamen und die stillen Bedingungen."""
...
class ReglerpresetsTest(SimpleTestCase):
    """Die Voreinstellungen setzen nur Regler, die das Stück wirklich hat."""
    ...
    def test_jeder_wert_liegt_in_seinem_bereich(self):
        """Ein Wert daneben wird vom Server stumm verworfen."""
        bereiche = {}
        for name in Katalog.STUECKE:
            for block in Regler.fuer(Katalog.entwurf(name)):
