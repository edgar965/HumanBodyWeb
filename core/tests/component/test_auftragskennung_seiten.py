# -*- coding: utf-8 -*-
"""Auftragsseiten unter `/process/<Datum.Uhrzeit>/…` — mit Datenbank.
...
class AuftragskennungSeitenTest(TestCase):
    ZEIT = datetime(2026, 9, 16, 20, 0, 12, tzinfo=UTC)

    def setUp(self):
        self.client = Client(HTTP_HOST='127.0.0.1')

    def _auftrag(self, name='probe.mp4', status='complete'):
        with mock.patch('core.daten.auftragskennung.timezone.now', return_value=self.ZEIT):
            return BVHJob.objects.create(
                name=name, pipeline='smplx', status=status, bvh_file=r'A:\irgendwo\ausgabe\smplx_probe.bvh'
            )
    ...
    def test_save_vergibt_die_kennung_aus_der_ortszeit_und_eindeutig(self):
        ...
        erster.save()
        self.assertEqual(
            BVHJob.objects.get(pk=erster.pk).kennung, '2026.09.16.22.00.12', 'ein zweites save ändert nichts'
        )
    ...
    def test_seiten_und_formulare_laufen_ueber_die_kennung(self):
        ...
        self.assertIn('href="/process/2026.09.16.22.00.13/"', ergebnis.content.decode('utf-8'))
        self.assertEqual(self.client.get('/process/2001.01.01.00.00.00/').status_code, 404)
    ...
    def test_alte_uuid_adresse_leitet_dauerhaft_auf_die_kennung(self):
        ...
        for rest in ('', 'result/'):
            ...
            self.assertEqual(antwort['Location'], '/process/2026.09.16.22.00.12/%s' % rest)
        self.assertEqual(self.client.get('/process/%s/' % uuid.uuid4()).status_code, 404)
