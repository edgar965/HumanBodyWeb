# -*- coding: utf-8 -*-
"""UMA-Skelett aus dem Figurkatalog: Endpunkt, Zeiger, Retarget-Ziel.
...
class UmaskelettTest(TestCase):
    ...
    def test_die_figur_blickt_nach_plus_z(self):
        """Die GLB steht um 180 Grad gedreht (`Avatar`); geliefert wird sie
    ...
    def test_unbekanntes_ziel_am_endpunkt_ist_eine_400(self):
        antwort = self.client.get(reverse('retarget'), {'category': 'x', 'name': 'y', 'target': 'quatsch'})
        self.assertEqual(antwort.status_code, 400)
        self.assertIn('Ziel', antwort.json()['error'])

    def test_die_ablage_unterscheidet_die_ziele(self):
        """DEF behaelt den alten Namen (die Ablagen von frueher gelten
        weiter), UMA bekommt einen eigenen."""
        pfad = str(self.katalog / 'x.bvh')
        self.assertEqual(Retargetdaten(pfad).ablage, Retargetdaten(pfad, ziel='def').ablage)
        self.assertNotEqual(Retargetdaten(pfad).ablage, Retargetdaten(pfad, ziel='uma').ablage)

    def test_retarget_auf_uma_ueber_retargetdaten(self):
        self._glb()
        bvh = self.katalog / 'probe.bvh'
        bvh.write_text(Umaattrappe.bvh_text([{}, {'LeftArm': (-45, 0, 0)}]), encoding='utf-8')
    ...
    def test_skelett_je_datei_nicht_nur_das_aus_dem_zeiger(self):
        ...
        self.assertIn('LeftArm', ergebnis.mapped_bones)
        self.assertNotEqual(
            Retargetdaten(str(bvh), ziel='uma', figur='zweite.glb').ablage,
