# -*- coding: utf-8 -*-
"""Retargetendpunkte — die HTTP-Schale um den Retarget.
...
class RetargetendpunkteTest(TestCase):
    ...
    def test_zuordnungstabellen_nennen_je_format_die_richtungskorrektur(self):
        ...
        daten = antwort.json()
        self.assertEqual(set(daten), {"mappings", "skip_dir_correction", "face_hand_bones"})
    ...
    def test_unbekanntes_ziel_400(self):
        antwort = self.client.get("/api/retarget/", {"category": "Probe", "name": "x", "target": "blender"})
        self.assertEqual(antwort.status_code, 400)
        self.assertIn("Unbekanntes Ziel", antwort.json()["error"])

    def test_fehlende_bibliotheksdatei_404(self):
        antwort = self.client.get("/api/retarget/", {"category": "Probe", "name": "fehlt"})
    ...
    def test_der_rumpf_sticht_die_abfrage(self):
        ...
        antwort = self.client.post(
            "/api/retarget/?target=def",
            json.dumps({"target": "blender", "category": "Probe", "name": "x"}),
    ...
    def test_bvh_verwalten_verlangt_post_und_rumpf(self):
        ...
        antwort = self.client.post(reverse("bvh_manage"), "kein json", content_type="application/json")
        self.assertEqual(antwort.status_code, 400)

    def test_bvh_verwalten_unbekannte_aktion(self):
        antwort = self.client.post(
            reverse("bvh_manage"), json.dumps({"action": "zaubern"}), content_type="application/json"
        )
        self.assertEqual(antwort.status_code, 400)
