# -*- coding: utf-8 -*-
"""Einstellungen → Effekte und ihre Wirkung auf die Effekte-Seite.
...
class EinstellungenEffekte(Effektseite):
    ...
    def test_vorgaben_des_modells(self):
        ...
        self.assertEqual(
            (
                s.effekte_default_pipeline,
                s.effekte_default_model,
                s.effekte_video_fps,
                s.effekte_video_width,
                s.effekte_video_height,
                s.effekte_wind,
                s.effekte_windrichtung,
            ),
    ...
    def test_speichern_haelt_die_grenzen_des_registers(self):
        antwort = self.client.post(
            reverse("settings_effekte"),
            {
                "effekte_default_pipeline": "kleid_wind",
                "effekte_default_model": "Female2",
                "effekte_default_animation": "/api/character/bvh/Walk/02_02/",
                "effekte_windrichtung": "vorn",
                "effekte_video_fps": "24",
                "effekte_video_width": "99999",
                "effekte_video_height": "quatsch",
                "effekte_wind": "2.5",
            },
        )
        ...
        s = AppSettings.load()
        self.assertEqual(
            (s.effekte_default_pipeline, s.effekte_windrichtung, s.effekte_video_fps, s.effekte_wind),
