# -*- coding: utf-8 -*-
"""Studioprojekte — Szenen im Modellordner, Projekte auf der Platte.
...
class StudioprojekteTest(TestCase):
    ...
    def setUp(self):
        ...
        self.medien.mkdir()
        umschaltung = override_settings(HUMANBODY_MODELS_DIR=str(self.modelle), MEDIA_ROOT=str(self.medien))
        umschaltung.enable()
        self.addCleanup(umschaltung.disable)

    def _szene(self, name, daten):
        (self.modelle / (name + Studioprojekte.SZENE)).write_text(json.dumps(daten), encoding='utf-8')
    ...
    def test_szenenliste_fuehrt_nur_szenen(self):
        ...
        self.assertEqual(
            antwort.json()['scenes'], [{'name': 'Zimmer', 'label': 'Mein Zimmer', 'character_count': 2}]
        )

    def test_eine_kaputte_szene_bleibt_in_der_liste(self):
        (self.modelle / ('Kaputt' + Studioprojekte.SZENE)).write_text('{nicht json', encoding='utf-8')
        eintraege = self.client.get(reverse('scene_list')).json()['scenes']
        self.assertEqual(eintraege, [{'name': 'Kaputt', 'label': 'Kaputt', 'character_count': 0}])
    ...
    def test_szene_sichern_ohne_brauchbaren_namen(self):
        antwort = self.client.post(
            reverse('scene_save'), json.dumps({'name': '!!!', 'data': {}}), content_type='application/json'
        )
    ...
    def test_projekt_sichern_laden_auflisten(self):
        ...
        liste = self.client.get(reverse('studio_project_list'), {'dir': str(ziel.parent)}).json()['files']
    ...
    def test_projekt_ausserhalb_der_wurzeln_wird_abgelehnt(self):
        ...
        self.assertEqual(
            self.client.get(reverse('studio_project_load'), {'path': str(fremd)}).status_code, 403
        )
        self.assertEqual(
            self.client.get(reverse('studio_project_list'), {'dir': str(self.wurzel)}).status_code, 403
        )
    ...
    def test_projekt_ohne_inhalt_und_ohne_datei(self):
        ...
        self.assertEqual(antwort.status_code, 400)
        fehlt = self.client.get(
            reverse('studio_project_load'), {'path': str(self.medien / 'fehlt.studio.json')}
        )
