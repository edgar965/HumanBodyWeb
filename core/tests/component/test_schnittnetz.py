# -*- coding: utf-8 -*-
"""Der Endpunkt, der die flachen Panels liefert (07.09.2026).
...
class SchnittnetzTest(SimpleTestCase):
    ...
    def test_ein_ausbruch_wird_abgewiesen(self):
        """`…/ausgabe/../../` faengt mit der Wurzel an und zeigt woanders hin."""
        wurzel = os.path.join(str(settings.ASSETS_ROOT), "GarmentCode", "ausgabe")
        for pfad in (
            os.path.join(wurzel, "..", "..", "settings.json"),
            r"C:\Windows\win.ini",
    ...
    @staticmethod
    def _spezifikation():
        """Irgendein fertiger Lauf im Ausgabeordner, oder None."""
        wurzel = os.path.join(str(settings.ASSETS_ROOT), "GarmentCode", "ausgabe")
