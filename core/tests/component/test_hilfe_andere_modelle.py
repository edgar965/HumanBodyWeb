# -*- coding: utf-8 -*-
"""Hilfe -> Architektur -> Andere Modelle: die Seite steht, ihre Zahlen
...
class SeiteAndereModelle(SimpleTestCase):
    ...
    def test_antwortet_unter_deutscher_adresse(self):
        ...
        self.assertEqual(reverse("hilfe_andere_modelle"), "/hilfe/architektur/andere-modelle/")
    ...
    def test_jede_quelle_jedes_urteil_jede_adresse_steht_auf_der_seite(self):
        for kennung, liste in (("koerper", Figurquellen.rangliste()), ("kopf", Figurkoepfe.rangliste())):
            for e in liste:
    ...
    def test_beide_tabellen_sind_djangobase_tabellen(self):
        for kennung in ("koerper", "kopf"):
            ...
            self.assertIn('data-sort-key="hilfe-andere-modelle-%s"' % kennung, self.text)
    ...
    def test_der_rang_folgt_der_vergleichszahl(self):
        for liste in (Figurquellen, Figurkoepfe):
            ...
            for e in liste.ohne_rang():
                self.assertTrue(e["hoechst_dreiecke"] is None or e.get("ohne_rang"), e["name"])

    def test_die_vergleichszahl_steht_in_beiden_tabellen(self):
        for e in Figurquellen.mit_rang() + Figurkoepfe.mit_rang():
            self.assertIn('data-sort="%d">%s<' % (e["hoechst_dreiecke"], e["hoechst_text"]), self.text)
    ...
    def test_mb_lab_original_steht_zum_vergleich_neben_humanbody(self):
        """Edgar: „eine Zeile MB-Lab, damit wir vergleichen können, ob ich
        ...
        self.assertEqual((hb["punkte"], hb["vierecke"]), (mblab["punkte"], mblab["vierecke"]))
    ...
    def test_die_lesehilfe_rechnet_mit_den_zeilen_der_tabelle(self):
        ...
        self.assertIn(
            "%s Punkte, %s Vierecke, %s Dreiecke"
            % (hb["basis_punkte_text"], hb["basis_vierecke_text"], hb["basis_dreiecke_text"]),
            self.text,
        )
        self.assertIn("%s → %s Dreiecke" % (g9["stufen_text"], g9["hoechst_text"]), self.text)
...
class Bildrouten(SimpleTestCase):
    ...
    def test_fremde_pfade_sind_keine_bilder(self):
        ...
        self.assertEqual(
            self.client.get("/hilfe/architektur/andere-modelle/bild/gibt_es_nicht/x.png").status_code, 404
        )
    ...
    def test_original_und_vorschau_kommen_als_bild(self):
        ...
        antwort = self.client.get(reverse("hilfe_andere_modelle_bild", args=[self.ordner, datei]))
        self.assertEqual(antwort.status_code, 200)
        self.assertEqual(antwort["Content-Type"], Figurbilder.typ(datei))
        antwort.close()
        with (
            Pruefablage.ordner("figurbilder_") as ablage,
            mock.patch.object(Figurbilder, "ABLAGE", Path(ablage)),
        ):
            antwort = self.client.get(reverse("hilfe_andere_modelle_vorschau", args=[self.ordner, datei]))
            self.assertEqual(antwort.status_code, 200)
            self.assertEqual(antwort["Content-Type"], "image/jpeg")
            antwort.close()
            ziel = Path(ablage) / self.ordner / (datei + ".jpg")
            self.assertTrue(ziel.is_file())
            self.assertLess(ziel.stat().st_size, (Figurbilder.ORDNER / self.ordner / datei).stat().st_size)
