# -*- coding: utf-8 -*-
"""Hilfe -> Animationseffekte: die Seite steht, ihre Texte kommen aus Python.
...
class SeiteAnimationseffekte(SimpleTestCase):
    ...
    def setUpClass(cls):
        ...
        cls.vorlage = (Path(settings.BASE_DIR) / 'templates' / 'hilfe' / 'animationseffekte.html').read_text(
            encoding='utf-8'
        )

    def test_antwortet_unter_deutscher_adresse(self):
        self.assertEqual(self.antwort.status_code, 200)
        self.assertEqual(reverse('hilfe_animationseffekte'), '/hilfe/animationseffekte/')
    ...
    def test_der_stand_bleibt_ehrlich(self):
        """„läuft" ohne Einschraenkung darf nur sagen, was im Bestand als
        laufend steht; Blender Cloth bleibt als Nicht-Wahl aufgefuehrt."""
        laufend = {b['was'] for b in Animationseffekte.bestand() if b['stand'].startswith('läuft')}
        for k in Effektkandidaten.kandidaten():
