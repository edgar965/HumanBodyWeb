# -*- coding: utf-8 -*-
"""Grundfall der Effekte-Seitenprüfungen: Ablage, Modelle, Client, Start.
...
class Effektseite(TestCase):
    ...
    def setUpClass(cls):
        ...
        if cls.MODELLE is not None:
            ...
            for name, vorlage in cls.MODELLE.items():
                cls.schreiben(os.path.join("models", name + ".json"), json.dumps(vorlage))
        cls.dateien()
    ...
    @classmethod
    def dateien(cls):
        """Die Prüfdateien der Klasse — zum Überschreiben."""

    @classmethod
    def schreiben(cls, name, text):
        """Datei `name` (relativ zur Ablage) mit `text`; gibt den Pfad zurück."""
    ...
    def nutzlast(self, **extra):
        """Die gültige Start-Nutzlast der Klasse, `extra` überschreibt Felder."""
    ...
    def abgewiesen(self, faelle):
        """Jeder Fall `(extra, erwarteter Fehlertext)` bekommt 400, nichts startet."""
