# -*- coding: utf-8 -*-
"""/process/VideoToBVH/ und /process/: die Auftragsliste ist eine
djangoBase-Tabelle.
...
class DieAuftragstabelle(TestCase):
    ...
    def setUp(self):
        self.fertig = BVHJob.objects.create(
            name="Tanz.mp4", video_file="uploads/probe_tanz.mp4", pipeline="gem", status="complete"
        )
        self.laeuft = BVHJob.objects.create(
            name="Lauf.mp4",
            video_file="uploads/probe_lauf.mp4",
            pipeline="hybrid_gem",
            status="processing",
            progress=47,
        ...
        self.zweid = BVHJob.objects.create(
            name="Flach.mp4", video_file="uploads/probe_flach.mp4", pipeline="mediapipe", status="failed"
        )
        self.client = Client()
    ...
    def tabelle(self, text):
        """Das Markup zwischen `id="auftragsliste"` und dem Tabellenende."""
    ...
    def test_die_dreid_seite_rendert_die_djangobase_vorlage(self):
        ...
        self.assertIn(
            '<table class="db-tabelle sortable auftragstabelle" data-sort-key="videoauftraege-3d">', tabelle
        )
    ...
    def test_die_zweid_seite_dieselbe_vorlage_mit_eigenem_schluessel(self):
        ...
        self.assertNotIn(
            'data-id="%s"' % self.fertig.id, tabelle, "3D-Aufträge gehören nicht auf die 2D-Seite"
        )
    ...
    def test_kopfkaestchen_ohne_sortierpfeil_knoepfe_ohne_sortierung(self):
        ...
        kopf = Sicher.wert(re.search(r'<th[^>]*data-key="wahl"[^>]*>(.*?)</th>', tabelle, re.S), "Kopf")
    ...
    def test_der_loeschknopf_beginnt_gesperrt_mit_null(self):
        text = self.seite(self.DREID)
        knopf = Sicher.wert(
            re.search(r'<button[^>]*id="bulk-delete-btn"[^>]*>(.*?)</button>', text, re.S), "Knopf"
        )
        self.assertIn("disabled", knopf.group(0))
        self.assertIn('Auswahl löschen (<span id="bulk-count">0</span>)', knopf.group(1))
