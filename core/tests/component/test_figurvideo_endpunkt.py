# -*- coding: utf-8 -*-
"""`POST /api/animation/video/` nimmt den Auftrag als Formular entgegen.
...
class FigurvideoEndpunktTest(SimpleTestCase):
    ...
    def _starten(self, **felder):
        with mock.patch.object(endpunkt.Figurvideo, 'starten', return_value='abc123') as attrappe:
            antwort = self.client.post(reverse('figurvideo_starten'), felder)
        return antwort, attrappe

    def test_formular_mit_stueck(self):
        auftrag = {
            'bvh_url': '/api/character/bvh/Walk/136_28/',
            'stuecke': [{'name': 'Hose', 'datei': 'stueck_0', 'punkte': 3, 'dreiecke': 1}],
            'knochen': ['DEF-spine'],
        }
        antwort, attrappe = self._starten(
            auftrag=json.dumps(auftrag), stueck_0=SimpleUploadedFile('stueck_0.bin', b'\x00' * 12)
        )
    ...
    def test_json_rumpf_geht_weiter(self):
        with mock.patch.object(endpunkt.Figurvideo, 'starten', return_value='def456') as attrappe:
    ...
    def test_wertfehler_wird_400(self):
        with mock.patch.object(endpunkt.Figurvideo, 'starten', side_effect=ValueError('Stueck ohne Daten')):
            antwort = self.client.post(
                reverse('figurvideo_starten'), {'auftrag': json.dumps({'bvh_url': '/x/y/'})}
            )
        self.assertEqual(antwort.status_code, 400)
    ...
    def test_aufnahme_reicht_ablage_durch(self):
        with mock.patch.object(
            endpunkt.Figurvideo, 'aus_bildfolge', return_value=('k1', '/media/x.mp4', PFAD)
        ) as attrappe:
            antwort = self.client.post(
        ...
        self.assertEqual(
            attrappe.call_args.kwargs['ablage'],
            {'ordner': ORDNER, 'name': 'a', 'figur': 'F', 'animation': 'Walk_1'},
        )

    def test_aufnahme_falsche_ablage_400(self):
        with mock.patch.object(
            endpunkt.Figurvideo, 'aus_bildfolge', side_effect=ValueError('vollständiger Pfad')
        ):
            antwort = self.client.post(
                reverse('figurvideo_aufnahme'),
                {'frames': SimpleUploadedFile('000000.png', PNG), 'ablage': 'relativ'},
            )
        self.assertEqual(antwort.status_code, 400)
