# -*- coding: utf-8 -*-
"""Retargetvorrat: der Retarget liegt neben jedem BVH, bevor der Auftrag fertig ist.
...
class RetargetvorratTest(TestCase):
    ...
    def _auftrag(self):
        job = BVHJob.objects.create(name='tanz.mp4', pipeline='gem', status='processing')
        ...
        for kennung in ('gem', 'gem_face', 'gem_p2'):
            (self.bibliothek / ('tanz_%s.bvh' % kennung)).write_text(BVH, encoding='utf-8')
        return job
    ...
    def test_ablage_neben_bvh_und_bibliothekskopie(self):
        ...
        namen = sorted(p.parent.name + '/' + p.name.split('_retarget_')[0] for p in ablagen)
        ...
        self.assertEqual(
            ...
            sorted(
                [
                    'A_Results/tanz_gem',
                    'A_Results/tanz_gem_face',
                    'A_Results/tanz_gem_p2',
                    eigene + '/tanz_gem',
                    eigene + '/tanz_face',
                    eigene + '/tanz_gem_p2',
                ]
        ...
        self.assertEqual(
            meldungen,
            [
                'Bewegung wird umgesetzt (tanz_gem.bvh) …',
    ...
    def test_scheitern_der_rechnung_laesst_den_auftrag_fertig_werden(self):
        job = self._auftrag()
        with mock.patch(
            'core.dienste.retargetdaten.Retargetdaten._rechnen', side_effect=RuntimeError('kaputt')
        ):
            Auftragsabschluss.fertig_mit_vorrat(job, 'Done')
        job.refresh_from_db()
        self.assertEqual((job.status, job.progress, job.progress_detail), ('complete', 100, 'Done'))
    ...
    def test_fertig_mit_vorrat_rechnet_vor_complete(self):
        ...
        lauf = (settings.BASE_DIR / 'core' / 'pipelines' / 'auftragslauf.py').read_text(encoding='utf-8')
