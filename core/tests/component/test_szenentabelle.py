# -*- coding: utf-8 -*-
"""Die Lichttabelle der SMPL-Einstellungen — mit gespeicherter Szene.
...
class SzenentabelleTest(TestCase):
    """Was die Schleife aus den gespeicherten Szenendaten macht."""

    def setUp(self):
        self.einstellungen = AppSettings.objects.create(smpl_default_scene=json.dumps(SZENE))

    def test_alle_vier_lichter_in_anzeigereihenfolge(self):
        lichter = SmplEinstellungen._lichter(SZENE)
        self.assertEqual(
            [licht['name'] for licht in lichter], ['Key Light', 'Fill Light', 'Back Light', 'Ambient']
        )
        self.assertEqual(lichter[0], {'name': 'Key Light', 'intensity': 1.2, 'color': '#ffffff'})
        self.assertEqual(lichter[3]['color'], '#404040')

    def test_fehlendes_licht_faellt_aus_der_liste(self):
        ohne = {'lighting': {k: v for k, v in SZENE['lighting'].items() if k != 'back'}}
    ...
    def test_keine_szene_gibt_leere_liste(self):
        """Kein Aufschrei, keine Zeile — die Karte zeigt dann ihren Hinweis."""
    ...
    def test_die_seite_zeigt_alle_vier_zeilen(self):
        ...
        for name, intensitaet, farbe in (
            ('Key Light', '1.2', '#ffffff'),
    ...
    def test_erste_zeile_behaelt_ihre_eigene_spaltenklasse(self):
        """`szenenspalte` steht nur in der ersten Zeile — wie vor dem Umbau.
