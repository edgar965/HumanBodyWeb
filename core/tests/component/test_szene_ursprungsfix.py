# -*- coding: utf-8 -*-
"""Szene → Animation → „Animation immer auf Ursprungspunkt".
...
class DerUrsprungsfix(TestCase):
    ...
    def anwenden(self, radius_m=RADIUS_M):
        antwort = self.client.post(
            self.URL,
            data={'category': 'Test', 'name': 'probe', 'fixed_radius': radius_m},
    ...
    def abstaende(self):
        """Abstand jedes Bildes zu Bild 0 in der Bodenebene (BVH-Zentimeter)."""
    ...
    def test_die_wurzel_bleibt_im_kreis_um_bild_null(self):
        ...
        for bild in nachher:
            self.assertLessEqual(
                math.hypot(bild[0] - nachher[0][0], bild[2] - nachher[0][2]), RADIUS_M * 100 + 1e-3
            )
    ...
    def test_radius_null_haelt_die_wurzel_auf_bild_null(self):
        """0 hieß auf dem Server „nichts tun" — jetzt ist es der engste Radius."""
    ...
    def test_die_bibliothek_bleibt_unberuehrt(self):
        """Die Prüfung schreibt nur in die Prüfablage."""
    ...
    @staticmethod
    def bewegung():
        """Die Wurzel wandert in 30 Bildern 300 cm nach +X und 200 cm nach −Z."""
        zeilen = []
...
class DasUrsprungsfixDrahtformat(TestCase):
    """Menüeintrag, Dispatch und Modul gehören zusammen."""
    ...
    def test_der_dialog_fragt_den_radius_von_null_an(self):
        """Regler, Anzeige und Knopf des Dialogs heißen so, wie das Modul sie ruft."""
        self.assertIn(
        ...
        for kennung in (
            'ursprungsfix-dialog',
            'ursprungsfix-radius',
            'ursprungsfix-radius-val',
            'ursprungsfix-confirm',
        ):
