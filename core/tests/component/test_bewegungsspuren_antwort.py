...
class BewegungsspurenAntwortTest(TestCase):
    ...
    def bvh_datei():
        ...
        (`projektpfade.md`).
        """
        return os.path.join(Bvhverzeichnis().wurzel(), KATEGORIE, NAME + ".bvh")
    ...
    def test_bibliotheks_bvh_antwortet_mit_allen_feldern(self):
        ...
        if not os.path.isfile(self.bvh_datei()):
            ...
            self.assertEqual(
                antwort.status_code,
                404,
                "Ohne Datei muss der Endpunkt 404 melden, nicht %d" % antwort.status_code,
            )
            return
        antwort = self.client.get(adresse)
        self.assertEqual(
            antwort.status_code, 200, "Retarget-Endpunkt antwortet nicht: %s" % antwort.content[:200]
        )
        daten = json.loads(antwort.content)
        self.assertEqual(
            sorted(daten), sorted(Bewegungsspuren.FELDER), "Die Felder der Schnittstelle haben sich geändert"
        )
    ...
    def test_antwort_traegt_wirklich_bewegung(self):
        ...
        self.assertEqual(len(daten["times"]), daten["frame_count"], "zu jedem Bild gehört ein Zeitpunkt")
    ...
    def test_datensatz_und_json_tragen_dasselbe(self):
        ...
        self.assertEqual(
            Bewegungsspuren.aus_dict(als_json).als_dict(), spuren.als_dict(), "JSON-Umlauf verliert Felder"
        )
    ...
    def test_leerer_datensatz_traegt_alle_felder(self):
        ...
        Feldern."""
        self.assertEqual(sorted(Bewegungsspuren.leer().als_dict()), sorted(Bewegungsspuren.FELDER))
