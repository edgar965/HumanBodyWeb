# -*- coding: utf-8 -*-
"""Wer ueber `localhost` kommt, landet auf 127.0.0.1.
...
class Weiterleitung(TestCase):
    ...
    def test_kein_dauerhafter_umzug(self):
        """301 bliebe im Browser stehen — auch nach einem Serverumbau."""
    ...
    def test_ein_modul_wird_nicht_umgeleitet(self):
        """Ein ES-Modul von fremder Herkunft laedt ohne CORS-Kopf NICHT.
    ...
    def test_ein_post_wird_nicht_umgeleitet(self):
        """Beim Weiterleiten ginge der Rumpf verloren."""
        anfrage = self.werk.post(
            '/api/garmentcode/drapieren/', {'a': '1'}, HTTP_HOST='localhost:8081', HTTP_ACCEPT=HTML
        )
        self.assertEqual(self.schicht(anfrage), 'durchgereicht')
    ...
    def test_im_betrieb_nicht(self):
        """Dort entscheidet der Vorschaltserver ueber Namen."""
        self.assertEqual(self.schicht(self._seite()), 'durchgereicht')

    def test_sie_haengt_wirklich_in_der_kette(self):
        """Sonst prueft dieser Test nur seine eigene Kopie."""
        from django.conf import settings

        self.assertIn('ui.schnelleadresse.Schnelleadresse', settings.MIDDLEWARE)
        # Und zwar VOR allem anderen: Eine Anfrage, die ohnehin weiterzieht,
        # soll keine Sitzung anlegen und keine Ansicht rechnen.
        self.assertEqual(settings.MIDDLEWARE[0], 'ui.schnelleadresse.Schnelleadresse')
