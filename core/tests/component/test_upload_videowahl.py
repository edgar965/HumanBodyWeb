# -*- coding: utf-8 -*-
"""/process/VideoToBVH/: nach dem Hochladen ist das neue Video das gewählte.
...
class NachDemHochladen(TestCase):
    ...
    def setUp(self):
        ...
        self.altjob = BVHJob.objects.create(
            name="alt.mp4", video_file="uploads/alt.mp4", pipeline="gem", status="complete"
        )
        self.vorlieben({"selected_video_path": Videoauswahl.pfad_von(self.altjob), "last_pipeline": "smplx"})
        self.client = Client()
    ...
    def hochladen(self, name="neu.mp4"):
        antwort = self.client.post(
            self.DREID,
            {"video": SimpleUploadedFile(name, b"\x00" * 16, content_type="video/mp4"), "pipeline": "v4"},
        )
    ...
    def test_das_neue_video_ist_danach_das_gewaehlte(self):
        self.assertTrue(
            self.radios()[Videoauswahl.pfad_von(self.altjob)], "Vorbedingung: das alte Video ist gewählt"
        )
        neu = self.hochladen()
        radios = self.radios()
    ...
    def test_der_upload_liegt_in_der_pruefablage(self):
        """Sonst schriebe diese Prüfung in `media/uploads/`."""
        neu = self.hochladen()
        self.assertTrue(Videoauswahl.pfad_von(neu).startswith(str(Path(self.ordner).resolve())))


class DerGemerktePfad(TestCase):
    """`merken` schreibt genau den Pfad, den die Liste führt."""
    ...
    def setUp(self):
        ...
        self.job = BVHJob.objects.create(name="probe.mp4", video_file="uploads/probe.mp4", pipeline="gem")
