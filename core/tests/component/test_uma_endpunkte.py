# -*- coding: utf-8 -*-
"""Zwei UMA-Endpunkte: der Baustand und die Garderobe.
...
class Garderobenattrappe:
    """Antwortet wie `UMA_Python.Garderobe.fuer_rassen`, merkt sich die Frage."""
    ...
    def fuer_rassen(self, rassen):
        ...
        return [
            {'platz': 'Chest', 'rezepte': [{'name': 'Hoodie'}, {'name': 'Bra'}]},
...
class UmaEndpunkteTest(SimpleTestCase):
    ...
    def setUp(self):
        ...
        self.garderobenpatch = mock.patch(
            'core.api.umakleidung.Umakleidung.garderobe', return_value=self.garderobe
        )
        self.garderobenpatch.start()
        self.addCleanup(self.garderobenpatch.stop)
        p = mock.patch(
            'core.api.umakleidung.Umabauer.rassen_details',
    ...
    def test_baustand_liefert_den_stand(self):
        ...
        with mock.patch('core.api.umafigur.Umabauer.stand', return_value=stand) as frage:
    ...
    def test_garderobe_einer_rasse_ohne_vertraegliche(self):
        daten = self.client.get(reverse('uma_garderobe'), {'rasse': 'Human Male 3.0'}).json()
    ...
    def test_garderobe_ohne_angabe_und_mit_unbekannter_figur(self):
        ...
        self.assertEqual(self.client.get(reverse('uma_garderobe'), {'figur': 'fehlt.glb'}).status_code, 404)
        self.assertEqual(self.client.get(reverse('uma_garderobe'), {'figur': '../x.glb'}).status_code, 400)
