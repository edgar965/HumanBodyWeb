# -*- coding: utf-8 -*-
"""Modelle und UMA-Figuren aus dem Dialog umbenennen und löschen.
...
class KatalogverwaltungTest(TestCase):
    def setUp(self):
        ...
        self.modelle.mkdir()
        umschaltung = override_settings(FIGUREN_KATALOG=self.katalog, HUMANBODY_MODELS_DIR=self.modelle)
        umschaltung.enable()
    ...
    def _zeiger(self, inhalt):
        (self.katalog / "aktuell.json").write_text(json.dumps(inhalt), encoding="utf-8")
    ...
    def _post(self, route, daten):
        return self.client.post(reverse(route), data=json.dumps(daten), content_type="application/json")

    def _da(self, *namen):
        return sorted(p.name for p in (self.katalog / "uma").iterdir() if p.name in namen or True)

    # -- UMA umbenennen -------------------------------------------------------

    def test_umbenennen_nimmt_den_zettel_mit(self):
        self._figur("Alt")
        antwort = self._post("katalog_uma_umbenennen", {"alt": "Alt.glb", "neu": "Neu.glb"})
    ...
    def test_umbenennen_zieht_den_zeiger_nach(self):
        ...
        self.assertEqual(self._zeiger_lesen(), {"uma": "Neu.glb", "humanbody": "Rig2.glb"})
    ...
    def test_umbenennen_ueberschreibt_keine_vorhandene_figur(self):
        self._figur("Alt")
        self._figur("Belegt")
        antwort = self._post("katalog_uma_umbenennen", {"alt": "Alt.glb", "neu": "Belegt.glb"})
    ...
    def test_ausbruch_aus_dem_ordner_wird_abgewiesen(self):
        ...
        for neu in ("../weg.glb", "unter/weg.glb", "..\\weg.glb"):
            antwort = self._post("katalog_uma_umbenennen", {"alt": "Alt.glb", "neu": neu})
            self.assertEqual(antwort.status_code, 400, neu)
        self.assertTrue((self.katalog / "uma" / "Alt.glb").exists())

    def test_nur_glb_dateien(self):
        antwort = self._post("katalog_uma_umbenennen", {"alt": "Alt.txt", "neu": "Neu.txt"})
        self.assertEqual(antwort.status_code, 400)
    ...
    def test_modell_umbenennen_und_loeschen(self):
        self._modell("Alt")
        antwort = self._post("katalog_modell_umbenennen", {"alt": "Alt", "neu": "Neu"})
        self.assertEqual(antwort.status_code, 200, antwort.content)
    ...
    def test_die_szenendatei_daneben_bleibt_liegen(self):
        """`<name>.scene.json` ist eine Szene, kein Modell."""
        self._modell("Figur")
        (self.modelle / "Figur.scene.json").write_text("{}", encoding="utf-8")
        self._post("katalog_modell_loeschen", {"name": "Figur"})
        self.assertEqual([p.name for p in self.modelle.iterdir()], ["Figur.scene.json"])
    ...
    def test_ohne_namen_kein_vorgang(self):
        ...
        self.assertEqual(self._post("katalog_uma_umbenennen", {"alt": "X.glb"}).status_code, 400)

    def test_gleicher_name_ist_kein_fehler(self):
        self._figur("Gleich")
        antwort = self._post("katalog_uma_umbenennen", {"alt": "Gleich.glb", "neu": "Gleich.glb"})
