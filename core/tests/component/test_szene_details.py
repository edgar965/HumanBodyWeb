# -*- coding: utf-8 -*-
"""Szene: die Bereiche Haut · Augen · Augenbrauen · Mund · Nägel im Reiter „Modell".
...
class SzeneDetails(TestCase):
    ...
    def test_fuenf_klappbare_bereiche_mit_reset(self):
        for name in BEREICHE:
            muster = (
                r'<div class="panel-section prop-section collapsed" '
                r'id="prop-details-%s">\s*<h3>[^<]+<span class="chevron">' % name
            )
            self.assertRegex(self.text, muster)
            self.assertRegex(
                self.text,
                r'class="btn-reset-morphs prop-detail-reset" '
    ...
    def test_jede_kennung_der_tabelle_steht_auf_der_seite(self):
        ...
        for name in morphe:
            muster = (
                r'<input type="range" id="prop-detail-morph-%s" '
    ...
    def test_die_uebrigen_bereiche_des_reiters_klappen_ebenso(self):
        for kennung in KLAPPBAR:
            self.assertRegex(
                self.text,
                r'<div class="panel-section prop-section[^"]*" '
                r'id="%s">\s*<h3>' % kennung,
                kennung,
            )
        self.assertLess(
            self.text.index('id="prop-details-section"'), self.text.index('id="prop-morphs-section"')
        )
