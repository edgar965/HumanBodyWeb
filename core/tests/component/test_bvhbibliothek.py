...
class BvhbibliothekTest(TestCase):
    ...
    def setUpTestData(cls):
        for i in range(75):
            BVHFile.objects.create(name='walk_%03d' % i, path='/x/walk_%03d.bvh' % i, source='cmu')
        for i in range(5):
            BVHFile.objects.create(name='jump_%03d' % i, path='/x/jump_%03d.bvh' % i, source='gvhmr')
    ...
    def test_suche_und_quelle_wirken_zusammen(self):
        ...
        self.assertEqual(Bvhbibliothek(suche='walk', quelle='gvhmr').seiteninhalt().paginator.count, 0)
    ...
    def test_zusatzfrage_erhaelt_filter_in_seitenlinks(self):
        self.assertEqual(Bvhbibliothek(suche='a', quelle='b').zusatzfrage(), '&q=a&source=b')
    ...
    def test_seite_bleibt_klein(self):
        ...
        self.assertEqual(antwort.content.count(b'class="bvh-card"'), Bvhbibliothek.JE_SEITE)
