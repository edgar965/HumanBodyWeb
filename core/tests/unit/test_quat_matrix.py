# -*- coding: utf-8 -*-
"""`Quat.matrix` und `Quat.from_matrix` — hin und zurueck, alle vier Zweige.
...
class QuatMatrixTest(SimpleTestCase):
    ...
    def test_vierteldrehung_um_y_legt_x_auf_minus_z(self):
        self.assertTrue(np.allclose(Quat.matrix(self._um((0, 1, 0), 90)) @ [1, 0, 0], [0, 0, -1]))

    def test_hin_und_zurueck_alle_vier_zweige(self):
        """Kleine Drehung (Spur > 0) und die drei halben Drehungen (Spur -1)."""
        for q in (
            self._um((1, 2, 3), 37),
            self._um((1, 0, 0), 180),
            self._um((0, 1, 0), 180),
    ...
    def test_zeilenweise_fuer_ein_ganzes_feld(self):
        """``reihe @ M.T`` — so nutzt es die Wurzelspur."""
