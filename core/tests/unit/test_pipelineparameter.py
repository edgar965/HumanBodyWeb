# -*- coding: utf-8 -*-
"""GVHMR-Formular: die vier Schalter, die nie ankamen (12.09.2026).
...
class Einstellungen(SimpleNamespace):
    """Nur, was `Smplbefehl` für GVHMR liest."""
...
class GvhmrFormularTest(SimpleTestCase):
    ...
    def test_die_vier_schalter_kommen_im_auftrag_an(self):
        p = Pipelineparameter.lesen(
            {'gvhmr_use_dpvo': 'on', 'gvhmr_verbose': 'on', 'gvhmr_smooth_sigma': '1.5'}, 'gvhmr'
        )
    ...
    def test_der_befehl_traegt_die_schalter_weiter(self):
        p = Pipelineparameter.lesen(
            {'gvhmr_use_dpvo': 'on', 'gvhmr_smooth_sigma': '1.5', 'gvhmr_joint_limits': 'on'}, 'gvhmr'
        )
