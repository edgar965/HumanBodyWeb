# -*- coding: utf-8 -*-
"""Die bildweise Vorstufe fuer GVHMR und GEM — der Teil ohne Grafikkarte
(12.09.2026).
...
class DieKommandozeileDerVorstufe(unittest.TestCase):
    ...
    def test_brennweite_null_bleibt_weg(self):
        ...
        self.assertNotIn('--kamera_verfolgen', Vorstufe.befehl('tanz.mp4', 'aus'))
    ...
    def test_gem_faehrt_gem_vorhersage_statt_des_demos(self):
        ...
        self.assertEqual(lauf.vorstufenordner('aus'), os.path.join('aus', 'tanz', 'preprocess'))

    def test_gvhmr_vorstufenordner_liegt_neben_dem_ergebnis(self):
        lauf = Gvhmrlauf('tanz.mp4', os.path.join('aus', 'tanz.bvh'))
        self.assertEqual(lauf.vorstufenordner(), os.path.join('aus', 'tanz', 'preprocess'))
        self.assertEqual(os.path.dirname(lauf.ergebnisdatei('aus')), os.path.dirname(lauf.vorstufenordner()))
...
class DieDateien(unittest.TestCase):
    def test_fehlende_werden_genannt_slam_nur_mit_verfolgung(self):
        with Pruefablage.ordner() as ordner:
            ...
            self.assertEqual(sorted(ohne.fehlende()), ['bbx', 'vit_features', 'vitpose'])
    ...
    def test_namen_sind_die_von_gems_demo(self):
        ...
        text = ''.join((demo / d).read_text(encoding='utf-8') for d in ('demo_utils.py', 'demo_smpl_hpe.py'))
...
class DieStuetzstellen(unittest.TestCase):
    def test_wie_simplevo(self):
        ...
        for laenge in (1, 7, 8, 9, 16, 17, 298, 7538):
            ...
            gewaehlt = [i for i in range(laenge) if Kameraverfolgung.ist_stuetzstelle(i, laenge)]
    ...
    def test_der_schritt_ist_der_von_simplevo(self):
        ...
        if not demo.is_file():
            self.skipTest('GVHMR nicht eingelagert')
        treffer = re.search(
            r'SimpleVO\(cfg\.video_path, scale=([\d.]+), '
            r'step=(\d+)',
