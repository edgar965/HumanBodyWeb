# -*- coding: utf-8 -*-
"""Die 2D-Erkennung von VideoToBVH: CSV-Format, Modellgroesse, Verteiler.
...
class DasCsvFormat(unittest.TestCase):
    """MocapNET liest die Spalten nach POSITION, nicht nach Name."""
    ...
    def test_abschnittslaengen_stehen_fest(self):
        """Eine Zeile mehr oder weniger verschiebt alles dahinter."""
    ...
    def test_bildgroesse_null_ergibt_keine_division(self):
        """Ein Video ohne lesbare Masse darf nicht abstuerzen."""
    ...
    def test_ohne_zuversicht_kein_errechnetes_gelenk(self):
        """Zwei unerkannte Schultern ergeben keinen Hals bei (0,0)."""
        coco = np.zeros((17, 3), dtype=np.float32)
        coco[5] = [100, 200, 0.0]
        coco[6] = [200, 200, 0.0]
        self.assertEqual(list(Koerperpunkte.aus_coco(coco, np)[1]), [0.0, 0.0, 0.0])


class DieModellgroesse(unittest.TestCase):
    """Die groesste Wahl muss das groesste Netz laden."""

    def test_x_ist_die_groesste_stufe(self):
        """Der Befund: `x` fuehrte auf `lightweight`, das kleinste Netz."""
    ...
    def test_vitpose_stufen_wirken(self):
        """b/l/h aus `vitpose_model_size` ergeben drei Antworten."""
    ...
    def test_unbekanntes_faellt_auf_die_mitte(self):
        """Ein Tippfehler darf nicht heimlich das kleinste Netz waehlen."""
        for wert in ('', None, 'quatsch', 'L '):
            with self.subTest(wert=wert):
                self.assertIn(Modellgroesse.modus(wert), (Modellgroesse.MITTEL, Modellgroesse.GROSS))
...
class DerVerteiler(unittest.TestCase):
    """Die Namensliste stand frueher zweimal — hier nur noch einmal."""

    def test_erkenner_decken_die_django_seite(self):
        from core.pipelines.erkennung2d import Erkennung2d

        self.assertLessEqual(set(Erkennung2d.MODELLFELD), set(Erkennerwahl.namen()))
    ...
    def test_lifter_kennen_die_schalter_der_django_seite(self):
        """`Smplbefehl` baut die Kommandozeile, `Lifterwahl` liest sie."""
    ...
    def test_fremde_schalter_fallen_weg(self):
        """Der Verteiler nimmt die Schalter ALLER Pipelines entgegen."""
...
class DerVideolauf(unittest.TestCase):
    """TOTAL und PROGRESS liest der Django-Prozess Zeile fuer Zeile."""
    ...
    def test_meldet_gesamtzahl_und_fortschritt(self):
        ...
        self.assertEqual(
            [z for z in zeilen if z.startswith('PROGRESS:')], ['PROGRESS:1/3', 'PROGRESS:2/3', 'PROGRESS:3/3']
        )
    ...
    def test_rtmlib_ergebnisse_in_allen_formen(self):
        """rtmlib liefert je nach Modell (Punkte, Guete) oder nur Punkte."""
        punkte = np.zeros((1, 17, 3), dtype=np.float32)
        punkte[0, 0] = [10, 20, 0.9]
        for name, ergebnis in (
            ('Tupel', (punkte, None)),
            ('nur Punkte', punkte),
