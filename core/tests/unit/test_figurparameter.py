# -*- coding: utf-8 -*-
"""Figurparameter und Modellfigur — die bpy-freie Haelfte der Figur-Pipeline.
...
class FigurparameterTest(SimpleTestCase):
    def test_vorgaben_grenzen_und_pflicht(self):
        ...
        self.assertEqual((p.bilder, p.fps, p.breite, p.hoehe, p.physik, p.ab), (120, 30, 720, 900, 0.0, 0.0))
        self.assertEqual(
            (p.stoff, p.wind, p.windrichtung, p.bund, p.teilschritte), (True, 4.0, 'seite', 3.0, 20)
        )
        p = Figurparameter(modell='m.json', bvh='a.bvh', ausgabe='o.mp4', bilder=99999, physik=-5, fps='24')
    ...
    def test_argumente_und_argv_sind_ein_kreis(self):
        ...
        arg = p.argumente()
        self.assertEqual(
            arg[:8], ['--modell', 'm.json', '--bvh', 'a.bvh', '--ausgabe', 'o.mp4', '--windrichtung', 'vorn']
        )
    ...
    def test_garmentcode_stueck_ueber_die_rig_adresse(self):
        ...
        m = self._modell(
            garmentcode=[
                {
                    'stueck': 'hose',
                    'rig_url': '/api/garmentcode/datei/_szene_Probe/hose_rig.json/',
        ...
        fremd = self._modell(
            garmentcode=[{'stueck': 'x', 'rig_url': '/api/garmentcode/datei/../../x_rig.json/'}]
        )
        with mock.patch('GarmentCode.entwurf.Entwurf.AUSGABE', wurzel):
            with self.assertRaises(ValueError):
