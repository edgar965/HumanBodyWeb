...
class ZuordnungTest(SimpleTestCase):
    def test_verhaeltnismaessig_wie_in_threejs(self):
        for bild, erwartet in ((0, 0), (50, 25), (100, 50)):
            self.assertEqual(Skelettfilm.zuordnen(bild, 101, 51), erwartet, 'Bild %d' % bild)
...
class ZeichnerTest(SimpleTestCase):
    ...
    def test_linie_wird_gemalt(self):
        bild = self.bild()
        Skelettzeichner([('a', 'b')]).zeichnen(bild, {'a': (10, 50, 0.9), 'b': (190, 50, 0.9)})
    ...
    def test_unsicherer_punkt_bekommt_keine_linie(self):
        ...
        """
        bild = self.bild()
        Skelettzeichner([('a', 'b')]).zeichnen(bild, {'a': (10, 50, 0.3), 'b': (190, 50, 0.9)})
        gemalt = np.argwhere(bild.sum(axis=2) > 0)
        self.assertTrue(len(gemalt))
        self.assertGreater(gemalt[:, 1].min(), 180, 'nur um den sicheren Punkt herum, keine Linie nach links')

    def test_punkt_ausserhalb_des_bildes_wird_nicht_gemalt(self):
        bild = self.bild()
        Skelettzeichner([('a', 'b')]).zeichnen(bild, {'a': (200, 50, 0.9), 'b': (100, 50, 0.9)})
...
class ZielpfadTest(SimpleTestCase):
    ...
    def test_bvh_rig_ist_eine_dritte_datei(self):
        ...
        self.assertEqual(Skelettfilm(auftrag, ueberlagern=False).zielpfad().name, 'v4_tanz_rig_only_bvh.mp4')
