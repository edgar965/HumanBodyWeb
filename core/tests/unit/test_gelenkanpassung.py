# -*- coding: utf-8 -*-
"""Das Skelett folgt dem Netz — und steht still, wenn das Netz stillsteht.
...
class Wuerfelnetz:
    """Das Testnetz — ein Gitter von Punkten auf und in einem Würfel.
...
class DieRuhelageBleibtDieRuhelage(SimpleTestCase):
    """Unverändertes Netz, unverändertes Skelett — auf die letzte Stelle."""
    ...
    def test_die_lokalen_lagen_sind_zahl_fuer_zahl_die_datei(self):
        """Keine Toleranz — und zwar auf dem Weg, der wirklich hinausgeht.
    ...
    def test_auch_die_weltkoepfe_stimmen(self):
        """`weltkoepfe` rundet nicht — hier bleibt der Staub aus der SVD
        ...
        np.testing.assert_allclose(self.anpassung.weltkoepfe(self.netz), erwartet, atol=1e-12)


class EineVerschiebungWandertMit(SimpleTestCase):
    """Das Netz als Ganzes versetzt: Die Knochen versetzen sich genauso,
    ...
    def test_die_koepfe_folgen_dem_netz(self):
        np.testing.assert_allclose(
            ...
            self.anpassung.koepfe + np.array([0.0, 0.0, 2.0]),
            atol=1e-9,
        )
...
class EinGroesseresNetzErgibtEinGroesseresSkelett(SimpleTestCase):
    """Der gemeldete Fall: Der Körper wächst, das Skelett soll mitwachsen."""
...
class EineDrehungIstKeineVerzerrung(SimpleTestCase):
    """Ein gedrehtes Netz darf das Skelett drehen, aber nicht strecken.
    ...
    def test_die_kettenlaenge_bleibt(self):
        ...
        winkel = np.pi / 3
        drehung = np.array(
            [[np.cos(winkel), -np.sin(winkel), 0.0], [np.sin(winkel), np.cos(winkel), 0.0], [0.0, 0.0, 1.0]]
        )
...
class DieKnochenlisteBehaeltIhreForm(SimpleTestCase):
    """`knochenliste` liefert dieselbe Struktur wie `def_skeleton.json`.
...
class EinZyklusInDerElternkette(SimpleTestCase):
    """Die Datei kommt von außen — eine Endlosrekursion wäre ein Hänger
