# -*- coding: utf-8 -*-
u"""`Videoschreiber` und `Bakedatei.auf_dem_boden` — je EINE Stelle.
...
class DerVideoschreiber(SimpleTestCase):
    ...
    def test_die_skripte_rendern_und_schreiben_nur_ueber_den_videoschreiber(self):
        ...
        for datei in ('ModelPhysik/angezogen_video.py',
                      'ModelPhysik/bake_nach_video.py', 'ModelPhysik/filmrender.py',
...
class DasBakeAufDemBoden(SimpleTestCase):
    ...
    def test_massstab_aus_dem_ersten_bild_mitte_und_boden_bei_null(self):
        ...
        np.testing.assert_allclose(punkte[0][:, :2].mean(axis=0), [0.0, 0.0],
                                   atol=1e-12)
