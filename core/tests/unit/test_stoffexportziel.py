...
class DateinameTest(SimpleTestCase):
    ...
    def test_zwei_motoren_zwei_dateien(self):
        ...
        self.assertNotEqual(
            Stoffexportziel(rumpf, 'warp_only').dateiname(),
            Stoffexportziel(rumpf, 'blender_eevee').dateiname(),
        )

    def test_ohne_namen_kommt_szene_zeit_und_zufall(self):
        ziel = Stoffexportziel({'scene_name': 'Mein Rock'}, 'warp_only', jetzt=1000)
        name = ziel.dateiname()
    ...
    def test_boeser_dateiname_wird_abgelehnt(self):
        ...
        for boese in ('..\\..\\x.mp4', 'C:\\x.mp4', 'video:1.mp4', '-i.mp4', 'NUL.mp4'):
...
class AdresseTest(SimpleTestCase):
    @override_settings(MEDIA_ROOT='A:/3DTools/HumanBodyWeb/media', MEDIA_URL='/media/')
    def test_datei_unter_media_bekommt_eine_url(self):
        self.assertEqual(Stoffexportziel.adresse('A:/3DTools/HumanBodyWeb/media/x/y.mp4'), '/media/x/y.mp4')
