# -*- coding: utf-8 -*-
"""Der Aufbau des Wrapperbaums bleibt, wie er nach dem Umbau ist.
...
class EineQuelle(unittest.TestCase):
    """Was zweimal dastand, hat zwei Korrekturen nur halb bekommen."""
    ...
    def test_die_hoehe_kommt_aus_dem_netz(self):
        ...
        aus_gelenken = modell.masse_aus_gelenken(modell.gelenke([0.0] * 10)).hoehe
...
class DasFortschrittsprotokoll(unittest.TestCase):
    ...
    def test_jede_protokollzeile_wird_geleert(self):
        """Ohne `flush` haelt Windows die Ausgabe im Puffer zurueck —
    ...
    def test_kein_logger_statt_protokoll(self):
        """Ein Logger schreibt in eine Datei, nicht auf stdout."""
        for pfad, text in Wrapperquellen.texte():
            for vorsatz in self.VORSAETZE:
                with self.subTest(datei=pfad.name, vorsatz=vorsatz):
                    self.assertNotRegex(text, r"logger\.\w+\(\s*'%s" % vorsatz)


class DieDateigroesse(unittest.TestCase):
    """Keine Monolithen — die Regel gilt auch hier."""
...
class JederRunnerLiefertSeineFelder(unittest.TestCase):
    """Was ein Runner mitschickt, liest die Django-Seite auch — und umgekehrt.
    ...
    def test_jedes_kamerafeld_kommt_von_einem_runner(self):
        """Ein Feld, das Django liest und niemand schickt, ist immer leer."""
