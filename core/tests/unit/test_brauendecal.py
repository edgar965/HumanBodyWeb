# -*- coding: utf-8 -*-
u"""Die gezeichnete Augenbraue: Retusche, Bogen, Zeichner, Endpunkte.
...
class BrauendecalTest(SimpleTestCase):
    ...
    def setUp(self):
        ...
        self.ordnerpatch = mock.patch.object(Brauendecal, 'ORDNER',
                                             Brauenbogen.ORDNER.__class__(self.ordner))
    ...
    def test_regler_geklemmt_und_kennung_folgt(self):
        r = Brauendecal.regler({'dichte': '9', 'lage': -99, 'farbe': '#ABCDEF',
                                'quatsch': 1})
        self.assertEqual((r['dichte'], r['lage'], r['farbe']), (3.0, -20.0, '#abcdef'))
        self.assertEqual(Brauendecal.regler({'farbe': 'rot'})['farbe'], '#3a2a1e')
        self.assertNotEqual(Brauendecal.kennung('female', r),
                            Brauendecal.kennung('female', Brauendecal.regler({})))
        self.assertNotEqual(Brauendecal.kennung('female', r),
                            Brauendecal.kennung('male', r))
    ...
    def test_beide_brauen_im_fenster_dichte_farbe_hoehe(self):
        ...
        self.assertLess(np.nonzero(hoch)[0].mean(), np.nonzero(alpha)[0].mean() - 6,
                        '10 mm höher = kleinere Bildzeile')
