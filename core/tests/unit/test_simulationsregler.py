...
class DieAngabenSindVollstaendigUndWiderspruchsfrei(TestCase):
    def test_jedes_feld_liegt_in_genau_einer_gruppe(self):
        ...
        gruppiert = [f['schluessel'] for g in Simulationsfelder.nach_gruppen() for f in g['felder']]
        self.assertEqual(alle, set(gruppiert))
        self.assertEqual(len(gruppiert), len(set(gruppiert)), 'Ein Feld steht in zwei Gruppen')
    ...
    def test_jede_vorgabe_liegt_in_ihren_eigenen_grenzen(self):
        ...
        for feld in Simulationsfelder.alle():
            if feld['schalter']:
                continue
            self.assertGreaterEqual(feld['vorgabe'], feld['min'], feld['schluessel'])
            self.assertLessEqual(feld['vorgabe'], feld['max'], feld['schluessel'])

    def test_jedes_feld_hat_einen_hinweis(self):
        """Ein Regler ohne Hover-Text ist ein Regler, den niemand benutzt."""
        for feld in Simulationsfelder.alle():
            self.assertTrue((feld['hinweis'] or '').strip(), 'ohne Hinweis: %s' % feld['schluessel'])
    ...
    def test_zahlenfelder_haben_grenzen_und_schritt(self):
        for feld in Simulationsfelder.alle():
            ...
            for name in ('min', 'max', 'schritt'):
                self.assertIsNotNone(feld[name], '%s ohne %s' % (feld['schluessel'], name))
    ...
    def test_schalter_haben_wahrheitswerte_als_vorgabe(self):
        for feld in Simulationsfelder.alle():
            if feld['schalter']:
                self.assertIsInstance(feld['vorgabe'], bool, feld['schluessel'])
...
class NurAbweichendesWirdWeitergereicht(TestCase):
    ...
    def test_ein_wert_auf_der_vorgabe_wird_nicht_uebernommen(self):
        ...
        regler = Simulationsregler.aus_anfrage({'sim_garment_edge_ke': str(vorgabe)})
        self.assertTrue(regler.leer)

    def test_ein_abweichender_wert_kommt_an(self):
        regler = Simulationsregler.aus_anfrage({'sim_garment_edge_ke': '50'})
        self.assertEqual(regler.fuer('material'), {'garment_edge_ke': 50.0})

    def test_leerer_text_gilt_als_keine_angabe(self):
        self.assertTrue(Simulationsregler.aus_anfrage({'sim_garment_edge_ke': ''}).leer)
...
class JederWertLandetImRichtigenAbschnitt(TestCase):
    ...
    def test_material_bekommt_nur_material(self):
        self.assertEqual(self.regler.fuer('material'), {'garment_edge_ke': 50.0})

    def test_optionen_bekommt_nur_optionen(self):
        self.assertEqual(self.regler.fuer('optionen'), {'body_collision_thickness': 0.30})
    ...
    def test_korrekturnamen_heissen_wie_in_der_stoffkorrektur(self):
        ...
        regler = Simulationsregler.aus_anfrage(
            {'sim_glaettschritte': '5', 'sim_hub_max_mm': '20', 'sim_durchgaenge': '4', 'sim_umkreis_cm': '8'}
        )
...
class WerteAusDemNetzWerdenGeprueft(TestCase):
    def test_zu_gross_wird_geklemmt(self):
        regler = Simulationsregler.aus_anfrage({'sim_body_collision_thickness': '99'})
        self.assertEqual(regler.fuer('optionen')['body_collision_thickness'], 1.0)
    ...
    def test_unlesbarer_wert_wird_uebergangen(self):
        ...
        self.assertTrue(Simulationsregler.aus_anfrage({'sim_garment_edge_ke': 'weich'}).leer)

    def test_nan_wird_uebergangen(self):
        self.assertTrue(Simulationsregler.aus_anfrage({'sim_garment_edge_ke': 'nan'}).leer)

    def test_unbekannter_schluessel_wird_ignoriert(self):
        self.assertTrue(Simulationsregler.aus_anfrage({'sim_gibt_es_nicht': '5'}).leer)
    ...
    def test_schrittzahlen_kommen_als_ganze_zahl_an(self):
        ...
        werte = Simulationsregler.aus_anfrage(
            {'sim_max_sim_steps': '8000', 'sim_attachment_frames': '200', 'sim_zero_gravity_steps': '20'}
        )
        self.assertIsInstance(werte.fuer('konfig')['max_sim_steps'], int)
        for name in ('attachment_frames', 'zero_gravity_steps'):
            wert = werte.fuer('optionen').get(name, werte.fuer('konfig').get(name))
    ...
    def test_kommazahlen_bleiben_kommazahlen(self):
        ...
        wert = Simulationsregler.aus_anfrage({'sim_static_threshold': '0.003'}).fuer('konfig')[
            'static_threshold'
        ]
    ...
    def test_die_ankersteifigkeit_wird_zur_liste(self):
        ...
        regler = Simulationsregler.aus_anfrage({'sim_attachment_stiffness': '2000'})
        self.assertEqual(regler.fuer('optionen')['attachment_stiffness'], [2000.0] * 4)
...
class DieKetteReichtBisInDieSimulation(TestCase):
    ...
    def test_die_drapierung_schreibt_konfig_in_den_auftrag(self):
        ...
        quelle = self._ohne_kommentare(self._quelle('Assets', 'GarmentCode', 'drapierung.py'))
        self.assertIn("'konfig': self.konfig", quelle)

    def test_der_lauf_wendet_konfig_an(self):
        quelle = self._ohne_kommentare(self._quelle('Assets', 'GarmentCode', 'drapierlauf.py'))
