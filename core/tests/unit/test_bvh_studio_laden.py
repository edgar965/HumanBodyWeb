...
class LadenTest(SimpleTestCase):
    ...
    def test_lichtkegel_der_szenenlichter_wird_gelesen(self):
        daten = _lies('projekt_daten.js')
        self.assertIn('coneVisible: t.coneVisible !== false', daten)
        lichter = _lies('szenenlichter.js')
    ...
    def test_bodenwerte_gelten_auch_beim_laden_zur_laufzeit(self):
        ...
        laden = _lies('projekt_wiederherstellung.js')
...
class LinealTest(SimpleTestCase):
    ...
    def test_lineal_haengt_am_sichtbaren_rand_und_liegt_ueber_den_reihen(self):
        zeichnen = _lies('zeitleiste_zeichnen.js')
        ...
        self.assertIn('static get oben()', _lies('zeitleiste_flaeche.js'))
        self.assertIn('ctx.translate(0, oben)', _lies('zeitleiste_lineal.js'))
        self.assertIn("addEventListener('scroll', () => renderTimeline())", _lies('timeline.js'))

    def test_treffer_im_lineal_rechnen_mit_dem_geblaetterten_anteil(self):
        ziehen = _lies('zeitleiste_ziehen.js')
        self.assertIn('my - Zeitleistenflaeche.oben <= RULER_HEIGHT', ziehen)
        self.assertNotIn('if (my <= RULER_HEIGHT)', ziehen)
        self.assertIn('my - Zeitleistenflaeche.oben <= RULER_HEIGHT', _lies('zeitleiste_menue.js'))
        self.assertIn('position:sticky;top:0', _lies('zeitleiste_kopfspalte.js'))
