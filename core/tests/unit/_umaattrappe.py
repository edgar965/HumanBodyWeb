# -*- coding: utf-8 -*-
"""Attrappen für die UMA-Prüfungen: eine Mixamo-BVH, ein +Y-Skelett, eine GLB.
...
class Umaattrappe:
    """Baukasten: BVH-Text, +Y-Skelett und GLB-Bytes für die UMA-Prüfungen."""
    ...
    def bvh_text(cls, bilder, bildzeit=1.0 / 30):
        """Eine Mixamo-BVH. `bilder`: je Bild ``{Gelenk: (Z, X, Y) in Grad}``;
        ...
        zeilen += ["MOTION", "Frames: %d" % len(bilder), "Frame Time: %.6f" % bildzeit]
        for bild in bilder:
            werte = []
    ...
    @classmethod
    def _gelenk_schreiben(cls, name, tiefe, wurzel, kinder, versatz, zeilen, reihenfolge):
        ein = "  " * tiefe
        zeilen.append("%s%s %s" % (ein, "ROOT" if wurzel else "JOINT", name))
        zeilen.append(ein + "{")
        zeilen.append("%s  OFFSET %s" % (ein, cls._zahlen(versatz[name])))
        zeilen.append(
            "%s  CHANNELS %s" % (ein, ("6 " + cls.KANAELE_WURZEL) if wurzel else ("3 " + cls.KANAELE))
        )
        reihenfolge.append(name)
        for kind in kinder.get(name, []):
            cls._gelenk_schreiben(kind, tiefe + 1, False, kinder, versatz, zeilen, reihenfolge)
    ...
    def knochen(name, elter, pos, quat=(0.0, 0.0, 0.0, 1.0)):
        return {"name": name, "parent": elter, "local_position": list(pos), "local_quaternion": list(quat)}

    @classmethod
    def _seite(cls, seite, vz):
        """Arm und Bein einer Seite; `vz` = +1 links, -1 rechts."""
    ...
    @classmethod
    def uma_knochen(cls):
        """Ein UMA-artiges Skelett in Three.js-Form, Knochen entlang +Y,
    ...
    def gedreht(cls, knochen=None, quat=YAW_180):
        """Dieselbe Liste, die Wurzel mit `quat` — wie die echte UMA-GLB."""
        raus = []
    ...
    def glb_bytes(cls, gltf, binaer=b""):
        """Eine GLB aus JSON und Binaerteil — beide auf 4 Byte aufgefuellt."""
    ...
    @staticmethod
    def _welt(knochen):
        """Weltlage (pos, rot, skal) je Knochen — Eltern zuerst in der Liste."""
        welt = {}
        for k in knochen:
            ...
            if k["parent"]:
                ...
                welt[k["name"]] = (ep + Quat.rotate(er, es * t), Quat.norm(Quat.mul(er, q)), es * s)
            else:
                welt[k["name"]] = (t, q, s)
        return welt
    ...
    def glb_beispiel(cls):
        """``(gltf, binaer)``: das +Y-Skelett unter `Avatar` (halbe Drehung)
        ...
        gltf = {
            ...
            "accessors": [
                {"bufferView": 0, "componentType": 5126, "count": len(joint_liste), "type": "MAT4"}
            ],
            "bufferViews": [{"buffer": 0, "byteOffset": 0, "byteLength": len(binaer)}],
    ...
    def _knotenliste(cls, joints):
        """glTF-Knoten: `Avatar`, `Root`, dann die Joints mit Kinderlisten."""
        knoten = [{"name": "Avatar", "rotation": list(cls.YAW_180)}, {"name": "Root", "children": [2]}]
        nummer = {k["name"]: i + 2 for i, k in enumerate(joints)}
        for k in joints:
            eintrag = {
                "name": k["name"],
                "translation": k["local_position"],
        ...
        for k in joints:
            if k["parent"]:
                knoten[nummer[k["parent"]]].setdefault("children", []).append(nummer[k["name"]])
        return knoten

    @classmethod
    def _bindmatrizen(cls, joints, joint_liste, knoten):
        """Inverse Bindmatrizen (spaltenweise float32) — Knotenpose samt
        Avatar-Drehung, ab den Hueften um BIND_VERSATZ verschoben."""
        kette = [
            cls.knochen("Avatar", None, (0, 0, 0), cls.YAW_180),
            cls.knochen("Root", "Avatar", (0, 0, 0)),
        ]
        welt = cls._welt(kette + [dict(k, parent=k["parent"] or "Root") for k in joints])
