...
from core.dienste.kleiderverwaltung import PAPIERKORB, KleiderFehler, Kleiderverwaltung
...
class KleiderverwaltungTest(SimpleTestCase):
    ...
    def test_umbenennen_verschiebt_den_ordner_mitsamt_meta(self):
        ...
        antwort = Kleiderverwaltung.ausfuehren({"action": "rename", "id": "Tops/shirt", "new_name": "hemd"})
    ...
    def test_kopieren_laesst_das_original_stehen(self):
        self._anlegen("Tops", "shirt")
        Kleiderverwaltung.ausfuehren({"action": "copy", "id": "Tops/shirt", "new_name": "shirt_kopie"})
        self.assertTrue((self.wurzel / "Tops" / "shirt" / "meta.json").is_file())
        self.assertTrue((self.wurzel / "Tops" / "shirt_kopie" / "meta.json").is_file())
    ...
    def test_loeschen_geht_in_den_papierkorb(self):
        ...
        """
        self._anlegen("Tops", "shirt")
        antwort = Kleiderverwaltung.ausfuehren({"action": "delete", "id": "Tops/shirt"})
        self.assertFalse((self.wurzel / "Tops" / "shirt").exists())
        ziel = self.wurzel / PAPIERKORB / "Tops__shirt"
        self.assertTrue((ziel / "meta.json").is_file(), antwort)
        self.assertTrue(PAPIERKORB.startswith("."), "Der Papierkorb muss vom Scan übersprungen werden")
    ...
    def test_fehlendes_feld(self):
        self._anlegen("Tops", "shirt")
        with self.assertRaises(KleiderFehler) as fall:
            Kleiderverwaltung.ausfuehren({"action": "rename", "id": "Tops/shirt"})
        self.assertEqual(fall.exception.kennzahl, 400)

    def test_unbekanntes_kleid_ist_404(self):
        with self.assertRaises(KleiderFehler) as fall:
            Kleiderverwaltung.ausfuehren({"action": "delete", "id": "Tops/gibtesnicht"})
    ...
    def test_vorhandenes_ziel_ist_409(self):
        ...
        with self.assertRaises(KleiderFehler) as fall:
            Kleiderverwaltung.ausfuehren({"action": "rename", "id": "Tops/shirt", "new_name": "hemd"})
    ...
    def test_ausbruch_ueber_punktpunkt(self):
        ...
        fremd.mkdir()
        for daten in (
            {"action": "rename", "id": "Tops/shirt", "new_name": "../../fremd/geklaut"},
            {"action": "move", "id": "Tops/shirt", "target_category": "../fremd"},
            {"action": "copy", "id": "Tops/shirt", "new_name": "../../fremd/kopie"},
    ...
    def test_kennung_ohne_kategorie_wird_abgelehnt(self):
        ...
        for kennung in ("shirt", "a/b/c", "", "Tops/"):
            with self.assertRaises(KleiderFehler, msg=kennung):
                Kleiderverwaltung.ausfuehren({"action": "delete", "id": kennung})
    ...
    def test_punkt_am_anfang_wird_abgelehnt(self):
        ...
        with self.assertRaises(KleiderFehler):
            Kleiderverwaltung.ausfuehren({"action": "rename", "id": "Tops/shirt", "new_name": ".versteckt"})
