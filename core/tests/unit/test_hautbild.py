# -*- coding: utf-8 -*-
"""Hautbild — die Hauttextur des Films mit gezeichneter Braue.
...
class HautbildTest(SimpleTestCase):
    ...
    def kunstbogen():
        ...
        return {
            ...
            'fenster': [0.375, 0.125, 0.625, 0.25],
            'mm_je_uv': 800.0,
            'form': [2048, 2048],
    ...
    def test_reglertabelle_wie_im_browser(self):
        ...
        js = {
            feld: (name, int(faktor))
            for feld, name, faktor in re.findall(r"(brauen\w*): \['(\w+)', (\d+)\]", block)
        }
        self.assertEqual(js, Hautbild.BRAUENFELDER)
        regler = Hautbild.regler(
            {'brauen': '#102030', 'brauen_lage': 0.0125, 'brauen_staerke': 0.64, 'haut': '#ffffff'}
        )
        self.assertEqual((regler['farbe'], regler['lage'], regler['haar_laenge']), ('#102030', 12.5, 0.64))
    ...
    def test_hautfarbe_der_koerperart_und_eigene(self):
        ...
        self.assertEqual(Hautbild.hautfarbe({'haut': '#102030'}, 'Female_Asian'), (16, 32, 48))
        self.assertEqual(Hautbild.hautfarbe({}, 'Quatsch'), Hautbild.hautfarbe({}, 'Female_Caucasian'))
    ...
    def test_braue_liegt_im_fenster(self):
        ...
        ort = mock.patch.object(Brauendecal, 'ORDNER', Brauenbogen.ORDNER.__class__(ordner))
        for patch in (laden, breite, ort):
            patch.start()
            self.addCleanup(patch.stop)
        grund = np.full((1024, 1024, 3), 200, dtype=np.uint8)
        bild = Hautbild.mit_brauen(grund, 'female', Brauendecal.regler({'farbe': '#000000'}))
    ...
    def test_details_gehen_von_der_szene_bis_zum_render(self):
        ...
        dienst = (settings.BASE_DIR / 'core' / 'dienste' / 'figurvideo.py').read_text(encoding='utf-8')
        ...
        self.assertIn('Filmhaut.anlegen(self.teile[0], details, geschlecht, body_type)', film)
