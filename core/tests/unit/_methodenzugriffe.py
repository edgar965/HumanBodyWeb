# -*- coding: utf-8 -*-
"""Methodenzugriffe — `Klasse.methode.attribut` finden, das es nicht gibt.
...
class Methodenzugriffe:
    """Sammelt Klassenmethoden und prueft Zugriffe darauf."""
    ...
    def methodennamen(gelesen):
        """`{Klassenname: {Methodenname, …}}` ueber alle Dateien."""
        gefunden = {}
    ...
    def auf_methoden(cls, baum, methoden):
        """[(Zeile, Ausdruck)] — jedes `Klasse.methode.attribut`."""
        gefunden = []
        for knoten in ast.walk(baum):
            if not (
