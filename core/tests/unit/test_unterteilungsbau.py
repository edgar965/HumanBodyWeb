# -*- coding: utf-8 -*-
u"""Unterteilungsbau: gebaut, abgelegt, geladen — und der Unterteiler
...
def _wuerfel():
    punkte = np.array([[x, y, z] for x in (0, 1) for y in (0, 1) for z in (0, 1)],
                      dtype=np.float64)
...
class BauUndAblage(TestCase):
    ...
    def test_geladen_rechnet_wie_gebaut(self):
        ...
        gebaut = CatmullClarkSubdivider(quads, face_materials=mat, levels=2,
                                        uv_loops=uv_loops)
        ...
        np.testing.assert_array_equal(geladen.subdivide(punkte),
                                      gebaut.subdivide(punkte))
        np.testing.assert_array_equal(geladen.triangles, gebaut.triangles)
        np.testing.assert_array_equal(geladen.triangle_materials,
                                      gebaut.triangle_materials)
    ...
    def test_zwei_stufen_vervierfachen_die_flaechen(self):
        punkte, quads, uv_loops, mat = _wuerfel()
        cc = CatmullClarkSubdivider(quads, face_materials=mat, levels=2,
                                    uv_loops=uv_loops)
        self.assertEqual(len(cc.triangles), 6 * 16 * 2)
        self.assertGreater(cc.naht_kopien, 0)
        self.assertEqual(cc.sub_vertex_count,
                         cc.geo_vertex_count + len(cc.kopien_eltern))
