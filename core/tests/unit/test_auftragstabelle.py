# -*- coding: utf-8 -*-
"""Auftragstabelle: die Struktur für `djangobase/_tabelle.html`.
...
class Tabellenauftrag:
    """Was `Auftragstabelle` von einem BVHJob liest."""

    def __init__(
        self, name="Tanz.mp4", status="complete", pipeline="gem", progress=0, detail="", groesse=4990294
    ):
        self.id = uuid.uuid4()
...
class DieZeile(SimpleTestCase):
    ...
    def test_kennung_und_drahtformat(self):
        ...
        html = zeile["html"]
        self.assertIn(
            '<td class="kaestchen"><input type="checkbox" class="job-check" value="%s">' % jid, html
        )
    ...
    def test_rohwerte_zum_sortieren(self):
        ...
        self.assertIn('<td data-sort="2026-09-12 17:26:46">12.09.2026 17:26</td>', html)
        self.assertIn('data-sort="Fertig"', html)

    def test_endzustaende_deutsch(self):
        for status, text in (("complete", "Fertig"), ("failed", "Fehlgeschlagen"), ("pending", "Wartet")):
    ...
    def test_laufend_zeigt_den_fortschritt_und_sortiert_danach(self):
        _, zeile = self.zeile(
            status="processing", progress=47, detail="SMPLest-X-Reihe: 500 / 1004 Bilder (469 erkannt)"
        )
        html = zeile["html"]
    ...
    def test_ein_dateiname_mit_spitzen_klammern_wird_maskiert(self):
        """Der Name kommt aus dem Upload (Befund Nemotron, 16.08.2026)."""
