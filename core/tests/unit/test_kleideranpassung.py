# -*- coding: utf-8 -*-
"""Ein angepasstes Kleidungsstück muss in der Liste landen, die gespeichert wird.
...
class AusgangspunkteTest(SimpleTestCase):
    ...
    def test_netzgeometrie_ist_importiert(self):
        ...
        self.assertIn("import { Netzgeometrie } from '../gemeinsam/netzgeometrie.js';", _anpassung())
...
class FehlerWirdProtokolliertTest(SimpleTestCase):
    """Ein Fehler, der nur auf der Konsole steht, ist keiner, den jemand
    ...
    def test_protokoll_ist_importiert(self):
        self.assertIn("import { Protokoll } from '../gemeinsam/protokoll.js';", _anpassung())


class ZweiterFehlerDerselbenArtTest(SimpleTestCase):
    """In `Charakterzubehoer.kleidung` stand dieselbe Sorte Fehler.
...
class KleiderLaufenNebeneinanderTest(SimpleTestCase):
    """Die Anpassungen liegen auf dem kritischen Pfad des Szenenaufbaus.
...
class ListeWirdGepflegtTest(SimpleTestCase):
    """Die Kette, an deren Ende das Speichern hängt."""
