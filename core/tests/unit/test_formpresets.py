# -*- coding: utf-8 -*-
"""Rock und Schuh als je EIN Katalogstueck, die Arten als Formen.
...
ROECKE = {
    'bleistiftrock': ('PencilSkirt', 'FittedWB'),
...
class FormpresetsTest(SimpleTestCase):
    ...
    def test_der_katalog_fuehrt_einen_rock_und_die_alten_namen_als_aliase(self):
        ...
        self.assertEqual(
            Katalog.aufloesen('kreisrock'), ('rock', {'meta.bottom': 'SkirtCircle', 'meta.wb': 'StraightWB'})
        )
    ...
    def test_die_formen_stehen_vor_der_passform_und_eine_ist_gehakt(self):
        ...
        self.assertEqual(
            ...
            ['Bleistiftrock', 'Kreisrock', 'Stufenrock', 'Godetrock', 'Faltenrock', 'Asymmetrischer Rock'],
        )
        self.assertEqual([p['gehakt'] for p in formen], [True, False, False, False, False, False])
    ...
    def test_der_rock_zeigt_die_gruppen_aller_formen(self):
        ...
        for gruppe in ('waistband', 'pencil-skirt', 'flare-skirt', 'godet-skirt', 'skirt', 'levels-skirt'):
    ...
    def test_oberteil_kleid_anzug_und_unterwaesche_sind_je_ein_stueck(self):
        """Edgar, 11.09.2026: „packe auch noch Kleid mit Sommerkleid,
        ...
        namen = [e['name'] for e in Katalog.liste()]
        self.assertEqual(
            namen, ['oberteil', 'hose', 'shorts', 'rock', 'kleid', 'anzug', 'unterwaesche', 'schuh']
        )
        erwartet = {
            'oberteil': ['T-Shirt', 'T-Shirt (anliegend)', 'Hemd (tailliert)', 'Trägertop'],
            'kleid': ['Kleid', 'Sommerkleid', 'Abendkleid'],
            'anzug': ['Jumpsuit'],
            'unterwaesche': ['BH', 'Höschen', 'Body'],
        }
    ...
    def test_die_aliase_bauen_denselben_entwurf_wie_vorher(self):
        ...
        self.assertEqual(
            (jumpsuit['meta']['wb']['v'], jumpsuit['meta']['bottom']['v'], jumpsuit['pants']['length']['v']),
            ('FittedWB', 'Pants', 0.85),
        )
    ...
    def test_die_vorbilder_folgen_dem_zweck_nicht_der_deutung(self):
        ...
        self.assertEqual(
            Vorbildgruppen.ziel('t-shirt', 'Sport-Bra01'), ('unterwaesche', 'form_bh', ('shirt.length',))
        )
        self.assertEqual(
            Vorbildgruppen.ziel('bleistiftrock', 'Female Panties 01'), ('unterwaesche', 'form_hoeschen', ())
        )
        self.assertEqual(Vorbildgruppen.ziel('kleid', 'F Bikini 01')[:2], ('unterwaesche', 'form_body'))
        for titel in ('Rei Ayanami', 'Cyborg Suit', 'Yoko Tsuno'):
            self.assertEqual(
                Vorbildgruppen.ziel('bleistiftrock', titel)[:2], ('anzug', 'form_jumpsuit'), titel
            )
        self.assertEqual(Vorbildgruppen.ziel('kleid', 'Fem Suit')[:2], ('anzug', 'form_jumpsuit'))
        self.assertEqual(Vorbildgruppen.ziel('t-shirt', 'Polo T-Shirt'), ('oberteil', 'form_t_shirt', ()))
        self.assertEqual(Vorbildgruppen.ziel('kleid', 'Dress Shift'), ('kleid', 'form_kleid', ()))
        self.assertEqual(Vorbildgruppen.ziel('hose', 'Stockings')[0], 'hose')

    def test_der_bh_bekommt_die_laenge_der_form(self):
        """Nicht die der Deutung (1,0) — die kennt nur die Oberkante."""
        from GarmentCode.vorbildgruppen import Vorbildgruppen
    ...
    def test_jedes_gemessene_vorbild_erscheint_genau_einmal(self):
        ...
        gezaehlt = sum(len([p for p in Vorbildpresets.fuer(e['name']) if p['bild']]) for e in Katalog.liste())
