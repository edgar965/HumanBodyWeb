# -*- coding: utf-8 -*-
u"""Genesis 9, die Nachzuegler vom 18.09.2026 („noch was offen? … fixe alle
...
def _glossy_doc():
    u"""Amelias `MU 01 Lips-Glossy`: ZWEI leere Grundebenen, dann die
    Lippenmaske mit Deckkraft 0,35; Top Coat an, Rauheit 0."""
    return G9dson('g.duf', {
        'image_library': [{'id': 'TC 1', 'map': [
            {'label': 'Blank', 'color': [0, 0, 0], 'transparency': 1},
            {'label': 'Background', 'color': [0, 0, 0], 'transparency': 1},
            {'url': '/Runtime/Textures/x/Lips_MW.png', 'label': 'Lips',
             'transparency': 0.35, 'operation': 'blend_source_over'}]},
            {'id': 'M 7', 'map': [
                {'label': 'Background', 'color': [0, 0, 0], 'transparency': 1},
                {'url': '/Runtime/Textures/x/E_M.png', 'transparency': 1}]},
            {'id': 'R 2', 'map': [
                {'label': 'Background', 'color': [0, 0, 0], 'transparency': 1},
                {'url': '/Runtime/Textures/x/E_R.png', 'transparency': 1,
                 'invert': True}]}],
        'scene': {'animations': [
...
class Genesis9Glanz(SimpleTestCase):
    ...
    def test_1_glanzbild_r_klarlack_g_rauheit_b_metall(self):
        ...
        aus = G9glanz.rechnen([{'klarlack': 'k', 'klarlackdeckkraft': 0.35,
                                'klarlackrauheit': 0.2},
                               {'metall': 'm', 'metalldeckkraft': 1.0}],
                              (4, 4), lambda p: bilder[p])
    ...
    def test_2_lips_glossy_wird_eine_glanzebene(self):
        ...
        self.assertEqual(ebenen, [{'gruppe': 'Head', 'ueber_schwarz': True,
                                   ...
                                   'klarlackrauheit': 0.0}])
        self.assertEqual(G9schminke.kategorie_von('Amelia 9 MU 01 Lips-Glossy'),
                         'lipgloss')
    ...
    def test_3_lie_ueberspringt_grundebenen_und_meldet_invert(self):
        ...
        self.assertEqual([(e['url'], e['deckkraft'], e['invers']) for e in lagen],
    ...
    def test_4_komponieren_nur_glanz_als_png(self):
        ...
        oeffnen = mock.patch.object(G9ebenen, '_oeffnen',
                                    side_effect=lambda p: bilder[p])
        with tempfile.TemporaryDirectory() as ordner, oeffnen, \
                mock.patch.object(G9pfade, 'ablage', return_value=_Pfad(ordner)):
            aus = G9ebenen.komponieren([{'gruppe': 'Head', 'klarlack': 'k',
                                         'klarlackdeckkraft': 1.0,
                                         'klarlackrauheit': 0.0}])
    ...
    def test_5_directx_normalen_kehren_die_y_achse(self):
        with mock.patch.object(G9browserbilder, 'tragbar', return_value=True):
            aus = G9browserbilder.fuer({'albedo': 'x/Dagger_MAT_D.png',
                                        'normalen': 'x/Dagger_MAT_Normal_DirectX.png'},
                                       hoch=False)
            self.assertEqual(aus['normalenachse'], -1)
            aus = G9browserbilder.fuer({'normalen': 'x/G9_Head_NM_1001.jpg'},
                                       hoch=False)
    ...
    def test_6_anhangmorphe_kennen_ihre_hd_morphs(self):
        with tempfile.TemporaryDirectory() as ordner:
            ...
            os.makedirs(morphs)
            G9dsonSchreiber.schreiben(os.path.join(morphs, 'HD Wrinkles.dsf'), {
                'modifier_library': [{'id': 'HD Wrinkles', 'name': 'HD Wrinkles',
                                      'channel': {'label': 'HD Wrinkles',
                                                  'visible': False},
                                      'morph': {
            ...
            with mock.patch.object(G9pfade, 'bibliothek',
                                   return_value=_Pfad(ordner)):
    ...
    def test_7_blendmodi_nach_w3c(self):
        u"""Je Kanal `Co = Cb·(1−a) + B(Cb, Cs)·a`; Plus = min(1, Cb + Cs·a).
        Untergrund (0,2, 0,6, 0,8), Ebene 0,6 — von Hand gerechnet, ±1."""
        grund = Image.new('RGB', (2, 2), (51, 153, 204))
        ebene = _bild((153, 153, 153), 255, (2, 2))
        faelle = {'blend_multiply': (31, 92, 122), 'blend_screen': (173, 214, 235),
                  'blend_overlay': (61, 173, 214), 'blend_color_burn': (0, 85, 170),
        ...
        self._nah(G9blendmodi.legen(grund, _bild((128, 128, 128), 255, (2, 2)),
                                    'blend_soft_light'), (51, 153, 204))
        # Alpha × Deckkraft: Multiply zur Haelfte -> Mitte von Untergrund und Produkt.
        self._nah(G9blendmodi.legen(grund, ebene, 'blend_multiply', 0.5),
                  (41, 122, 163))
    ...
    def test_8_makeupstapel_grundierung_ist_der_untergrund_des_rouges(self):
        u"""Grundierung (Plus ueber Schwarz = das Bild, Maske 0,4) und darueber
        ein Rouge mit `soft_light` (50 % Grau: Farbe bleibt die Grundierung)
        und Maske 0,45: Gewicht 0,4 + 0,45·0,6 = 0,67 -> 171."""
        bilder = {'F': _bild((200, 150, 120), 255), 'WF': _bild((255, 255, 255), 255),
                  'R': _bild((128, 128, 128), 255), 'WR': _bild((255, 255, 255), 255)}
        farbe, gewicht = G9makeupstapel.rechnen(
            [{'farbe': 'F', 'farbmodus': 'blend_plus', 'maske': 'WF', 'deckkraft': 0.4},
             {'farbe': 'R', 'farbmodus': 'blend_soft_light', 'maske': 'WR',
              'deckkraft': 0.45}],
            (4, 4), lambda p: bilder[p])
        ...
        farbe, _gewicht = G9makeupstapel.rechnen(
            [{'farbe': 'F', 'farbmodus': 'blend_multiply', 'maske': 'WF',
              'deckkraft': 1.0}], (4, 4), lambda p: bilder[p])
        self.assertEqual(farbe.getpixel((0, 0)), (0, 0, 0))

    def test_9_glitzer_normalen_und_klarlackwerte(self):
        doc = _glossy_doc()
        doc.daten['image_library'].append({'id': 'NM 3', 'map': [
            {'label': 'Background', 'color': [0, 0, 0], 'transparency': 1},
            {'url': '/Runtime/Textures/x/E_NM.png', 'transparency': 1,
             'operation': 'blend_overlay'}]})
        doc.daten['scene']['animations'] += [
            {'url': KANAL + 'Normal%20Map/image', 'keys': [[0, '#NM%203']]},
            {'url': KANAL + 'Top%20Coat%20Color/value',
             'keys': [[0, [0.95, 0.77, 0.67]]]},
        ...
        self.assertEqual(G9glanz.werte([ebene]),
                         {'klarlackfarbe': [0.95, 0.77, 0.67], 'klarlackbump': 0.15,
                          'normalenmodus': 'overlay'})
        bilder = {'n': _bild((100, 140, 250), 200)}
        aus = G9glanz.normalen([{'normalen': 'n', 'normalendeckkraft': 0.5}], (4, 4),
                               lambda p: bilder[p])
...
class _Pfad(str):
    u"""Ein `Path`-Ersatz, der `/` kann und `is_file` versteht."""
