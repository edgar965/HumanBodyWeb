# -*- coding: utf-8 -*-
"""Gespeicherte Szenen behalten ihr drapiertes Netz.
...
class AblageAufProbe(SimpleTestCase):
    """Legt `Entwurf.AUSGABE` fuer die Dauer des Tests um."""

    def setUp(self):
        wegwerf = os.path.join(
            os.path.dirname(os.path.abspath(__file__)), '..', '..', '..', '_wegwerf', 'szenenprobe'
        )
        self.wurzel = tempfile.mkdtemp(dir=os.path.abspath(os.path.dirname(wegwerf)), prefix='szenenprobe_')
    ...
    def gelesen(self, rig_url):
        """Was unter dieser Adresse wirklich liegt."""
...
class DasNetzUeberlebtDenNaechstenBau(AblageAufProbe):
    def setUp(self):
        super().setUp()
        self.netz_anlegen('t-shirt_female', 't-shirt_female_sim_rig.json', {'punkte': 'LANG'})
        self.szene = [
            {
                'stueck': 't-shirt',
                'rig_url': '/api/garmentcode/datei/t-shirt_female/t-shirt_female_sim_rig.json/',
    ...
    def test_und_dahinter_liegt_dasselbe_netz(self):
        gesichert = Szenenstuecke.sichern('FemaleGarmentCode', self.szene)
        self.assertEqual(self.gelesen(gesichert[0]['rig_url']), {'punkte': 'LANG'})

    def test_der_naechste_bau_aendert_die_szene_nicht_mehr(self):
        """Der eigentliche Test — genau das ist am 09.09.2026 passiert."""
        gesichert = Szenenstuecke.sichern('FemaleGarmentCode', self.szene)
        # Ein neuer Bau ueberschreibt den Arbeitsordner.
        self.netz_anlegen('t-shirt_female', 't-shirt_female_sim_rig.json', {'punkte': 'KURZ'})
        self.assertEqual(self.gelesen(gesichert[0]['rig_url']), {'punkte': 'LANG'})

    def test_gegenprobe_ohne_sicherung_geht_es_verloren(self):
        """Sonst pruefte der Test oben nur, dass Kopieren funktioniert."""
        self.netz_anlegen('t-shirt_female', 't-shirt_female_sim_rig.json', {'punkte': 'KURZ'})
        self.assertEqual(self.gelesen(self.szene[0]['rig_url']), {'punkte': 'KURZ'})
...
class WasNichtGehtBlockiertDasSpeichernNicht(AblageAufProbe):
    """Eine Szene, die wegen eines fehlenden Netzes gar nicht gespeichert
    wird, waere schlimmer als eine, der ein Kleidungsstueck fehlt."""

    def test_fehlendes_netz_laesst_den_zeiger_stehen(self):
        szene = [{'stueck': 'hose', 'rig_url': '/api/garmentcode/datei/gibt_es_nicht/x_rig.json/'}]
...
class NiemandKommtAusDerWurzelHeraus(AblageAufProbe):
    """Die Adresse kommt aus dem Browser — dieselbe Vorsicht wie in der
    Ausliefer-Route."""

    def test_punkt_punkt_wird_abgewiesen(self):
        self.assertIsNone(Szenenstuecke._quellpfad('/api/garmentcode/datei/../../geheim/x_rig.json/'))

    def test_zu_wenige_teile(self):
        self.assertIsNone(Szenenstuecke._quellpfad('/api/garmentcode/datei/nurordner/'))

    def test_eine_gueltige_adresse_loest_auf(self):
        pfad = Sicher.wert(
            Szenenstuecke._quellpfad('/api/garmentcode/datei/t-shirt_female/x_rig.json/'), 'Quellpfad'
        )
...
class DerOrdnernameVertraegtAllesAusDemNamensfeld(AblageAufProbe):
    def test_leerzeichen_und_umlaute_werden_ersetzt(self):
        name = Szenenstuecke.ordnername('Anna Müller 2')
