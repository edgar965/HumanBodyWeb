# -*- coding: utf-8 -*-
"""`Knochenwelt` — die EINE Vorwärtskinematik des Rigs.
...
KETTE = {
    'wurzel': {'local_position': [0.0, 0.0, 0.8], 'local_quaternion': EINHEIT, 'parent': None},
    'arm': {'local_position': [0.0, 1.0, 0.0], 'local_quaternion': EINHEIT, 'parent': 'wurzel'},
    'hand': {'local_position': [0.0, 1.0, 0.0], 'local_quaternion': EINHEIT, 'parent': 'arm'},
    'waise': {'local_position': [5.0, 0.0, 0.0], 'local_quaternion': EINHEIT, 'parent': 'gibtesnicht'},
}
...
class DieVorwaertskinematik(SimpleTestCase):
    ...
    def test_eine_drehung_des_arms_schwenkt_die_hand_um_den_arm(self):
        """Arm 90° um Z: die Hand liegt 1 m in −X neben dem Armansatz."""
    ...
    def test_ein_unbekannter_elternteil_macht_den_knochen_zur_wurzel(self):
        welt = self.welt(lambda name: np.array(EINHEIT), namen=('waise',), ort=[0.0, 0.0, 1.0])
