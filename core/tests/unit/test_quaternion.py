# -*- coding: utf-8 -*-
"""`humanbody_core.quaternion.Quat` gegen SciPy prüfen.
...
class Drehungsbau:
    """Baut die Zufallsdrehungen fuer diese Faelle.
...
class QuatGegenScipyTest(SimpleTestCase):
    """Jede Rechnung gegen eine fremde Umsetzung derselben Mathematik."""

    def test_mul_entspricht_scipy_verkettung(self):
        """`Quat.mul(a, b)` dreht wie erst b, dann a."""
        ...
        for a, b in zip(eine, andere):
            ...
            self.assertAlmostEqual(abs(float(np.dot(eigen, fremd))), 1.0, places=10)

    def test_rotate_dreht_wie_scipy(self):
        """Der gedrehte Vektor ist das, was zählt — dort gibt es kein Vorzeichen."""
        punkte = Drehungsbau.zufallsdrehungen(8, 3)[:, :3]
        for q, v in zip(Drehungsbau.zufallsdrehungen(8, 11), punkte):
            np.testing.assert_allclose(Quat.rotate(q, v), Rotation.from_quat(q).apply(v), atol=1e-12)

    def test_mul_reihe_ist_mul_in_einem_zug(self):
        """Die vektorisierte Form muss dasselbe rechnen wie die einzelne.
    ...
    def test_mul_reihe_nimmt_auch_links_eine_reihe(self):
        """Beide Seiten Reihen — der Fall aus `retarget/motor.py`.
        ...
        for i in range(11):
            np.testing.assert_allclose(zusammen[i], Quat.mul(links[i], rechts[i]), atol=1e-12)
    ...
    def test_inv_macht_die_drehung_rueckgaengig(self):
        for q in Drehungsbau.zufallsdrehungen(8, 5):
            np.testing.assert_allclose(Quat.mul(q, Quat.inv(q)), Quat.ID, atol=1e-12)

    def test_norm_liefert_einheitslaenge(self):
        for q in Drehungsbau.zufallsdrehungen(6, 13) * 7.5:
            self.assertAlmostEqual(float(np.linalg.norm(Quat.norm(q))), 1.0, places=12)

    def test_norm_faengt_den_nullvektor_ab(self):
        """Ohne die Abfrage käme hier eine Division durch fast Null."""
        np.testing.assert_allclose(Quat.norm(np.zeros(4)), Quat.ID, atol=0)
    ...
    def test_from_unit_vectors_trifft_das_ziel(self):
        ...
        for a, b in zip(richtungen, np.roll(richtungen, 1, axis=0)):
            np.testing.assert_allclose(Quat.rotate(Quat.from_unit_vectors(a, b), a), b, atol=1e-10)

    def test_from_unit_vectors_gegenrichtung(self):
        """Antiparallel hat keine eindeutige Antwort — aber eine gültige."""
    ...
    def test_from_unit_vectors_gleichrichtung_ist_identitaet(self):
        ...
        np.testing.assert_allclose(Quat.from_unit_vectors(a, a), Quat.ID, atol=0)

    def test_slerp_endpunkte_und_mitte(self):
        a, b = Drehungsbau.zufallsdrehungen(2, 23)
        np.testing.assert_allclose(
            Quat.rotate(Quat.slerp(a, b, 0.0), [1, 0, 0]), Quat.rotate(a, [1, 0, 0]), atol=1e-10
        )
        np.testing.assert_allclose(
            Quat.rotate(Quat.slerp(a, b, 1.0), [1, 0, 0]), Quat.rotate(b, [1, 0, 0]), atol=1e-10
        )
        # Die Mitte liegt von beiden Enden gleich weit weg.
        mitte = Quat.slerp(a, b, 0.5)
        self.assertAlmostEqual(abs(float(np.dot(mitte, a))), abs(float(np.dot(mitte, b))), places=10)


class QuatIdTest(SimpleTestCase):
    """`Quat.ID` ist EIN geteiltes Array — wer es verändert, verändert es überall."""
    ...
    def test_norm_gibt_eine_kopie_zurueck(self):
        """Sonst schriebe ein Aufrufer versehentlich in die geteilte Konstante."""
...
class QuatEinzigeFassungTest(SimpleTestCase):
    """Diese Rechnung darf nicht wieder in zwei Dateien wandern.
