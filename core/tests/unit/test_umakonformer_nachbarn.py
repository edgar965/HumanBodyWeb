# -*- coding: utf-8 -*-
"""Was bei UMA `ClothingConformerSpatialIndex` leistet: das naechste Dreieck.
...
class Nachbarsuche(unittest.TestCase):
    """Was bei UMA `ClothingConformerSpatialIndex` leistet."""
    ...
    def test_das_naechste_dreieck_wird_gefunden(self):
        """Ein Punkt dicht über der Haut muss auf ein Dreieck fallen, das
        ihn wirklich trägt: Der Fusspunkt liegt radial unter ihm."""
        probe = np.array([[0.25, 0.5, 0.0], [0.0, 0.5, 0.25], [-0.25, 0.5, 0.0]])
    ...
    def _koerper_mit_freiem_punkt(self):
        """Zylinder plus EIN Körperpunkt, der zu keinem Dreieck gehört.
    ...
    def test_rueckfall_greift_wenn_kein_dreieck_trifft(self):
        """UMAs `FindNearestVertices`: Wo kein Dreieck im Höchstabstand
    ...
    def test_gewichte_fallen_mit_dem_abstand(self):
        """`CalculateInverseDistanceWeights`: Der nächste Punkt bekommt
        ...
        self.assertEqual(list(np.argsort(abstaende)), list(np.argsort(-gewichte)))
