# -*- coding: utf-8 -*-
u"""Koerperfeinheit: Corrective Smooth → Unterteilung → Hautverschiebung im Film.
...
class KoerperfeinheitTest(SimpleTestCase):
    ...
    def test_kopfmaske_nimmt_kopf_und_gesicht_nicht_den_unterarm(self):
        namen = ['DEF-forearm.L', 'DEF-spine.006', 'DEF-nose', 'MCH-eye.L',
                 'DEF-spine.005', 'ORG-teeth.T', 'DEF-ear.L.002']
    ...
    def test_ohne_textur_und_korrektur_ist_es_die_unterteilung(self):
        ...
        np.testing.assert_allclose(f.punkte(self.punkte, self.dreiecke),
                                   self.cc.subdivide(self.punkte), atol=1e-12)
    ...
    def test_textur_mitte_schiebt_nichts_textur_eins_schiebt_fuenf_millimeter(self):
        ...
        np.testing.assert_allclose(f.punkte(self.punkte, self.dreiecke),
                                   self.cc.subdivide(self.punkte), atol=1e-6)
    ...
    def test_korrektur_laesst_die_ruhelage_in_ruhe(self):
        ...
        gewichte[:, 0] = 1.0
        f = self.kf.Koerperfeinheit(self.cc, self.quads).anlegen(
            self.punkte, gewichte, namen)
        self.assertIsNotNone(f.korrektur)
        np.testing.assert_allclose(f.punkte(self.punkte, self.dreiecke),
                                   self.cc.subdivide(self.punkte), atol=1e-9)
    ...
    def test_feinkoerper_rechnet_ueber_die_feinheit_und_haelt_wenige_bilder(self):
        ...
        teil = {'haut': Haut(), 'dreiecke': self.dreiecke, 'feinheit': f,
                'unterteiler': self.cc, 'fein_dreiecke': f.dreiecke}
        ...
        np.testing.assert_allclose(F.bild(teil, 5),
                                   f.punkte(Haut.folge[5], self.dreiecke))
