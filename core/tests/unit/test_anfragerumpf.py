# -*- coding: utf-8 -*-
"""`Anfragerumpf`: der JSON-Rumpf einer Anfrage — und was bei Unsinn passiert.
...
class LesenTest(SimpleTestCase):
    ...
    def test_eigene_meldung_bleibt_erhalten(self):
        """Vier Endpunkte antworten „Invalid JSON body\" — das ist ihr
...
class NameUndDatenTest(SimpleTestCase):
    def test_beide_da(self):
        name, daten, fehler = Anfragerumpf.name_und_daten(_Anfrage(b'{"name": " Hose ", "data": {"x": 1}}'))
        self.assertIsNone(fehler)
        self.assertEqual(name, "Hose")
        self.assertEqual(daten, {"x": 1})

    def test_name_nur_aus_leerzeichen_zaehlt_nicht(self):
        _, _, fehler = Anfragerumpf.name_und_daten(_Anfrage(b'{"name": "   ", "data": {"x": 1}}'))
        self.assertEqual(Sicher.wert(fehler, "Fehler").status_code, 400)
        self.assertEqual(_Anfrage.roh(fehler), {"error": "name and data required"})

    def test_name_null_wirft_nicht(self):
        """`rumpf.get('name', '')` liefert bei `"name": null` ein `None` —
        und `None.strip()` ist ein 500. Deshalb `or ''`."""
        _, _, fehler = Anfragerumpf.name_und_daten(_Anfrage(b'{"name": null, "data": {"x": 1}}'))
        self.assertEqual(Sicher.wert(fehler, "Fehler").status_code, 400)
