# -*- coding: utf-8 -*-
"""Der Absatz als Teil der Pose — gerechnet vom Server, nicht vom Betrachter.
...
class AbsatzposeTest(SimpleTestCase):
    ...
    @staticmethod
    def _winkel_um_x(q):
        """Drehwinkel (Grad, mit Vorzeichen) einer [w,x,y,z] um die x-Achse."""
        w, x, y, z = q
    ...
    def test_aus_anfrage_ohne_absatz_ist_nichts(self):
        ...
        pose = Absatzpose.aus_anfrage({"winkel_grad": "7", "hebung_cm": "4.9", "plateau_cm": "1"})
    ...
    def test_der_endpunkt_liefert_die_ruhelage_mit_und_ohne_absatz(self):
        ...
        mit = self.client.get(
            "/api/character/pose/ruhelage/?winkel_grad=25&sprengung_grad=8&hebung_cm=4.9"
        ).json()
    ...
    @staticmethod
    def _skelett(ordner):
        """Ein Kunstskelett: Bein senkrecht, Fuss um 90° nach vorn gekippt,
        ...
        knochen = [
            {
                "name": "DEF-spine",
                "parent": None,
                "local_position": [0, 0, 0.8],
                "local_quaternion": um_x(90.0),
            },
            {
                "name": "DEF-shin.L",
                "parent": "DEF-spine",
                "local_position": [0, 0.4, 0],
