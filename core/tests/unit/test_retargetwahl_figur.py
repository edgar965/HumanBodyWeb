# -*- coding: utf-8 -*-
"""`Retargetwahl.figur` und die Ablage je Figur.
...
class RetargetwahlFigurTest(SimpleTestCase):
    ...
    def test_uma_verlangt_eine_glb(self):
        """Seit dem 07.09.2026 ist `figur` nicht mehr nur ein Dateiname.
        ...
        self.assertEqual(Retargetwahl({'target': 'smpl', 'figur': 'mean_all'}, 1.68).figur, 'mean_all')
        self.assertEqual(Retargetwahl({'target': 'makehuman', 'figur': 'basis'}, 1.68).figur, 'basis')
    ...
    def test_makehuman_bringt_seine_regler_mit(self):
        """Ohne sie stuende das Ziel in der Vorgabefigur, nicht in der Szene."""
        wahl = Retargetwahl(
            {
                'target': 'makehuman',
                'figur': 'basis',
                'makro': {'gender': 0.9},
                'regler': {'head/head-age-decr|incr': 0.4},
            },
            1.68,
        )
    ...
    def test_ablage_ist_je_figur_eine_eigene(self):
        ...
        self.assertEqual(
            Retargetdaten('x/clip.bvh', 1.56).ablage, Retargetdaten('x/clip.bvh', 1.56, figur=None).ablage
        )
