# -*- coding: utf-8 -*-
"""Zwei Schnitte zu einem Netz vereinen — und zurueck zerlegen.
...
from GarmentCode.schnittvereinigung import Schnittvereinigung, VereinigungsFehler
...
class SchnittvereinigungTest(SimpleTestCase):
    ...
    def _paar(self):
        hose = self._ablegen(
            'hose',
            SchnittvereinigungTest._schnitt(
                ['wb_front', 'wb_back'], [[{'panel': 'wb_front', 'edge': 6}, {'panel': 'wb_back', 'edge': 0}]]
            ),
        )
        shirt = self._ablegen(
            't-shirt',
            SchnittvereinigungTest._schnitt(
                ...
                [[{'panel': 'left_ftorso', 'edge': 1}, {'panel': 'left_btorso', 'edge': 2}]],
            ),
        )
        return hose, shirt
    ...
    def _vereint(self):
        ...
        with open(ziel, encoding='utf-8') as quelle:
            return bericht, json.load(quelle)

    # -------------------------------------------------------------- vereinen

    def test_alle_panels_und_naehte_kommen_mit(self):
        """Kein Panel darf verlorengehen — es wuerde nie vernetzt.
        ...
        self.assertEqual(
            sorted(panels),
            ['hose__wb_back', 'hose__wb_front', 't-shirt__left_btorso', 't-shirt__left_ftorso'],
        )
        self.assertEqual(sorted(vereint['pattern']['panel_order']), sorted(panels))
        self.assertEqual(len(vereint['pattern']['stitches']), 2)

    def test_naehte_zeigen_auf_die_umbenannten_panels(self):
        """Eine Naht verweist ueber den NAMEN — der muss mitwandern."""
        _, vereint = self._vereint()
    ...
    def test_keine_naht_ueber_die_stueckgrenze(self):
        """Genau das unterscheidet zwei Stuecke von einem Jumpsuit.
        ...
        _, vereint = self._vereint()
        for naht in vereint['pattern']['stitches']:
            stuecke = {Schnittvereinigung.stueck_von(s['panel']) for s in naht}
            self.assertEqual(len(stuecke), 1, naht)

    def test_parameters_stehen_drin_auch_wenn_sie_leer_sind(self):
        """`VisPattern` erbt von `ParametrizedPattern`.
    ...
    def test_gleiches_stueck_zweimal_bekommt_eigene_praefixe(self):
        """Zwei Roecke uebereinander sind erlaubt.
        ...
        self.assertEqual([s['name'] for s in bericht['stuecke']], ['hose', 'hose_2'])
        self.assertEqual(len(bericht['panels']), 4)

    # ------------------------------------------------------------- Abbrueche

    def test_verschiedene_einheiten_brechen_ab(self):
        """`units_in_meter` deutet JEDE Koordinate.
        ...
        with self.assertRaises(VereinigungsFehler) as fall:
            Schnittvereinigung([hose, shirt]).vereinen(os.path.join(self.ordner, 'k_specification.json'))
    ...
    def test_ein_einzelnes_stueck_wird_abgelehnt(self):
        ...
        with self.assertRaises(VereinigungsFehler):
            Schnittvereinigung([hose]).vereinen(os.path.join(self.ordner, 'e_specification.json'))

    def test_altes_parameterformat_wird_abgelehnt(self):
        """Die `influence`-Listen nennen PANELNAMEN.
        ...
        with open(shirt['spezifikation'], 'w', encoding='utf-8') as datei:
            json.dump(spez, datei)
        with self.assertRaises(VereinigungsFehler) as fall:
            Schnittvereinigung([hose, shirt]).vereinen(os.path.join(self.ordner, 'p_specification.json'))
        self.assertIn('Parameterformat', str(fall.exception))

    def test_fehlende_datei_wird_gemeldet(self):
        hose, _ = self._paar()
        with self.assertRaises(VereinigungsFehler):
            Schnittvereinigung([hose, {'name': 'x', 'spezifikation': 'gibtsnicht.json'}]).vereinen(
                os.path.join(self.ordner, 'f_specification.json')
            )

    # --------------------------------------------------------------- teilen

    def _netz_und_segmentierung(self):
        """Ein Kunstnetz: zwei Dreiecke Hose, zwei Dreiecke T-Shirt.
    ...
    def test_nahtpunkte_finden_ihr_stueck_ueber_die_naht(self):
        """In der Segmentierung steht bei ihnen KEIN Panelname.
    ...
    def test_die_indizes_bilden_auf_das_ganze_netz_ab(self):
        """Die Verankerung rechnet auf den KORRIGIERTEN Punkten.
    ...
    def test_flaeche_ueber_die_grenze_wird_gezaehlt_nicht_verschluckt(self):
        """Sie KANN nicht auftreten — und wenn doch, wird sie gemeldet."""
        ziel, netz, seg = self._netz_und_segmentierung()
    ...
    def test_geschriebenes_teilnetz_ist_wieder_lesbar(self):
        """Die Indizes im OBJ sind 1-basiert — ein Versatz von eins gibt
        ein Netz, das laedt und falsche Dreiecke hat."""
        ziel, netz, seg = self._netz_und_segmentierung()
        teile = Stoffteilung(ziel).teilen(netz, seg)
        pfad = os.path.join(self.ordner, 'hose_sim.obj')
        Stoffteilung.netz_schreiben(pfad, teile['hose']['punkte'], teile['hose']['dreiecke'])
