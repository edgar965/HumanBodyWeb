# -*- coding: utf-8 -*-
"""Blenders Module nachbilden, so weit ein Import sie braucht.
...
class Platzhalter:
    """Nimmt jeden Zugriff hin und gibt wieder einen Platzhalter.
...
class Blenderattrappe:
    """Setzt Blenders Module in ``sys.modules`` und raeumt wieder auf."""
    ...
    @staticmethod
    def _addon_entladen():
        """Das Addon aus ``sys.modules`` nehmen — sonst haelt es Leichen.
    ...
    def module(self):
        """``{Modulname: Modul}`` — alles, was das Addon einfuehrt."""
        bpy = self.bpy()
