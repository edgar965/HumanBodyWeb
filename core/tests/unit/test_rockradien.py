...
class RockradienTest(SimpleTestCase):
    ...
    def test_toleranz_von_fuenf_millimetern(self):
        ...
        self.assertEqual(self.pruefen(Probeform.zylinder(0.10), Probeform.zylinder(0.102)).verletzt, 0)
        # 8 mm zu eng liegt über der Toleranz.
        self.assertGreater(self.pruefen(Probeform.zylinder(0.10), Probeform.zylinder(0.092)).verletzt, 0)
    ...
    def test_mitte_wandert_mit_dem_stoff(self):
        ...
        """
        pruefung = self.pruefen(
            Probeform.zylinder(0.10, mitte=(0.5, 0.0)), Probeform.zylinder(0.15, mitte=(0.5, 0.0))
        )
        self.assertEqual(pruefung.verletzt, 0)

    def test_flacher_stoff_wird_uebersprungen(self):
        """Ein Fetzen (unter 5 cm hoch) ist Sache eines anderen Tests."""
        pruefung = self.pruefen(Probeform.zylinder(0.10), Probeform.zylinder(0.05, von=0.0, bis=0.02))
