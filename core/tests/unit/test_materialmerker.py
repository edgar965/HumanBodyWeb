# -*- coding: utf-8 -*-
"""Die Farbe eines Garment-Fit-Stücks muss dort ankommen, wo gespeichert wird.
...
def _linear(kanal):
    """sRGB-Kanal 0..1 in linearen Raum — die Umrechnung, die THREE.Color
...
class DerZustandBekommtDieFarbeTest(SimpleTestCase):
    ...
    def test_der_reiter_verdrahtet_den_merker(self):
        ...
        self.assertIn("import { Materialmerker } from './materialmerker.js';", quelle)

    def test_kein_material_ohne_gedaechtnis_mehr(self):
        """Die drei alten Hörer schrieben ins Material und sonst nirgends.
...
class OhneAuswahlPassiertNichtsTest(SimpleTestCase):
    """Dieselbe Vorsicht wie bei den GarmentCode-Farben am selben Tag: Ein
...
class DerBefundIstBelegtTest(SimpleTestCase):
    ...
    def test_die_gespeicherte_farbe_war_die_vorgabe(self):
        """#4d5980 in linear — auf drei Stellen genau der gespeicherte Wert.

        Ohne diese Rechnung wäre „die Schuhe trugen die Vorgabefarbe" eine
        Behauptung; mit ihr ist es eine Messung.
        """
        gespeichert = [0.07421356837213867, 0.09989872823822872, 0.21586050010324417]
...
class DieKetteZumSpeichernTest(SimpleTestCase):
    """Das Feld, das der Merker schreibt, ist genau das, was gespeichert wird.
    ...
    def test_der_farbwaehler_zeigt_den_zustand_an(self):
        """Die Gegenrichtung — sonst stünde im Feld etwas anderes als im
        Netz, und genau daran war der Befund nicht zu sehen."""
        self.assertIn("const colorEl = document.getElementById('prop-garment-color');", _prop())


class DieRegionsverschiebungKommtAnTest(SimpleTestCase):
    """Beim Laden reichte der Zustand allein nicht.
    ...
    def test_die_verschiebungen_werden_ueberhaupt_gespeichert(self):
        """Die feste Feldliste in `toJSON` führte die fünf Regionen NICHT.
