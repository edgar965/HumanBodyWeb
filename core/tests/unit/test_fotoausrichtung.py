...
class NetzmasseTest(SimpleTestCase):
    ...
    def test_bildhoehe_von(self):
        ...
        self.assertAlmostEqual(m.bildhoehe_von(m.y_max, verschiebung), 100.0, places=3)
        self.assertAlmostEqual(m.bildhoehe_von(m.y_min, verschiebung), 900.0, places=3)
...
class PymafxTest(SimpleTestCase):
    ...
    def cam(self, **zusatz):
        daten = dict(BILD, pred_cam=[1.0, 0.0, 0.0], bbox_cxcywh=[960, 540, 400, 400], bbox_scale=1.0)
        daten.update(zusatz)
        return daten
    ...
    def test_ohne_bbox_kein_vorschlag(self):
        masse = Netzmasse.aus(NETZ, BILD)
        self.assertIsNone(Fotoausrichtung._aus_pymafx(self.cam(bbox_cxcywh=None), masse))
...
class SmplestxTest(SimpleTestCase):
    ...
    def cam(self, **zusatz):
        daten = dict(
            BILD,
            cam_trans=[0.0, 0.0, 2.0],
            cam_focal=[1000.0, 1000.0],
            cam_princpt=[128.0, 128.0],
            processed_bbox=[100, 50, 256, 256],
    ...
    def test_kamera_auf_der_bildebene_ergibt_keinen_vorschlag(self):
        ...
        masse = Netzmasse.aus(NETZ, BILD)
        self.assertIsNone(Fotoausrichtung._aus_smplestx(self.cam(cam_trans=[0, 0, 0]), masse))
...
class EinpassenTest(SimpleTestCase):
    def test_netz_fuellt_95_prozent_der_personenhoehe(self):
        ...
        aus = Fotoausrichtung._einpassen({"method": "pymafx"}, masse, grenzen)
