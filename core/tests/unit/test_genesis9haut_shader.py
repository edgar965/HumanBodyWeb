# -*- coding: utf-8 -*-
u"""Der Durchlicht-Shader (`gemeinsam/genesis9haut.js`) übersteht Threes
...
class DurchlichtShader(SimpleTestCase):
    ...
    def rumpf(self):
        text = self.glsl()
        treffer = re.search(r'#pragma unroll_loop_start(.*?)#pragma unroll_loop_end',
                            text, re.S)
    ...
    def test_1_keine_deklaration_im_schleifenrumpf(self):
        ...
        self.assertIsNone(re.search(r'\b(vec[234]|float|int)\s+\w+\s*=', rumpf),
    ...
    def test_3_einhaengepunkte_und_uniforms(self):
        text = self.quelltext()
        for stueck in ('#include <lights_fragment_end>', 'uDurchlicht',
                       'uDurchlichtFarbe',
                       'attribute float dicke', 'vDuenne', 'customProgramCacheKey'):
            self.assertIn(stueck, text)

    def test_4_schminke_mischt_hinter_map_und_roughnessmap(self):
        u"""Schminke (18.09.2026): `mix(diffuseColor, farbe, gewicht)` hinter
        ...
        text = self.quelltext()
        for stueck in ('uSchminkeFarbe', 'uSchminkeGewicht', 'uSchminkeRauheit',
                       'uSchminkeAn',
        ...
        glsl = self.glsl()
        self.assertIn('diffuseColor.rgb = mix( diffuseColor.rgb, schminkeFarbe.rgb, '
                      'g9Schminke )', Genesis9hautQuelle.glsl(glsl, 'SCHMINKE'))
        ...
        self.assertIn('roughnessFactor *= mix( 1.0, schminkeRauheit.r, g9Schminke )',
                      rauheit)
        self.assertIn('mix( roughnessFactor, schminkeRauheit.g, '
                      'schminkeRauheit.b * uSchminkeAn )', rauheit)

    def test_5_detailnormalen_als_rnm_an_stelle_von_normal_fragment_maps(self):
        u"""8K-Detailnormalen (18.09.2026): Reoriented Normal Mapping wie MDLs
        ...
        text = self.quelltext()
        self.assertIn("'#include <normal_fragment_maps>',\n"
                      "                    Genesis9hautGLSL.normalen(", text)
        glsl = self.glsl()
        detail = Genesis9hautQuelle.glsl(glsl, 'DETAIL')
        for stueck in ('n1.z += 1.0;', 'n2.xy = -n2.xy;',
                       'vec3 gemischt = n1 * dot( n1, n2 ) / n1.z - n2;',
                       'normal = normalize( tbn * gemischt );',
                       'mix( vec3( 0.0, 0.0, 1.0 ), detailN, '
                       'uDetailGewicht * uDetailAn )'):
            self.assertIn(stueck, detail, stueck)
        rahmen = glsl[glsl.index('static normalen('):]
        for stueck in ('#if defined( USE_NORMALMAP_TANGENTSPACE )',
                       'vec3 mapN = basisN * 2.0 - 1.0;', 'mapN.xy *= normalScale;',
    ...
    def test_6_glitzer_normalen_als_w3c_overlay_ueber_der_hautnormale(self):
        u"""Snow Queens Glitzer (18.09.2026 abends): Daz' LIE legt die Ebene mit
        ...
        for stueck in ('texture2D( uSchminkeNormalen, vNormalMapUv )',
                       'mix( 2.0 * basisN * glitzer.xyz,',
                       '1.0 - 2.0 * ( 1.0 - basisN ) * ( 1.0 - glitzer.xyz ),',
                       'step( 0.5, basisN ) );',
                       'mix( glitzer.xyz, ueberlagert, uSchminkeNormalenModus )',
                       'basisN = mix( basisN, ueberlagert, '
                       'glitzer.a * uSchminkeNormalenAn )'):
            self.assertIn(stueck, glitzer, stueck)
        text = self.quelltext()
        self.assertIn("uSchminkeNormalenModus.value = "
                      "werte.normalenmodus === 'overlay' ? 1 : 0", text)

    def test_7_klarlackfarbe_und_bump(self):
        u"""Irays Top Coat Color toent die Klarlackschicht (`pbr_skin.mdl`:
        ...
        self.assertIn("KLARLACK_NEU = '%s * material.clearcoat * uKlarlackFarbe;'"
                      % summe, glsl)
        text = self.quelltext()
        for stueck in ('.replace(Genesis9hautGLSL.KLARLACK_ALT, '
                       'Genesis9hautGLSL.KLARLACK_NEU)',
                       'uniform vec3 uKlarlackFarbe;',
                       'material.clearcoatNormalMap = material.normalMap;',
                       'material.clearcoatNormalScale.set(w, '
                       'w * Math.sign(material.normalScale.y || 1))',
                       "zusatz.klarlack ? 'k' : 'x'"):
            self.assertIn(stueck, text, stueck)


class Genesis9hautQuelle:
    u"""Ein GLSL-Block (`static NAME = ...`) aus dem Modultext."""
