# -*- coding: utf-8 -*-
"""Die eigene SMPL-X-Pipeline, zweiter Teil (12.09.2026): Quellen, Handgelenk,
...
class Kameraattrappe:
    """`grab`/`read` zaehlen; `set` ist verboten — genau das war die Bremse."""
...
class SomaNachSmplx(unittest.TestCase):
    ...
    def _params(self, drehung):
        """`drehung`: Name -> Achse-Winkel, alles andere null."""
    ...
    def test_grundgelenk_verkettet_mittelhand_und_grundgelenk(self):
        soma = Somahaende(
            self._params(
                {'LeftHandIndex1': [0, 0, 0.2], 'LeftHandIndex2': [0, 0, 0.3], 'LeftHandIndex3': [0, 0, 0.5]}
            ),
            np,
        )
    ...
    def test_daumen_zaehlt_ab_dem_sattelgelenk(self):
        soma = Somahaende(self._params({'RightHandThumb1': [0.4, 0, 0], 'RightHandThumb3': [0, 0.1, 0]}), np)
...
class DasHandgelenk(unittest.TestCase):
            ...
            reihe.dazu(
                {
                    ...
                    'smplx_body_pose': koerper.reshape(-1),
    ...
    def test_handgelenk_nimmt_die_weltdrehung_unter_gems_unterarm(self):
        """SMPLest-X: Ellbogen 20 Grad, Handgelenk 30 Grad um x -> Welt 50.
...
class DerBodenkontakt(unittest.TestCase):
    ...
    def _bahn(self, wippe_cm=3.0, drift_cm_je_bild=0.3):
        """Stehende Figur (Pose null), deren Wurzel wippt und wegdriftet."""
    ...
    def test_ein_kurzer_kontakt_verschiebt_nicht_die_ganze_spur(self):
        """Ballett: 4 Standbilder von 298 — unter 5 % bleibt die Spur unangetastet."""
    ...
    def test_ohne_stand_keine_korrektur(self):
        """Ein Sprung ueber alle Bilder: nichts nah am Boden -> nichts geaendert."""
...
class DieQuellen(unittest.TestCase):
    ...
    def test_reihe_laeuft_nur_wenn_etwas_daran_haengt(self):
        ...
        self.assertFalse(
            Smplxlauf('v.mp4', 'z.bvh', haende='gemx', gesicht='keins', handgelenk='koerper').braucht_reihe
        )
        self.assertTrue(
            Smplxlauf('v.mp4', 'z.bvh', haende='gemx', gesicht='keins', handgelenk='hand').braucht_reihe
        )
...
class DieVerteiler(unittest.TestCase):
    ...
    def test_lifterwahl_kennt_die_neuen_schalter(self):
        """`video` heisst im Verteiler `netzvideo`: `Lifterwahl.starten(name, video, ...)`
    ...
    def test_lift_3d_verwechselt_video_nicht_mit_dem_pfad(self):
        """`--video` ist der Eingabepfad; `--no_video` darf nicht auf `video`
    ...
    def test_smplbefehl_reicht_quellen_und_zugaben_durch(self):
        ...
        class Auftrag:
            pipeline = 'smplx'
            pipeline_params = {'body_source': 'gvhmr', 'hands_source': 'gemx', 'video': False}
    ...
    def test_pipelineparameter_liest_die_auswahlen(self):
        ...
        post = QueryDict(
            'smplx_body_source=duomo&smplx_hands_source=gemx'
            '&smplx_face_source=keins&smplx_wrist_source=koerper'
            '&smplx_ground=on'
        )
        p = Pipelineparameter.lesen(post, 'smplx')
        self.assertEqual(
            (p['body_source'], p['hands_source'], p['face_source'], p['wrist_source']),
            ('duomo', 'gemx', 'keins', 'koerper'),
        )
    ...
    def test_karte_und_js_kennen_die_felder(self):
        ...
        felder = (wurzel / 'static' / 'js' / 'auftraege' / 'pipelinefelder.js').read_text(encoding='utf-8')
...
class DieAehnlichkeitInNode(unittest.TestCase):
    """`bvh_player/aehnlichkeit.js` — die Rechnung hinter der Rig-Ueberlagerung,
