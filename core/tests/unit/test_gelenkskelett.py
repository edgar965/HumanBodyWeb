# -*- coding: utf-8 -*-
"""Gelenkskelett — aus Gelenkpunkten ein Skelett, das man BESPIELEN kann.
...
def kette(*glieder):
    """Eine Knochenliste aus `(name, eltern, kopf, schwanz)`."""
    return [{"name": n, "eltern": e, "kopf": list(k), "schwanz": list(s)} for n, e, k, s in glieder]
...
class GelenkskelettTest(unittest.TestCase):
    ...
    def test_die_kette_trifft_die_gelenke_wieder(self):
        """Vorwaerts gerechnet muss jeder Knochen an seinem Punkt landen."""
        skelett = Gelenkskelett(ARM)
        lagen = skelett.weltlagen()
        for k in ARM:
            punkt, _ = lagen[k["name"]]
            self.assertLess(np.linalg.norm(punkt - np.array(k["kopf"])), 1e-5, k["name"])

    def test_die_kette_trifft_die_richtungen_wieder(self):
        """Und in die Richtung zeigen, die `schwanz` vorgibt."""
        skelett = Gelenkskelett(ARM)
        lagen = skelett.weltlagen()
        for k in ARM:
    ...
    def test_ohne_ruhedrehung_zeigt_alles_in_dieselbe_richtung(self):
        """Die Sabotage-Gegenprobe: genau das war der Fehler.
        ...
        self.assertLess(min(paare), 0.99, "alle Knochen zeigen in dieselbe Richtung")

    # ------------------------------------------------------------- Bauplan

    def test_lokale_lage_ist_relativ_zum_elternteil(self):
        """Weltpunkte unveraendert einzusetzen addiert jede Ebene doppelt."""
    ...
    def test_jedes_blatt_bekommt_einen_endknochen(self):
        ...
        self.assertLess(np.linalg.norm(lagen["hand_ende"][0] - np.array(ARM[-1]["schwanz"])), 1e-5)

    def test_endknochen_der_laenge_null_entfaellt(self):
        """Eine Linie ohne Laenge wird gezeichnet und ist doch unsichtbar."""
        plan = Gelenkskelett(kette(("a", None, (0, 1, 0), (0, 1, 0)))).bauplan()
        self.assertEqual([k for k in plan if k["ende"]], [])

    def test_eltern_stehen_vor_ihren_kindern(self):
        """Auch wenn die Quelle sie verkehrt herum liefert."""
    ...
    def test_unbekannter_elternteil_gilt_als_wurzel(self):
        """Verworfen wird nichts — ein Knochen im Nichts faellt eher auf."""
        plan = Gelenkskelett(kette(("a", "gibtsnicht", (0, 1, 0), (0, 1.2, 0)))).bauplan()
        self.assertEqual(plan[0]["eltern"], None)

    def test_knochen_ohne_laenge_erbt_die_lage_des_elternteils(self):
        """Eine Richtung zu erfinden waere schlimmer als sie zu erben."""
        plan = {
            k["name"]: k
            for k in Gelenkskelett(
                kette(("a", None, (0, 1, 0), (0.3, 1, 0)), ("b", "a", (0.3, 1, 0), (0.3, 1, 0)))
            ).bauplan()
        }
        self.assertEqual([round(w, 6) for w in plan["b"]["quat"]], [0.0, 0.0, 0.0, 1.0])

    # ------------------------------------------------------- fuer den Motor

    def test_geometrie_kennt_die_achse_und_alle_knochen(self):
        """Ein Knochen ohne Eintrag bekaeme keine Spur und bliebe stehen."""
        ...
        self.assertTrue(np.allclose(geometrie.richtungsachse, Gelenkskelett.ACHSE))

    def test_geometrie_kennt_die_ruherichtungen(self):
        """Was `Richtungskorrektur._ruherichtung` liest, muss stimmen."""
        geometrie = Gelenkskelett(ARM).geometrie()
        for k in ARM:
            soll = np.array(k["schwanz"]) - np.array(k["kopf"])
            soll = soll / np.linalg.norm(soll)
            ist = Quat.rotate(geometrie.bones[k["name"]].world_rest_quat, geometrie.richtungsachse)
            self.assertLess(np.linalg.norm(ist - soll), 1e-5, k["name"])
