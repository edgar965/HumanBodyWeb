...
class BvhverzeichnisTest(BvhBaum, TestCase):
    def test_kategorienamen_sortiert(self):
        self.assertEqual(self.verzeichnis().kategorienamen(), ['Aist', 'Bandai 1', 'Leer'])
...
class AnimationslisteTest(BvhBaum, TestCase):
    ...
    def test_felder_je_eintrag(self):
        ...
        self.assertEqual(
            eintrag,
            {
                'name': 'gehen',
                'category': 'Bandai 1',
                'url': '/api/character/bvh/Bandai 1/gehen/',
                'frames': 12,
            },
        )
    ...
    def test_bildzahlen_aus_der_datei(self):
        ...
        self.assertEqual({e['name']: e['frames'] for e in eintraege}, {'a b': 5, 'a': 7, 'b': 9})
