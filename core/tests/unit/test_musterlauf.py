# -*- coding: utf-8 -*-
"""`Musterlauf` — und die Naht, die am Schwerpunkt zog.
...
class Musterbau:
    """Baut die Punktwolken und Muster fuer diese Faelle.
    ...
    @staticmethod
    def punktwolke():
        """Eine grobe stehende Röhre — NUR Punkte, keine Flächen.
        ...
        punkte = [[0.16 * np.cos(w), 0.16 * np.sin(w), z] for z in np.linspace(0, 1.7, 10) for w in winkel]
...
class KantenkarteTest(SimpleTestCase):
    """Die Vertexnummern je Kante zeigen auf Randpunkte."""
...
class NahtTest(SimpleTestCase):
    """Vernähte Kanten liegen danach aufeinander."""
...
class LeeresMusterTest(SimpleTestCase):
    """Was nicht reicht, gibt None statt einer Ausnahme."""
