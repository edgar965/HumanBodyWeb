# -*- coding: utf-8 -*-
u"""Genesis 9, 18.09.2026 abends (Edgar: „genesis 8 … kannst du das in Genesis9
...
def _knochen(kennung, eltern, orientation, reihenfolge='XYZ'):
    return {'id': kennung, 'type': 'bone', 'parent': '#%s' % eltern if eltern else None,
...
class Genesis9Autofit(SimpleTestCase):
    ...
    def test_1_schablone_liest_skelett_und_drehung(self):
        roh, drehung = G9autofit.schablone({'nodes': [
            {'id': 'ROOT_SKELETON', 'center_point': [0, 0, 0]},
            {'id': 'hip', 'parent': 'ROOT_SKELETON', 'center_point': [0, 105, 1.8],
             'end_point': [0, 88, 0], 'orientation': [0, 0, 0], 'rotation': [0, 0, 0]},
            {'id': 'lThighBend', 'parent': 'hip', 'center_point': [7.9, 96.4, 0.4],
             'end_point': [11.5, 75.8, -0.1], 'orientation': [1.2, 4.9, 7.9],
             'rotation': [0, 0, -6]}]})
    ...
    def test_3_umrechnen_gleiche_drehung_in_der_welt(self):
        u"""Ein G8-Knochen mit 30° Orientierung um y und ein G9-Knochen ohne:
        ...
        neu = G9knochenmatrizen.euler([aus.get('rotation/%s' % a, 0.0) for a in 'xyz'],
                                      'ZYX')
        ...
        self.assertEqual(G9autofit.umrechnen(g8, g9, [25.9, 0.0, 0.0]),
                         {'rotation/x': 25.9})

    def test_4_figurpose_aus_einer_g8_pose(self):
        pose = G9dson('p.duf', {'scene': {'animations': [
            {'url': 'lFoot:/data/x/Genesis8Female.dsf#lFoot?rotation/x/value',
             'keys': [[0, 25.9]]},
            {'url': 'lFoot:/data/x/Genesis8Female.dsf#lFoot?scale/x/value',
             'keys': [[0, 1]]},
        ...
        with mock.patch.object(G9skelett, 'roh', return_value=roh):
            self.assertEqual(G9autofit.figurpose(pose, rig),
                             {'l_foot': {'rotation/x': 25.9}})
...
class Genesis9Stoff(SimpleTestCase):
    ...
    def _doc(typ, szene=None, gewichte=None):
        ...
        extra.append({'type': 'studio_modifier_channels', 'channels': [
            {'channel': {'id': G9stoff.KANAL, 'type': 'enum', 'value': typ}}]})
        doc = G9dson('n.dsf', {'modifier_library': [
            {'id': 'dForce Simulation', 'extra': extra}]})
        if szene is None:
            return doc, None
        return doc, G9dson('s.duf', {'scene': {'modifiers': [
            {'id': 'dForce Simulation', 'extra': [
                {'type': 'studio_modifier_channels', 'channels': [
                    {'channel': {'id': G9stoff.KANAL, 'current_value': szene}}]}]}]}})
    ...
    def test_5_dynamisch_aus_dsf_und_szene(self):
        ...
        ohne = G9dson('o.dsf', {'modifier_library': [{'id': 'SkinBinding',
                                                      'extra': []}]})
    ...
    def test_7_kanten_und_bauplan(self):
        ...
        folger = mock.Mock(punkte=np.zeros((4, 3)), polys=[[0, 0, 0, 1, 2, 3]],
                           haut=None)
