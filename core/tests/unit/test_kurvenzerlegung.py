# -*- coding: utf-8 -*-
"""`Kurvenzerlegung`: dieselben Punkte, ohne 12.402 Integrale.
...
class Kurve:
    """Eine Attrappe mit `point(t)` — mehr braucht `teilstellen` nicht."""
...
def _zerlegung():
    """Die Klasse aus dem GarmentCode-Paket."""
...
class KurvenzerlegungTest(SimpleTestCase):
    ...
    def test_gerade_trifft_exakt(self):
        """Auf einer Geraden sind Bogenlaenge und Parameter dasselbe."""
        Z = _zerlegung()
    ...
    def test_gleichmaessig_nach_bogenlaenge_nicht_nach_parameter(self):
        """Der ganze Zweck: gleiche ABSTAENDE, nicht gleiche t-Schritte.
    ...
    def test_viertelkreis_gegen_die_analytische_loesung(self):
        ...
        Z = _zerlegung()
        kreis = Kurve(lambda t: complex(math.cos(t * math.pi / 2), math.sin(t * math.pi / 2)))
    ...
    def test_entartete_kante_faellt_nicht_um(self):
        """Anfang = Ende: Gesamtlaenge null, keine Division durch null."""
        Z = _zerlegung()
    ...
    def test_anzahl_und_raender(self):
        """Ohne Anfang und Ende — wie beim Upstream."""
        for n in (1, 4, 9, 20):
            stellen = _zerlegung().teilstellen(Kurve(lambda t: complex(t, 0.0)), n)
    ...
    def test_aufwand_haengt_nicht_an_der_punktzahl(self):
        """EINMAL tabellieren, dann interpolieren — das ist der Gewinn.
