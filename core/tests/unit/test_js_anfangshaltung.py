...
class DieHaltungAusBildNull(SimpleTestCase):
    """Gegeben: Eine Animation mit Dreh- und Ortsspuren."""
...
"""
    ...
    def test_eine_zu_kurze_spur_wird_uebergangen(self):
        """Weniger als vier Werte sind keine Drehung — der Knochen bleibt."""
        self.assertEqual(
            MODUL.laufen(
                SKELETT
                + """
...
"""
    ...
    def test_ein_unbekannter_knochen_stoert_nicht(self):
        """Eine Spur ohne Knochen darf den Lauf nicht abbrechen."""
        self.assertEqual(
            MODUL.laufen(
                SKELETT
                + """
...
class OhneAnimation(SimpleTestCase):
    """Gegeben: Es gibt keinen Klip oder er hat keine Spuren."""
