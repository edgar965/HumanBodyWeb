# -*- coding: utf-8 -*-
"""Die eigene SMPL-X-Pipeline im VideoToBVH-Baum — der Teil ohne Grafikkarte (12.09.2026).
...
class DasKanonischeFormat(unittest.TestCase):
    ...
    def _smpl(self):
        ...
        return {
            'body_pose': rng.normal(0, 0.1, (self.N, 69)).astype(np.float32),
    ...
    def test_aus_smpl_nimmt_21_koerpergelenke_und_laesst_die_handflaechen(self):
        ...
        np.testing.assert_allclose(bahn.body_pose.reshape(self.N, 63), smpl['body_pose'][:, :63])
...
class DieMischung(unittest.TestCase):
            ...
            reihe.dazu(
                {
                    ...
                    'smplx_body_pose': np.zeros(63),
                    'smplx_lhand_pose': np.full(45, 0.5),
                    'smplx_rhand_pose': np.full(45, -0.5),
                    'smplx_jaw_pose': np.array([0.2, 0.0, 0.0]),
                    'smplx_expr': np.arange(10) / 10.0,
    ...
    def test_handmittel_wird_addiert(self):
        """SMPLest-X liefert relativ zur gekruemmten Mittelhand; gegen die
...
class DasSkelettDesBvh(unittest.TestCase):
    ...
    def test_finger_haengen_am_handgelenk(self):
        ...
        for finger in ('index', 'middle', 'pinky', 'ring', 'thumb'):
            ...
            self.assertEqual(eltern[namen.index('left_%s2' % finger)], namen.index('left_%s1' % finger))
...
class DasRetargetFormat(unittest.TestCase):
    ...
    def test_jeder_finger_und_jeder_koerperknochen_ist_zugeordnet(self):
        from humanbody_core.skeleton.formats import SkeletonAIST_SMPL, SkeletonSMPLX
