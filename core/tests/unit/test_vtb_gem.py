# -*- coding: utf-8 -*-
"""GEM-SMPL im VideoToBVH-Baum — der Teil ohne Grafikkarte (11.09.2026).
...
class DerGemBefehl(unittest.TestCase):
    ...
    def test_ergebnis_liegt_unter_dem_videonamen(self):
        self.assertEqual(self._lauf().ergebnisdatei('aus'), os.path.join('aus', 'tanz', 'smpl_params.pt'))


class DieErgebnisumformung(unittest.TestCase):
    """Ohne torch: der Leser wird hereingereicht."""

    def test_body_params_incam_wird_smpl_params_incam(self):
        daten = {
            'body_params_incam': {'transl': [[0, 0, 0]]},
        ...
        self.assertEqual(
            v,
            {
                'smpl_params_incam': daten['body_params_incam'],
...
class DieBvhHilfsmodule(unittest.TestCase):
    def test_bvh_und_quat_liegen_im_wrapperbaum(self):
        ...
        self.assertEqual(os.path.normcase(ordner), os.path.normcase(str(WRAPPERS / 'smpl2bvh')))
