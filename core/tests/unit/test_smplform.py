# -*- coding: utf-8 -*-
"""Smplform — die Umrechnung Regler <-> SMPL-Betas.
...
class SmplformTest(unittest.TestCase):
    ...
    def test_groesser_hat_je_geschlecht_ein_anderes_vorzeichen(self):
        """Der eigentliche Befund: dieselbe Zahl macht den Mann groesser
    ...
    def test_schlank_ist_bei_beiden_dasselbe(self):
        """Fuer die Fuelle gilt bei beiden Geschlechtern dasselbe Vorzeichen
        ...
        for geschlecht in ("male", "female"):
            for groesse, fuelle in ((0, 0), (100, -100), (-35, 60), (12, 7)):
                ...
                self.assertAlmostEqual(
                    zurueck["groesse"], groesse, places=6, msg="%s %s" % (geschlecht, groesse)
                )
                self.assertAlmostEqual(
                    zurueck["fuelle"], fuelle, places=6, msg="%s %s" % (geschlecht, fuelle)
                )
    ...
    def test_unsinn_wird_zur_mitte(self):
        """Ein leeres Feld darf keinen Koerper verformen."""
