# -*- coding: utf-8 -*-
"""Smplkoerper — die Gegenprobe gegen GarmentCodes eigene A40-Koerper.
...
class SmplkoerperTest(Smplmodelle, unittest.TestCase):
    ...
    def _pruefe(self, geschlecht, datei):
        ...
        self.assertLess(float(np.median(abstand)), 5.0, "Median %.1f mm" % float(np.median(abstand)))
    ...
    def test_arme_sind_abgesenkt(self):
        """Die Gegenprobe zur Gegenprobe: Ohne die A-Haltung stehen die Arme
        ...
        self.assertGreater(spanne(rest) - spanne(a40), 0.15, "A-Haltung senkt die Arme nicht")
        self.assertAlmostEqual(hoehe(rest), hoehe(a40), delta=0.02)


class ArmlaengeTest(Smplmodelle, unittest.TestCase):
    """Die Armlaenge muss GEMESSEN sein, nicht aus der YAML kommen.
    ...
    def test_armlaenge_waechst_mit_der_figur(self):
        """Eine groessere Figur hat laengere Arme — sonst waere der Wert
