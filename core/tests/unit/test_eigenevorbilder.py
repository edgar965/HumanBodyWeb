# -*- coding: utf-8 -*-
"""Eigene Vorbilder der Kleiderbibliothek — Leggings (11.09.2026).
...
class EigeneVorbilderTest(SimpleTestCase):
    ...
    def test_bild_liegt_da_und_kommt_ueber_den_endpunkt(self):
        for eintraege in EIGENE.values():
            for eintrag in eintraege:
                ...
                antwort = self.client.get(reverse("garmentcode_vorbildbild", args=[eintrag["bild"]]))
    ...
    def test_fremder_name_gibt_404(self):
        for name in ("nichtda.png", "..", "x"):
            antwort = self.client.get(reverse("garmentcode_vorbildbild", args=[name]))
    ...
    def test_die_passform_kennt_den_bauwert(self):
        """Das Passform-Preset „Leggings" traegt denselben Bauwert."""
        ...
        def sammeln(gruppen):
            for gruppe in gruppen:
                for feld in gruppe.get("felder", ()):
                    ...
                    if (
                        isinstance(bereich, (list, tuple))
                        and bereich
                        and all(isinstance(x, (int, float)) for x in bereich)
                    ):
...
class LeggingsOhneBuendchenTest(SimpleTestCase):
    """Kein Ruesche-Buendchen mehr (Edgar, 11.09.2026: „an den Knoecheln
