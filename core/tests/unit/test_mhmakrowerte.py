# -*- coding: utf-8 -*-
"""Mhmakrowerte — die Umrechnung der neun Makroregler in Zielfaktoren.
...
class MhmakrowerteTest(unittest.TestCase):
    ...
    def test_alter_am_knick_ist_ganz_kind(self):
        """0,1875 = zehn Jahre — der Knickpunkt aus MakeHumans Kommentar."""
    ...
    def test_alter_summiert_sich_immer_auf_eins(self):
        for schritt in range(0, 101):
            ...
            self.assertAlmostEqual(summe, 1.0, places=6, msg='age=%.2f' % (schritt / 100.0))
        ...
        for regler in Mhmakrowerte.DREITEILIG:
            for schritt in (0, 25, 50, 75, 100):
                ...
                self.assertAlmostEqual(
                    f[klein] + f[mitte] + f[gross], 1.0, places=6, msg='%s=%d' % (regler, schritt)
                )

    # ----------------------------------------------------------------- Rasse

    def test_rassen_werden_auf_eins_normiert(self):
        """Das Verhältnis bleibt, die Summe wird 1.
