...
1. `Bildmodelloptionen.proportionen_pruefen`: nur bekannte Maße im Bereich,
   Unbekanntes und Unlesbares fällt weg; `pruefen` trägt sie.
...
class PruefungTest(unittest.TestCase):
    def test_nur_formbare_im_bereich(self):
        ...
        self.assertEqual(Bildmodelloptionen.proportionen_pruefen(roh),
                         {'huefte_breite': 38.3, 'augen_abstand': 7.0})
        ...
        self.assertEqual(len(katalog['proportionen']), 19)
        self.assertTrue(all(m['formbar'] for m in katalog['proportionen']))
...
class EndpunktTest(TestCase):
    ...
    def test_stellen_und_start_behaelt(self):
        antwort = self.client.post(
            ...
            data=json.dumps({'proportionen': {'huefte_breite': 38, 'quatsch': 7}}),
