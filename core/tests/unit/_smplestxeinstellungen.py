# -*- coding: utf-8 -*-
"""Smplestxeinstellungen — der Teil von `cfg`, den `Smplestxbild` liest.
...
class Erkennungsteil:
    """`cfg.inference.detection` — nur das Feld `conf` wird gelesen."""
...
class Datenteil:
    """`cfg.data` — nur `bbox_ratio`."""

    bbox_ratio = 1.5


class Modellteil:
    """`cfg.model` — nur die Eingabeform."""

    input_img_shape = (256, 192)


class Smplestxeinstellungen:
    """Steht fuer `cfg`, soweit `Smplestxbild` es anfasst."""
