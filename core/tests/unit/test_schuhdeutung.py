# -*- coding: utf-8 -*-
"""Bibliotheksschuhe als Vorbilder: Fussmasse, Schuhdeutung, Schuhweg.
...
def _quader(x0, x1, y0, y1, z0, z1, n=9):
    """Die Oberfläche eines Quaders als Punktwolke (Meter)."""
...
class FussmasseTest(SimpleTestCase):
    ...
    def _figur(self):
        """Ein Bein als Säule, darunter ein Fuss als Quader — links (x > 0)."""
...
class SchuhdeutungTest(SimpleTestCase):
    ...
    def setUp(self):
        self.fuss = Fussvorgabe(
            dict(height=168.0, foot_length=24.41, foot_ball_width=9.14, ankle_height=13.78, knee_height=53.1)
        )

    def _schuh(self, laenge, breite, hoehe):
        """Ein Schuh als Quader, Zehen nach -Y (Meter)."""
    ...
    def test_ein_paar_wird_am_fuss_getrennt_auch_wenn_die_schaefte_sich_beruehren(self):
        ...
        links = np.vstack([self._schuh(0.27, 0.10, 0.15), _quader(0.05, 0.27, -0.20, 0.0, 0.15, 0.45, n=13)])
