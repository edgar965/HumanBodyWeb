...
class SmplbefehlTest(SimpleTestCase):
    def befehl(self, pipeline, **params):
        ...
        with override_settings(PIPELINE_PYTHON='py.exe'):
            return Smplbefehl(auftrag, Smplbefehlseinstellungen()).bauen('lift_3d.py', 'tanz.mp4', 'out.bvh')
    ...
    def test_gelenkgrenzen_sind_umgekehrt(self):
        ...
        self.assertNotIn('--no_joint_limits', self.befehl('gvhmr', joint_limits=True))
        self.assertIn('--no_joint_limits', self.befehl('gvhmr', joint_limits=False))
    ...
    def test_jedes_einstellungsfeld_der_schalter_steht_im_modell(self):
        ...
        for pipeline, schalter in Smplbefehl.SCHALTER.items():
            for _schluessel, feld, _argument in schalter:
                if feld:
                    ...
                    self.assertTrue(hasattr(Smplbefehlseinstellungen, feld), 'Attrappe ohne %s' % feld)
        for pipeline in Smplbefehl.MIT_GLAETTUNG:
            ...
            self.assertTrue(
                hasattr(Smplbefehlseinstellungen, name + '_smooth_sigma'),
    ...
    def test_unbekannte_pipeline_bekommt_nur_das_grundgeruest(self):
        ...
        self.assertEqual(
            [a for a in befehl if a.startswith('--')], ['--pipeline', '--video', '--output', '--device']
        )
