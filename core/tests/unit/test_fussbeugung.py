# -*- coding: utf-8 -*-
"""Der Fuss auf dem Absatz: Beugung, Umkehrung, Absatzblock, Endpunkt.
...
class FussbeugungTest(SimpleTestCase):
    ...
    def _huelle(self, n=4000):
        """Zufallspunkte in der Hülle von Fuss und Bein — auch beide Seiten."""
    ...
    def test_das_plateau_zieht_vom_absatz_ab(self):
        ...
        self.assertLess(halb.winkel_grad, Fussbeugung(BALLEN, KNOECHEL, FERSE_Z, 9.0).winkel_grad)
    ...
    def test_die_sprengung_hebt_die_zehenspitze_und_sonst_nichts(self):
        ...
        self.assertAlmostEqual(
            spitze[1] - 2.0, 7.6 * np.sin(np.radians(10.0)), delta=0.15
        )  # 7,6 cm vor dem Ballen
        ...
        self.assertAlmostEqual(beides.winkel_grad, Fussbeugung(BALLEN, KNOECHEL, FERSE_Z, 7.0).winkel_grad)
...
class AbsatzblockTest(SimpleTestCase):
    ...
    @staticmethod
    def _spez():
        """Eine Spezifikation mit einer flachen Hintersohle (Rechteck
...
class GarmentabsatzvorschauTest(SimpleTestCase):
    """Der Regler stellt die Figur sofort — dieselbe Rechnung wie der Bau."""
