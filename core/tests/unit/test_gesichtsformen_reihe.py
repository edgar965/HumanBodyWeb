# -*- coding: utf-8 -*-
"""Euler → Quaternion je Knochen in einem Aufruf — bitgleich zum Einzelaufruf.
...
class DieReihe(SimpleTestCase):
    ...
    def test_der_einzelaufruf_bleibt(self):
        np.testing.assert_allclose(
            Gesichtsformen._euler_to_threejs_quat(0.3, -0.2, 0.5),
            DieReihe.einzeln(0.3, -0.2, 0.5),
            atol=1e-12,
        )
    ...
    def test_die_ausdrucksspuren_sind_die_alten(self):
        ...
        for name, werte in spuren.tracks.items():
            ...
            for bild, ausdruck in enumerate(bilder):
                ...
                np.testing.assert_allclose(
                    werte[bild * 4 : bild * 4 + 4], DieReihe.einzeln(*euler), atol=1e-12
                )
    ...
    def test_die_kieferspuren_sind_die_alten(self):
        ...
        for knochen, anteil in Kieferspuren.ANTEILE:
            for bild, wert in enumerate(soll):
                np.testing.assert_allclose(
                    spuren.tracks[knochen][bild * 4 : bild * 4 + 4],
                    DieReihe.einzeln(wert * anteil, 0.0, 0.0),
                    atol=1e-12,
                )
