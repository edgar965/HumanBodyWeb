# -*- coding: utf-8 -*-
u"""Mimik: MB-Lab-Ausdrücke → DEF-Gesichtsknochen, Nebendatei, Verdrahtung im Studio.
...
class KunstSkelett:
    u"""Kopf `DEF-spine.006` in Ruhe, Kiefer `DEF-jaw` darunter, Kinn am Kiefer."""

    @staticmethod
    def bauen(ordner):
        bones = [
            {'name': 'DEF-spine.006', 'parent': None, 'local_position': [0, 0, 1.5],
             'local_quaternion': [1, 0, 0, 0]},
            {'name': 'DEF-jaw', 'parent': 'DEF-spine.006',
             'local_position': [0, 0, -0.05],
             'local_quaternion': [1, 0, 0, 0]},
            {'name': 'DEF-chin', 'parent': 'DEF-jaw', 'local_position': [0, 0.05, 0],
             'local_quaternion': [1, 0, 0, 0]},
        ]
        # Punkte: Kiefer 0–3 auf der y-Achse vor dem Kieferkopf, Kinn 4–5 weiter vorn.
        netz = np.array([[0, 0.02, 1.45], [0, 0.04, 1.45], [0, 0.06, 1.45],
                         [0, 0.08, 1.45], [0, 0.07, 1.45], [0, 0.09, 1.45]],
                        dtype=float)
        gewichte = {'vertex_count': 6,
...
class MimikTest(SimpleTestCase):
    ...
    def test_mblab_gewichte(self):
        g = MblabAusdruecke.gewichte({
            'Expressions_mouthSmile': 1.0, 'Expressions_browsMidVert': 0.2,
            'Expressions_eyeSquintL': 0.5, 'Expressions_chestExpansion': 0.9})
    ...
    def test_knochendeltas_drehung_und_versatz(self):
        with Pruefablage.ordner() as ordner:
            ...
            rot = np.array([[1, 0, 0], [0, np.cos(winkel), -np.sin(winkel)],
    ...
    def test_mimikspuren_und_nebendatei(self):
        ...
        basis = {'e1': {'plus': {'DEF-jaw': [0.2, 0, 0, 0, 0.01, 0]},
                        'minus': {'DEF-jaw': [-0.1, 0, 0, 0, 0, 0]}}}
        spuren = Mimikspuren.spuren(
            {'fps': 30, 'einheiten': ['e1'], 'bilder': [[0], [1], [-1]]}, basis)
        ...
        with Pruefablage.ordner() as ordner:
            ...
            neben = Mimikspuren.schreiben(
                bvh, {'fps': 30, 'einheiten': ['e1'], 'bilder': [[0.5]]})
            self.assertTrue(neben.endswith('probe_mimik.json'))
            self.assertTrue(os.path.isfile(neben))
            self.assertIsNone(
                Mimikspuren.laden(os.path.join(str(ordner), 'andere.bvh')))
