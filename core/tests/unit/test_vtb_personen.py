# -*- coding: utf-8 -*-
"""Mehrere Personen im Video: ein BVH je Person, synchron (14.09.2026).
...
def verlauf():
    """Zehn Bilder: A groß und immer da, B mittel in acht Bildern, C klein
...
class DieSpurwahl(unittest.TestCase):
    ...
    def test_bruchstuecke_werden_verkettet(self):
        """002_Dance: eine Taenzerin, zwei Kennungen (0-271 und 273-1003, ein
    ...
    def _szenen(zweite_person=False):
        """80 Bilder, drei Schnitte: Kennung 1 (0-19, links), 2 (19-44, rechts,
    ...
    def test_szenenwechsel_eine_person(self):
        """005 DanceLang: keine zwei Spuren zur selben Zeit -> eine Person,
        ...
        self.assertEqual(wahl.bilder[1].tolist(), list(range(45)) + list(range(52, 80)))
    ...
    def test_szenenwechsel_zwei_personen_bleiben_getrennt(self):
        """Ueberschneiden sich zwei Spuren laenger als `MINDESTLAENGE` Bilder
...
class LaufAttrappe:
    """Merkt sich, womit `Personenlauf` sie baut; `fahren` gibt das BVH."""
...
class DerPersonenlauf(unittest.TestCase):
    def test_namen(self):
        ...
        self.assertEqual(
            Personenlauf.spuren_datei("aus/x.bvh", "ein/tanz.mp4"), os.path.join("aus", "tanz_spuren.npz")
        )

    def test_ein_lauf_je_person_mit_gemeinsamer_spurablage(self):
        LaufAttrappe.gebaut = []
        fertig = Personenlauf(LaufAttrappe, "ein/tanz.mp4", "aus/gvhmr_tanz.bvh", 3, geraet="cuda").fahren()
        self.assertEqual(fertig, ["aus/gvhmr_tanz.bvh", "aus/gvhmr_tanz_p2.bvh", "aus/gvhmr_tanz_p3.bvh"])
        self.assertEqual([l.person for l in LaufAttrappe.gebaut], [1, 2, 3])
        self.assertEqual({l.spuren for l in LaufAttrappe.gebaut}, {os.path.join("aus", "tanz_spuren.npz")})
        self.assertEqual(LaufAttrappe.gebaut[0].rest, {"geraet": "cuda"})

    def test_weniger_personen_als_bestellt(self):
        """Nach Person 1 liegt die Spurablage: das Video hat zwei Personen,
        ...
        with Pruefablage.ordner("personen_") as ordner:
            ...
            Spurwahl(verlauf(), 10, 640, 360).speichern(Personenlauf.spuren_datei(ziel, "ein/tanz.mp4"))
            fertig = Personenlauf(LaufAttrappe, "ein/tanz.mp4", ziel, 3).fahren()
        self.assertEqual([os.path.basename(f) for f in fertig], ["gvhmr_tanz.bvh", "gvhmr_tanz_p2.bvh"])
...
class DieKommandozeile(unittest.TestCase):
    ...
    def test_gvhmr_rechnet_ab_person_2_im_eigenen_ordner(self):
        ...
        self.assertEqual(zwei.befehl()[zwei.befehl().index("--output_root") + 1], os.path.join("aus", "p2"))
        self.assertEqual(zwei.vorstufenordner(), os.path.join("aus", "p2", "tanz", "preprocess"))
        self.assertEqual(
            zwei.ergebnisdatei(zwei.ausgabewurzel), os.path.join("aus", "p2", "tanz", Gvhmrlauf.ERGEBNISDATEI)
        )
...
class DieDjangoSeite(SimpleTestCase):
    def befehl(self, pipeline, **params):
        ...
        with override_settings(PIPELINE_PYTHON="py.exe"):
            return Smplbefehl(AuftragsAttrappe(pipeline, params), EinstellungenAttrappe()).bauen(
    ...
    def test_personenergebnisse_finden_und_eintragen(self):
        ...
        with Pruefablage.ordner("personen_") as ordner:
            ...
            self.assertEqual(
                [os.path.basename(p) for p in gefunden], ["gvhmr_tanz_p2.bvh", "gvhmr_tanz_p3.bvh"]
            )
            ...
            self.assertEqual(
                [os.path.basename(e[0]) for e in eintraege], ["tanz_gvhmr_p2.bvh", "tanz_gvhmr_p3.bvh"]
            )
    ...
    def test_formular_karten_und_adresse(self):
        ...
        gvhmr = Sicher.wert(
            re.search(r"\n        gvhmr: \[(.*?)\n        \],", felder, re.S), "gvhmr-Felder"
        ).group(1)
        gem = Sicher.wert(
            re.search(r"\n        gem: \[(.*?)\n        \],", felder, re.S), "gem-Felder"
        ).group(1)
        self.assertIn("['persons', 'int']", gvhmr)
        self.assertIn("['persons', 'int']", gem)
        for karte, name in (("_pipeline_gvhmr.html", "gvhmr_persons"), ("_pipeline_gem.html", "gem_persons")):
...
class EinstellungenAttrappe2(EinstellungenAttrappe):
    """Alles, was `Pipelineparameter.vorgaben` liest."""
