# -*- coding: utf-8 -*-
"""Der Schuh als vierter Baustein: Katalog, Regler, Titel, Erkennung.
...
class SchuhkatalogTest(SimpleTestCase):
    ...
    def test_der_katalog_fuehrt_einen_schuh_und_die_alten_namen_als_formen(self):
        """Edgar, 11.09.2026: „alle Schuhe zu einem, und z.B: Slipper und
        ...
        self.assertEqual(
            [p['titel'] for p in formen],
            ['Ballerina', 'Slipper', 'Pumps', 'Plateauschuh', 'Stiefelette', 'Stiefel', 'Socke'],
        )
    ...
    def test_jeder_schuh_traegt_den_vierten_baustein(self):
        for schuh in SCHUHE:
            ...
            self.assertIn(Schuhentwurf.baustein(entwurf), ('Halbschuh', 'Stiefel'), schuh)
    ...
    def test_der_schuh_zeigt_schuh_und_schaftregler(self):
        """Die Gruppen aller Formen — der Reiter kann sie beim Anhaken
        einer Form nicht neu holen."""
        self.assertEqual([g['gruppe'] for g in Katalog.regler('schuh')], ['shoe', 'boot'])
    ...
    def test_die_schuhregler_sind_vollstaendig_beschriftet(self):
        """Titel UND Hilfetext für jedes Feld — englische Reste fallen auf."""
        for gruppe in Katalog.regler('schuh'):
            self.assertNotEqual(Reglertitel.gruppe(gruppe['gruppe']), gruppe['gruppe'])
    ...
    def test_pumps_stehen_auf_stiletto_mit_sprengung(self):
        ...
        self.assertEqual(Schuhentwurf.absatzform(Katalog.entwurf('ballerina')), 'block')
        self.assertEqual(Katalog.entwurf('ballerina')['shoe']['toe_spring']['v'], 0.0)

    def test_die_werte_der_yaml_liegen_in_ihren_bereichen(self):
        """Ein Vorgabewert ausserhalb des Bereichs klemmt am Anschlag."""
        ...
        for gruppe in Schuhentwurf.GRUPPEN:
            for name, eintrag in quelle[gruppe].items():
                if eintrag['type'] in ('float', 'int'):
                    ...
                    self.assertTrue(unten <= eintrag['v'] <= oben, '%s.%s' % (gruppe, name))
                else:
                    self.assertIn(eintrag['v'], eintrag['range'], '%s.%s' % (gruppe, name))
...
class SchuhentwurfTest(SimpleTestCase):
    ...
    def test_mischen_ergaenzt_nur_was_fehlt(self):
        entwurf = {
            'meta': {'upper': {'v': 'Shirt'}},
            'shoe': {'length': {'v': 9.9, 'range': [0, 10], 'type': 'float'}},
        }
...
class SchuhschnittErkennungTest(SimpleTestCase):
    ...
    def setUp(self):
        self.ordner = os.path.join(settings.BASE_DIR, '_wegwerf', 'test_schuhentwurf')
    ...
    def test_ein_schnitt_mit_sohle_ist_ein_schuh(self):
        self.assertTrue(Schuhentwurf.ist_schuhschnitt(self._spec(['probe_l_sohle', 'probe_l_blatt'])))

    def test_ein_kleid_ist_keiner(self):
        self.assertFalse(Schuhentwurf.ist_schuhschnitt(self._spec(['front', 'back', 'sleeve_l'])))

    def test_eine_fehlende_datei_ist_keiner(self):
        self.assertFalse(Schuhentwurf.ist_schuhschnitt(os.path.join(self.ordner, 'gibtsnicht.json')))


class SchuhstoffTest(SimpleTestCase):
    """Der Stoff steht in der Spezifikation, die Drapierung liest ihn.
    ...
    def setUp(self):
        self.ordner = os.path.join(settings.BASE_DIR, '_wegwerf', 'test_schuhstoff')
        os.makedirs(self.ordner, exist_ok=True)
    ...
    def test_stiefel_aus_leder_socke_und_ballerina_aus_stoff(self):
        self.assertEqual(Schuhentwurf.stoffname(Katalog.entwurf('stiefel')), 'leather')
        self.assertEqual(Schuhentwurf.stoffname(Katalog.entwurf('stiefelette')), 'leather')
        for stueck in ('socke', 'ballerina', 'slipper'):
            self.assertEqual(Schuhentwurf.stoffname(Katalog.entwurf(stueck)), 'cloth', stueck)

    def test_der_vermerk_kommt_als_materialwerte_zurueck(self):
        Schuhentwurf.stoff_vermerken(self.pfad, Katalog.entwurf('stiefel'))
        self.assertEqual(Schuhentwurf.stoff(self.pfad), Schuhentwurf.STOFFE['leather'])
        ...
        self.assertEqual(Schuhentwurf.stoff(self.pfad), Schuhentwurf.HALBSCHUH_STEIFE)
        self.assertLess(
            Schuhentwurf.stoff(self.pfad)['garment_edge_ke'],
    ...
    def test_ohne_vermerk_keine_werte(self):
        self.assertEqual(Schuhentwurf.stoff(self.pfad), {})
        self.assertEqual(Schuhentwurf.stoff(os.path.join(self.ordner, 'gibtsnicht.json')), {})
