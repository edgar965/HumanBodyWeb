# -*- coding: utf-8 -*-
"""Die letzten Einstellungen stehen beim naechsten Aufruf.
...
class ReitergedaechtnisTest(SimpleTestCase):
    ...
    def test_die_kennung_ist_nur_die_id(self):
        """Kein `data-pfad` — sonst zwei Buchfuehrungen fuer eine Zahl.
    ...
    def test_die_bauregler_eines_presets_werden_gemerkt(self):
        """Ein Preset setzt „An die Haut ziehen" OHNE `input`-Ereignis
    ...
    @staticmethod
    def _quelle(pfad):
        return io.open(settings.BASE_DIR / "static" / "viewer" / pfad, encoding="utf-8").read()
