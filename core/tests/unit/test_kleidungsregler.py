...
class KleidungsreglerTest(SimpleTestCase):
    ...
    def test_anfrage_schlaegt_vorlage(self):
        regler = Kleidungsregler.aus_parametern({"offset": "0.02", "stiffness": "0.9"}, Vorlagenattrappe())
    ...
    def test_unbrauchbarer_wert_faellt_auf_die_vorgabe_zurueck(self):
        ...
        for kaputt in ("viel", "", None, "NaN?"):
            with self.subTest(wert=kaputt):
                regler = Kleidungsregler.aus_parametern({"offset": kaputt}, Vorlagenattrappe())
                self.assertAlmostEqual(regler.abstand, 0.01)

    def test_farbe_kanalweise_ueberschreibbar(self):
        regler = Kleidungsregler.aus_parametern({"color_g": "0.75"}, Vorlagenattrappe())
    ...
    def test_um_huelle_nur_bei_rig_hull(self):
        self.assertTrue(
            Kleidungsregler.aus_parametern({"fit_mode": "rig_hull"}, Vorlagenattrappe()).um_huelle
        )
        self.assertFalse(Kleidungsregler.aus_parametern({"fit_mode": "body"}, Vorlagenattrappe()).um_huelle)
        self.assertFalse(Kleidungsregler.aus_parametern({}, Vorlagenattrappe()).um_huelle)
