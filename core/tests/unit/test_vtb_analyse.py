# -*- coding: utf-8 -*-
"""Die Foto-Analyse von VideoToBVH: Backendpruefung, Unterlauf, Masse.
...
class DerUnterlauf(unittest.TestCase):
    """Die Fehlerzweige, die im Betrieb nie vorkommen — bis sie es tun."""
...
class DieKoerpermasse(unittest.TestCase):
    """Sechs Laengen in Metern — die Zentimeter entstehen erst zuletzt."""
    ...
    def test_anteile_sind_groessenfrei(self):
        """Ein doppelt so grosser Mensch hat dieselben Verhaeltnisse."""
        klein = self._masse()
        gross = Koerpermasse(
            hoehe=3.44,
            schulterbreite=0.64,
    ...
    def test_sechs_anteile_in_fester_ordnung(self):
        ...
        self.assertEqual(len(self._masse().anteile()), len(Koerperanteile.SCHLUESSEL))
...
class DieBackendliste(unittest.TestCase):
    """Vier Backends, vier ladbare Module, vier vollstaendige Antworten."""
