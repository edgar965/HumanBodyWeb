# -*- coding: utf-8 -*-
"""Der Start der beiden anderen Foto-Backends: HMR 2.0 und PyMAF-X.
...
class DerHmr2Start(unittest.TestCase):
    """Drei Eingriffe in fremde Bibliotheken, benannt und begruendet."""

    #: Eine Person (Klasse 0), ein Hund (16), eine unsichere Person.
    KLASSEN = [0, 16, 0]
    GUETE = [0.72, 0.99, 0.31]
    KAESTEN = [[10.0, 20.0, 110.0, 220.0], [0.0, 0.0, 50.0, 50.0], [1.0, 1.0, 2.0, 2.0]]

    def _detektor(self, klassen=None, guete=None):
        return Detectronattrappe(
            Erkennungen(
    ...
    def test_weights_only_wird_ueberschrieben_nicht_vorbelegt(self):
        """Der Befund: pytorch_lightning uebergibt den Schalter selbst.
    ...
    def test_unter_der_schwelle_zaehlt_niemand(self):
        kasten, guete = Hmr2start.beste_person(self._detektor(guete=[0.3, 0.4, 0.1]), None)
        self.assertIsNone(kasten)
        self.assertEqual(guete, 0.0)

    def test_die_schwellen_liegen_wie_bisher(self):
        """Was der Detektor gar nicht meldet, rettet die zweite nicht."""
...
class DiePymafxAusgabe(unittest.TestCase):
    """Vier Felder aus `mesh_out`, jedes frueher mit eigenem Vorspann."""
    ...
    def _ausgabe(self, **abweichend):
        return Pymafxausgabe(
            self.THETA, self._felder(**abweichend), np.array([[50.0, 60.0, 100.0, 200.0]]), np.array([1.25])
        )

    def test_pred_shape_schlaegt_die_theta_betas(self):
        """Elf Werte aus dem SMPL-X-Kopf sind genauer als zehn aus theta."""
    ...
    def test_zu_wenige_werte_lassen_theta_stehen(self):
        """Weniger als zehn Parameter waeren ein Rueckschritt."""
...
class Personenattrappe:
    """Eine Erkennung von openpifpaf: 133 Punkte und eine Guete."""
...
class Vorhersageattrappe:
    """Ein openpifpaf-`Predictor`, der eine feste Antwort gibt."""
...
class DerPymafxLauf(unittest.TestCase):
    """Eine Person, ueber der Guetegrenze — mehr nimmt die Analyse nicht."""
    ...
    def test_theta_traegt_kamera_und_form(self):
        ...
        self.assertEqual((Pymafxlauf.BETAS.start, Pymafxlauf.BETAS.stop), (3, 13))
