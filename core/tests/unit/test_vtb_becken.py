# -*- coding: utf-8 -*-
"""Das Becken der 2D-Ueberlagerung: wo die BVH-Wurzel im Kameraraum landet.
...
class DasBecken(unittest.TestCase):
    """Die Wurzel liegt im Kameraraum auf `transl` plus Wurzeloffset."""

    #: Der Wurzeloffset des BVH: die Lage des Beckens ueber dem Ursprung.
    RUHEHOEHE_CM = 35.070

    def _bvh(self, verschiebung):
        """Ein BVH-Datensatz ohne Drehungen, nur mit Wurzelbewegung."""
    ...
    def _kameraraum(self, bvh, bild):
        """Genau die Umrechnung von `Bildpunkte.aus_bvh`: drehen, teilen."""
    ...
    def test_mit_abzug_saesse_das_becken_auf_dem_ursprung(self):
        """Die Gegenprobe zur alten Fassung: Wer den Offset abzieht, setzt
    ...
    def test_die_kette_ist_zusammenhaengend(self):
        """Kein Gelenk liegt weiter als einen Knochen vom Elter weg."""
        ...
        for kind, elter in enumerate(Smplskelett.ELTERN):
            ...
            soll = float(np.linalg.norm(np.array(Smplskelett.OFFSETS[kind]))) / 100
