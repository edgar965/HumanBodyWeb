...
class SafePathTest(TestCase):
    ...
    def test_gross_klein_folgt_dem_dateisystem(self):
        ...
        dasselbe_verzeichnis = os.path.normcase(oben) == os.path.normcase(str(self.medien))
    ...
    def test_unc_pfad(self):
        ...
        for roh in (r'\\attacker\share\evil.json', '//attacker/share/evil.json', r'\\?\C:\Windows\evil.json'):
    ...
    def test_projektwurzel_gibt_nicht_das_ganze_arbeitsverzeichnis_frei(self):
        ...
        self.assertNotIn(tools, sp.wurzeln, 'TOOLS_ROOT ist als Wurzel eingetragen — das hebt den Schutz auf')
        for roh in (
            str(tools / 'evil.studio.json'),
