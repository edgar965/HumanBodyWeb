# -*- coding: utf-8 -*-
"""Genesis 9 ohne Daz-Bibliothek: Formeln, Netzteilung, Haut, Pfade, Ziel.
...
def kunstablage():
    """Drei Kanaele: ein Steuerregler, ein Morph mit Deltas, eine Knochenformel."""
    kanaele = {
        'ctrl': {
            'id': 'ctrl',
            'label': 'Steuer',
            'gruppe': '/People/Feminine',
            'min': 0.0,
            'max': 1.0,
            'vorgabe': 0.0,
            'sichtbar': True,
            'formeln': [
                {
                    'ziel': ['morph', 'body', 'value'],
                    'stufe': 'sum',
                    'ops': [{'op': 'push', 'kanal': 'ctrl'}, {'op': 'push', 'val': 0.5}, {'op': 'mult'}],
                },
                {
                    'ziel': ['knochen', 'hip', 'center_point/y'],
                    'stufe': 'sum',
                    'ops': [{'op': 'push', 'kanal': 'ctrl'}, {'op': 'push', 'val': 2.0}, {'op': 'mult'}],
                },
            ],
            'datei': 'ctrl.dsf',
        },
            ...
            'formeln': [
                {
                    ...
                    'stufe': 'sum',
                    'ops': [
                        {'op': 'push', 'kanal': 'spline'},
                        {'op': 'spline_linear', 'val': [[0, 0], [1, 4]]},
                    ],
                }
            ],
            'datei': 'spline.dsf',
    ...
    return G9morphablage(
        kanaele,
        punkt,
        delta,
        {'body': 0, 'ctrl': 3, 'halb': 3, 'spline': 3},
...
class FormelnTest(SimpleTestCase):
    ...
    def test_sum_mult_und_knochen(self):
        """ctrl 0,8 -> body = 0,8·0,5 (sum) × halb 1,0 (mult) = 0,4; hip +1,6 cm."""
        G9formeln.vergessen()
    ...
    def test_spline_und_grenzen(self):
        """Spline linear 0..1 -> 0..4, dann auf max 2 begrenzt."""
...
class NetzteilungTest(SimpleTestCase):
    ...
    def test_naht_teilt_punkt_und_kachel_faellt_weg(self):
        """Zwei Vierecke teilen die Kante 1-2; Viereck 1 liest sie mit UV 4/5
        in Kachel 1002."""
        polys = [[0, 0, 0, 1, 2, 3], [0, 1, 1, 4, 5, 2]]
        uvs = np.array([[0.1, 0.1], [0.9, 0.1], [0.9, 0.9], [0.1, 0.9], [1.1, 0.1], [1.1, 0.9]])
        # Viereck 1 liest Punkt 1 als UV 4 (u 1.1) und Punkt 2 als UV 5
        ueber = {(1, 1): 4, (1, 2): 5}
        ursprung, uv, dreiecke, gruppen = G9netzteilung.teilen(polys, ['Head', 'Body'], uvs, ueber)
...
class PfadeUndMaterialTest(SimpleTestCase):
    ...
    def test_daz_adresse_aufloesen(self):
        pfad, kennung = G9pfade.aufloesen('/data/Daz%203D/Genesis%209/Base/Genesis9.dsf#hip')
    ...
    def test_nur_bilder_unter_runtime_textures(self):
        self.assertEqual(
            G9material.relativ('/Runtime/Textures/DAZ/a%20b/x.jpg'), 'Runtime/Textures/DAZ/a b/x.jpg'
        )
    ...
    def test_morphkanalname_und_ziel(self):
        self.assertEqual(
            G9morphablage.kanalname(
                'Genesis9:/data/Daz%203D/x/Amala_head_bs_Head.dsf#Amala_head_bs_Head?value'
            ),
            'Amala_head_bs_Head',
        )
        self.assertEqual(
            G9morphablage.ziel('hip:/data/x.dsf#hip?center_point/y'), ('knochen', 'hip', 'center_point/y')
        )
        self.assertEqual(G9morphablage.ziel('Genesis9:#X?value'), ('morph', 'X', 'value'))
...
class ZielTest(SimpleTestCase):
    ...
    def test_retargetwahl_kennt_genesis9(self):
        wahl = Retargetwahl(
            {'target': 'genesis9', 'figur': 'amala', 'regler': {'Amala_figure_ctrl_Character': 1}}, 1.68
        )
