# -*- coding: utf-8 -*-
"""`Figurvideoablage`: Ordner, Name, Kopie — und dass nichts ueberschrieben wird.
...
class FigurvideoablageOrdnerTest(SimpleTestCase):
    ...
    def test_leer_ist_vorgabe(self):
        self.assertEqual(Figurvideoablage.ordner_pruefen(''), Figurvideoablage.vorgabe_ordner())
        self.assertEqual(Figurvideoablage.ordner_pruefen(None), Figurvideoablage.vorgabe_ordner())
...
class FigurvideoablageKopieTest(SimpleTestCase):
    ...
    def test_fuer_auftrag_kopiert_einmal(self):
        with Pruefablage.ordner('videoablage_') as wurzel:
            ...
            with open(os.path.join(auftrag, 'auftrag.json'), 'w', encoding='utf-8') as datei:
    ...
    def test_fuer_auftrag_ohne_ablage(self):
        with Pruefablage.ordner('videoablage_') as wurzel:
            with open(os.path.join(wurzel, 'auftrag.json'), 'w', encoding='utf-8') as datei:
