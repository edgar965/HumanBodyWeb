# -*- coding: utf-8 -*-
"""`Videokodierer.aus_bildfolge`: ungerade Bilder muessen kodierbar bleiben.
...
class VideokodiererTest(SimpleTestCase):
    ...
    def test_befehl_rundet_ohne_feste_groesse(self):
        ...
        fest = Videokodierer.aus_bildfolge('ordner', 'ziel.mp4', breite=1920, hoehe=1080)
    ...
    def test_ungerade_bilder_werden_kodiert(self):
        ...
        with Pruefablage.ordner('videokodierer_') as ordner:
            for i in range(3):
                cv2.imwrite(
                    os.path.join(ordner, '%06d.png' % i), np.full((185, 319, 3), 90 + 40 * i, np.uint8)
                )
