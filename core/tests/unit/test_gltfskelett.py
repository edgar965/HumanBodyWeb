# -*- coding: utf-8 -*-
"""`Gltfskelett` — das Skelett aus einer GLB, ohne das Netz zu lesen.
...
class GltfskelettTest(SimpleTestCase):
    ...
    def test_joints_kommen_einmal(self):
        """`Hips` steht zweimal in der Joint-Liste — und einmal im Skelett."""
    ...
    def test_eine_zu_kurze_datei_ist_ein_valueerror_kein_struct_error(self):
        """9 Byte: `struct` wuerde einen eigenen Fehler werfen, und der
    ...
    def test_die_wurzel_traegt_die_drehung_der_vorfahren(self):
        """`Avatar` (halbe Drehung um Y) ist kein Joint — seine Drehung
        ...
        self.assertTrue(np.allclose(np.abs(q), np.abs(Umaattrappe.YAW_180), atol=1e-6), q)
    ...
    def test_kinder_behalten_ihre_lokale_lage(self):
        ...
        self.assertTrue(np.allclose(knochen['LeftArm']['local_position'], [0, 0.1, 0], atol=1e-6))

    def test_skalierung_geht_in_die_positionen(self):
        """`LeftFoot` ist um 2 in Y skaliert: `LeftToeBase` sitzt doppelt
        so weit — und die Skalierung selbst verschwindet."""
        zehe = self._knochen()['LeftToeBase']
        self.assertTrue(
            np.allclose(zehe['local_position'], [0, 0.2, 0.05], atol=1e-6), zehe['local_position']
        )

    def test_die_weltlage_stimmt_mit_der_attrappe_ueberein(self):
        """Linke Hand: bei ungedrehter Wurzel bei +X, mit der Avatar-Drehung
        bei -X — wie in der echten GLB (x = -0,34)."""
        welt = SkeletonGeometry.from_three(self.skelett.knochen()).compute_world_transforms()
        self.assertLess(welt['LeftHand']['world_pos'][0], -0.5)
        soll = SkeletonGeometry.from_three(Umaattrappe.gedreht()).compute_world_transforms()
        self.assertTrue(np.allclose(welt['LeftHand']['world_pos'], soll['LeftHand']['world_pos'], atol=1e-6))

    # ------------------------------------------------------------- Bindpose

    def test_bindpose_ist_nicht_knotenpose(self):
        """Ab den Hueften ist die Bindpose um BIND_VERSATZ verschoben:
        ...
        erwartet = np.array(knoten['Hips']['local_position']) + Quat.rotate(
            Quat.inv(eltern_rot), Umaattrappe.BIND_VERSATZ
        )
        self.assertTrue(
            np.allclose(bind['Hips']['local_position'], erwartet, atol=1e-5), bind['Hips']['local_position']
        )
        for name in ('LowerBack', 'LeftArm', 'LeftUpLeg', 'LeftToeBase'):
            self.assertTrue(
                np.allclose(bind[name]['local_position'], knoten[name]['local_position'], atol=1e-5), name
            )
    ...
    def test_nur_der_json_chunk_wird_beim_oeffnen_gelesen(self):
        """Der Binaerteil ist bei der echten GLB 27 MB — er darf erst fuer
