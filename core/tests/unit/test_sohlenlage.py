# -*- coding: utf-8 -*-
"""Die Sohle folgt der Fussmitte und bleibt in der Simulation, wo sie liegt.
...
class SohlenlageTest(SimpleTestCase):
    ...
    def test_die_sohle_beginnt_im_ursprung_und_liegt_auf_der_fersenmitte(self):
        ...
        self.assertNotAlmostEqual(p.randlaenge_hinten("a"), p.randlaenge_hinten("i"), delta=0.01)

    def test_die_sohle_folgt_der_fussmitte_im_raum(self):
        """An der Ferse liegt die Sohlenmitte auf `foot_heel_x`, die Spitze
        ...
        self.assertAlmostEqual(
            (hinten[:, 0].min() + hinten[:, 0].max()) / 2.0, self.FUSS["foot_heel_x"], delta=0.3
        )
    ...
    def test_der_sohlenhalt_bindet_innen_an_die_anfangslage_und_den_rand_an_die_kontur(self):
        """Ein Sohlenpanel als Quadrat (Punkte 0–3 innen), zwei Nahtpunkte
        ...
        stoff = Stoff()
        init = np.array(
            [[0, 0, 0], [1, 0, 0], [1, 0, 1], [0, 0, 1], [1.5, 2.0, 0.5], [-0.5, 2.0, 0.5]], dtype=float
        )
    ...
    def test_der_absatz_endpunkt_liefert_die_reinen_fussdeltas(self):
        mit = self.client.get("/api/character/pose/ruhelage/?winkel_grad=25&hebung_cm=4.9").json()
        self.assertEqual(sorted(mit["absatz_deltas"]), ["DEF-foot.L", "DEF-foot.R", "DEF-toe.L", "DEF-toe.R"])
