# -*- coding: utf-8 -*-
"""`restart_server.py` darf nur den Server DIESES Projekts beenden.
...
class ServerneustartTest(SimpleTestCase):
    ...
    def test_nur_das_eigene_verzeichnis_zaehlt(self):
        """Der Kern: ein fremdes Projekt darf nie getroffen werden."""
        eigen = mock.Mock()
    ...
    def test_ein_unlesbares_verzeichnis_bleibt_am_leben(self):
        """Im Zweifel lieber ein Server zu viel als ein fremder weniger."""
        import psutil
    ...
    def test_gestartet_wird_mit_dem_python_dieses_laufs(self):
        """Nicht `python` aus dem PATH — damit kam der Server nicht hoch."""
        import sys

        self.assertEqual(self.neustart.python(), sys.executable)

    def test_der_port_wird_ueber_127_0_0_1_geprueft(self):
        """Nie `localhost`: Die IPv6-Aufloesung kostet unter Windows Sekunden.
