# -*- coding: utf-8 -*-
"""Die Nacharbeit rechnet gegen Haut PLUS getragene Stuecke (11.09.2026).
...
def _kasten():
    """Dichter Kasten 1 x 0,2 x 1 um den Ursprung (Punkte, Dreiecke)."""
    import trimesh
...
def _gitter(y, x0=-0.4, x1=0.4, n=17):
    """Ein triangulierter Stoffteppich bei Hoehe y (Punkte, Dreiecke)."""
...
class HautmitstoffTest(SimpleTestCase):
    ...
    def test_ohne_das_stueck_landet_der_stoff_auf_der_haut(self):
        """Die Sabotage-Gegenprobe: genau Edgars Bild."""
        stoff, dreiecke = _gitter(0.115)
        haut = Hautmitstoff(self.koerper, self.flaechen)
    ...
    def test_knapp_unter_dem_saum_bleibt_der_stoff_ueber_dem_stueck(self):
        """Die Saumregel: Ein Punkt 5 mm neben der Kante des Stuecks findet
        ...
        punkte = np.vstack([haut, shirt])
        normalen = np.vstack([oben, oben_s])
...
class NacharbeitTest(SimpleTestCase):
    ...
    def test_die_nacharbeit_nimmt_getragene_rig_dateien(self):
        ...
        try:
            ...
            self.assertAlmostEqual(
                float(np.median(neu[(stoff[:, 0] > 0.1) & (stoff[:, 0] < 0.3), 1])), 0.122, places=3
            )
            ...
            self.assertIsNone(
                Stoffnacharbeit(koerper, flaechen, dreiecke, getragen=["/gibt/es/nicht_rig.json"]).haut
            )
        finally:
            os.remove(pfad)
            os.rmdir(ordner)
    ...
    def test_der_einzelbau_reicht_getragen_bis_zur_nacharbeit(self):
        quelle = io.open(settings.ASSETS_ROOT / "GarmentCode" / "drapierdienst.py", encoding="utf-8").read()
...
class GemeinsamTest(SimpleTestCase):
    ...
    def test_je_stueck_gehoeren_die_anderen_zur_haut(self):
        ...
        teile = {"hose": {"indizes": [0, 1]}, "shirt": {"indizes": [2, 3, 4]}, "leer": {"indizes": []}}
        ...
        quelle = io.open(settings.ASSETS_ROOT / "GarmentCode" / "gemeinsamablage.py", encoding="utf-8").read()
