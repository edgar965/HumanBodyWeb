# -*- coding: utf-8 -*-
"""Der Pipelinevergleich (Hilfe -> Video to BVH) ist vollstaendig und stimmig.
...
class DerVergleich(unittest.TestCase):
    def test_jede_pipeline_des_modells_hat_genau_einen_grundeintrag(self):
        """Varianten (12.09.2026, `hybrid_gem` mit GEM-X-Fingern) sind
        ...
        for e in varianten:
            with self.subTest(pipeline=e['schluessel']):
                ...
                self.assertNotEqual(
                    ...
                    next(
                        g['rang']
                        for g in Pipelinevergleich.grundeintraege()
                        if g['schluessel'] == e['schluessel']
                    ),
                )

    def test_jeder_lauf_mit_rang_nennt_sein_skelettvideo(self):
        """Edgar (12.09.2026): „ist das auch im ranking gekennzeichnet?" —
        ...
        grund = next(e for e in Pipelinevergleich.grundeintraege() if e['schluessel'] == 'hybrid_gem')
    ...
    def test_raenge_sind_eins_bis_n_ohne_luecke_nur_fuer_die_mit_ergebnis(self):
        """Rang 1..10 fuer die, die ein BVH liefern; die anderen haben keinen
    ...
    def test_jeder_eintrag_traegt_alle_felder(self):
        for e in Pipelinevergleich.alle():
            with self.subTest(pipeline=e['schluessel']):
                ...
                self.assertEqual(e['zustand_text'], Pipelinevergleich.ZUSTAND[e['zustand']])
    ...
    def test_namen_kommen_aus_dem_modell(self):
        """Eine Variante haengt ihren Zusatz an den Modellnamen an."""
