# -*- coding: utf-8 -*-
u"""Genesis 9, 18.09.2026 (Edgar: „Forsche wie das geht!"): HD-Morphs (`.dhdm`),
...
def _dhdm(stufen):
    u"""Eine `.dhdm` aus `{stufe: [(flaeche, idx16, x, y, z)]}` bauen."""
        ...
        for flaeche, eintraege in je_flaeche.items():
            ...
            for idx, x, y, z in eintraege:
                if stufe < 4:
                    ...
                    daten += struct.pack('<fBBBBff', x, 0, idx & 255, idx >> 8,
                                         (stufe + 1) * 16, y, z)
...
class Genesis9HdMorph(SimpleTestCase):
    ...
    def test_1_leser_zerlegt_adresse_und_vektor(self):
        ...
        roh = _dhdm({1: [(5, 0b10_11 << 12, 1.0, 2.0, 3.0)],
                     2: [(5, (9 << 12) | (1 << 10), 0.5, 0.0, -0.5),
                         (7, (0 << 12) | (2 << 10), 0.0, 1.0, 0.0)],
                     3: [(5, 0, 9.0, 9.0, 9.0)]})
        ...
        s1 = m.stufen[1]
        self.assertEqual((int(s1['flaeche'][0]), int(s1['unterflaeche'][0]),
                          int(s1['ecke'][0])), (5, 2, 3))
    ...
    def test_2_rahmen_der_flaechenecke(self):
        u"""Viereck in der xz-Ebene, Umlauf gegen den Uhrzeiger von oben: z-Achse
        ...
        self.assertEqual(G9hdmorphe._beginn(np.array([[0, 1, 2, -1], [0, 1, 2, 3],
                                                      [0, 1, 2, 3]])).tolist(),
                         [0, 3, 7])
...
class Genesis9Requisit(SimpleTestCase):
    ...
    KNOTEN = {'id': 'Dolch', 'parent': 'name://@selection/r_hand:',
              'translation': [{'id': 'x', 'current_value': -50.0},
                              {'id': 'y', 'current_value': 70.0}],
              'rotation': [{'id': 'z', 'current_value': 90.0}],
              'center_point': [{'id': 'y', 'current_value': 20.0}],
              'rotation_order': 'YXZ'}

    def test_3_knoten_lesen(self):
        definition = {'rotation_order': 'XYZ',
                      'center_point': [{'id': 'y', 'value': -38.0}]}
    ...
    def test_5_griffpose_ohne_skalierung_eins(self):
        doc = G9dson('x.duf', {'scene': {'animations': [
            {'url': 'name://@selection/r_thumb1:?rotation/x/value',
             'keys': [[0, -8.4]]},
            {'url': 'name://@selection/r_thumb1:?scale/general/value',
             'keys': [[0, 1]]},
...
class Genesis9Eigenknochen(SimpleTestCase):
    ...
    def _doc(self):
        return G9dson('haar.dsf', {'node_library': [
            {'id': 'Haar', 'type': 'figure'},
            {'id': 'spine4', 'type': 'bone', 'parent': '#Haar',
             'center_point': [{'id': 'y', 'value': 120}],
             'end_point': [{'id': 'y', 'value': 140}]},
            {'id': 'Zopf', 'type': 'bone', 'parent': '#spine4',
             'center_point': [{'id': 'y', 'value': 160}],
             'end_point': [{'id': 'y', 'value': 120}], 'rotation_order': 'XYZ',
             'inherits_scale': True},
        ], 'modifier_library': [{'id': 'SkinBinding', 'skin': {
            'vertex_count': 2,
            'joints': [{'id': 'spine4', 'node': '#spine4',
    ...
    def test_6_eigene_knochen_drehen_ihre_punkte(self):
        ...
        roh = [{'name': 'spine4', 'eltern': None, 'kopf': np.array([0., 120., 0.]),
                'schwanz': np.array([0., 140., 0.]), 'erbt': False,
                'orientation': np.zeros(3), 'reihenfolge': 'XYZ'}] + eigene.knochen
        ...
        self.assertTrue(np.allclose(gelenke[0]['schwanz'], [0.4, 1.5, 0.0], atol=1e-6),
                        gelenke[0])
...
class Genesis9Verschiebung(SimpleTestCase):
    ...
    def test_7_verschiebung_im_elternraum(self):
        u"""`hip?translation/y` unter der um 25 % skalierten Figur: Daz waechst
        die Verschiebung mit (`W = W_eltern · T(t) · …`)."""
        roh = [{'name': 'hip', 'eltern': None, 'kopf': np.array([0., 100., 0.]),
                'schwanz': np.array([0., 80., 0.]), 'erbt': True,
                'orientation': np.zeros(3), 'reihenfolge': 'XYZ'}]
        m = G9knochenmatrizen({'Genesis9': {'scale/general': 1.25},
                               'hip': {'translation/y': 20.0}}, roh=roh)
        gelenk = (m.matrizen()['hip'] @ np.array([0., 100., 0., 1.]))[:3]
        self.assertTrue(np.allclose(gelenk, [0., 150., 0.]), gelenk)   # 1,25·(100+20)
...
class Genesis9Stilarten(SimpleTestCase):
    ...
    def test_8_stil_pose_laenge(self):
        def doc(zeilen):
            return G9dson('s.duf', {'scene': {'animations': [
                {'url': u, 'keys': [[0, w]]} for u, w in zeilen]}})
        eigene, knochen = {'Frizz'}, {'Zopf'}
        werte, gedreht, fremd = G9garderobeeintrag._stilwerte(doc([
            ('name://@selection/Zopf:?rotation/x/value', 10.7),
            ('name://@selection/Zopf:?scale/general/value', 1),
            ('name://@selection#Frizz:?value/value', 0.45)]), eigene, knochen)
        self.assertEqual((werte, gedreht, fremd),
                         ({'Frizz': 0.45}, {'Zopf': {'rotation/x': 10.7}}, False))
        self.assertEqual(G9garderobeeintrag._stilart(gedreht), 'pose')
        _w, laenge, _f = G9garderobeeintrag._stilwerte(doc([
            ('name://@selection/Zopf:?scale/y/value', 1.07)]), eigene, knochen)
        self.assertEqual(G9garderobeeintrag._stilart(laenge), 'laenge')
        self.assertEqual(G9garderobeeintrag._stilart({}), 'stil')
        _w, _g, fremd = G9garderobeeintrag._stilwerte(doc([
            ('name://@selection/r_thumb1:?rotation/x/value', -8.4)]), eigene, knochen)
...
class Genesis9Browserbilder(SimpleTestCase):
    ...
    def test_9_detailnormalen_nur_hoch(self):
        ...
        with mock.patch.object(G9browserbilder, 'tragbar', side_effect=tragbar):
            ...
            ohne = dict(bilder)
            del ohne['normalen']
            self.assertEqual(G9browserbilder.fuer(ohne, hoch=True)['normalen'],
                             'Runtime/Textures/x/8k_NM.jpg')
            self.assertEqual(G9browserbilder.fuer(ohne, hoch=False)['normalen'],
                             'Runtime/Textures/x/A_NM_1001.jpg')
