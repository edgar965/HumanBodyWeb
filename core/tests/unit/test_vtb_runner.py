# -*- coding: utf-8 -*-
"""Die Runner-Bausteine von VideoToBVH — der Teil ohne Grafikkarte.
...
class DerRunnerrahmen(unittest.TestCase):
    """Argumente pruefen, Fremdprojekt einbinden, Ergebnis melden."""

    def _rahmen(self, ordner='.'):
        return Runnerrahmen('Probe', ordner)

    def _abfangen(self, rufen):
        """Die JSON-Zeile eines abbrechenden Aufrufs."""
        import io
    ...
    def test_ohne_argument_ein_fehler(self):
        daten, code = self._abfangen(lambda: self._rahmen().bildpfad(['_run_x.py']))
        self.assertIn('Aufruf', daten['error'])
        self.assertEqual(code, 1)

    def test_ein_fehlendes_bild_ist_ein_fehler(self):
        daten, code = self._abfangen(lambda: self._rahmen().bildpfad(['_run_x.py', '/gibt/es/nicht']))
    ...
    def test_ein_vorhandenes_bild_kommt_durch(self):
        with Pruefablage.datei('x', endung='.jpg') as pfad:
            self.assertEqual(self._rahmen().bildpfad(['_run_x.py', pfad]), pfad)

    def test_der_traceback_kommt_mit(self):
        """Ohne ihn steht im Protokoll „ImportError" und sonst nichts."""
...
class DasFotoergebnis(unittest.TestCase):
    """Acht Pflichtfelder, vorher dreimal von Hand gebaut."""

    def _ergebnis(self, **kw):
        werte = dict(backend='hmr2', modellart='smpl', betas=[0.1] * 10, zuversicht=0.87)
        werte.update(kw)
    ...
    def test_die_zuversicht_kommt_vom_aufrufer(self):
        """Der Befund: PyMAF-X meldete fest 0,9 statt des Messwerts."""
        self.assertEqual(self._ergebnis(zuversicht=0.31).als_dict()['confidence'], 0.31)

    def test_masse_und_hautfarbe_bleiben_leer(self):
        """Diese Backends messen nichts — `Morphzuordnung` rechnet."""
        daten = self._ergebnis().als_dict()
    ...
    def test_hoechstens_zehn_parameter(self):
        daten = self._ergebnis(betas=[0.1] * 30, ausdruck=[0.2] * 30).als_dict()
    ...
    def test_zusatzfelder_kommen_dazu(self):
        daten = self._ergebnis().dazu(image_width=1920, pred_cam=[1.0, 0.0, 0.0]).als_dict()
    ...
    def test_leere_zusatzfelder_fallen_weg(self):
        """Sonst stand `if x: result['x'] = x` hinter jedem Feld."""
...
class DieGanzkoerperpunkte(unittest.TestCase):
    """133 Punkte, an festen Grenzen geschnitten."""

    def _punkte(self):
        """Ein Feld, in dem x den Index verraet."""
    ...
    def test_das_gesicht_wird_umsortiert(self):
        """FLAME will die Kinnlinie hinten, COCO-WholeBody fuehrt sie vorn."""
        eintrag = Ganzkoerperpunkte.person(self._punkte(), 0.9, 0, np)
        gesicht = eintrag['joints2d_face'][0]
        self.assertEqual(gesicht[0, 0], 40.0)  # 23 + 17
        self.assertEqual(gesicht[-1, 0], 39.0)  # 23 + 16
        self.assertEqual(sorted(gesicht[:, 0]), [float(i) for i in range(23, 91)])

    def test_die_erkennungsguete_wird_uebernommen(self):
        self.assertEqual(Ganzkoerperpunkte.person(self._punkte(), 0.42, 0, np)['score'], 0.42)
    ...
    def test_mehrere_personen_werden_aneinandergehaengt(self):
        eintraege = [Ganzkoerperpunkte.person(self._punkte(), 0.9, i, np) for i in range(3)]
...
class DieSmplestxKonfiguration(unittest.TestCase):
    """Die Datei liegt an zwei moeglichen Stellen."""
    ...
    def test_die_erste_stelle_gewinnt(self):
        with Pruefablage.ordner() as wurzel:
            ...
            self.assertIn('pretrained_models', Sicher.wert(Smplestxkonfiguration(wurzel).pfad(), 'Pfad'))

    def test_die_laufnamen_bleiben_wie_bisher(self):
        """Der Ordner in `outputs/` soll erkennbar bleiben."""
        k = Smplestxkonfiguration('/wurzel')
        self.assertEqual(k.laufname('C:/b/portrait.jpg', '20260902_0010'), 'inference_portrait_20260902_0010')
        self.assertEqual(k.laufname('', '20260902_0010', vorsatz=k.VORSATZ_VIDEO), 'video_expr_20260902_0010')

    def test_die_laufeinstellungen_nennen_gewicht_und_protokoll(self):
        einstellungen = Smplestxkonfiguration('/wurzel').laufeinstellungen('bild.jpg', '20260902_0010')
        self.assertIn('pth.tar', einstellungen['model']['pretrained_model_path'])
        self.assertIn('inference_bild_20260902_0010', einstellungen['log']['log_dir'])
