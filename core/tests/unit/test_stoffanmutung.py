# -*- coding: utf-8 -*-
"""Der Weg der UV vom Server bis ans Material — geprüft am Quelltext.
...
class DasNetzBekommtSeineUvTest(SimpleTestCase):
    ...
    def test_die_laenge_wird_gegen_die_punkte_geprueft(self):
        """Eine UV-Liste anderer Länge gehört nicht an dieses Netz."""
        self.assertIn("uv.length === punkte.length", self.geometrie)

    def test_die_uv_wird_nicht_in_three_achsen_gedreht(self):
        """Die Punkte werden von Z-oben auf Y-oben gedreht; eine UV ist eine
...
class DasMaterialIstStoffTest(SimpleTestCase):
    ...
    def test_die_kachel_wird_nur_einmal_gerechnet(self):
        """128×128 RGBA je Stück neu zu rechnen wäre Verschwendung — und
        vier Texturen statt einer im Speicher."""
        self.assertIn(
            "if (Garmentstoff._kacheln[artname]) return Garmentstoff._kacheln[artname]", self.quelle
        )

    def test_die_kachel_wiederholt_sich(self):
        """Ohne `RepeatWrapping` läuft die Kachel EINMAL über das Stück, und
    ...
    def test_mipmaps_und_anisotropie(self):
        """Bei rund fünfzig Wiederholungen flimmert eine Karte ohne
    ...
    def test_der_glanzsaum_traegt_die_stofffarbe(self):
        """Ein weißer Saum auf schwarzer Satin-Leggings stand in der
    ...
    def test_die_rechnung_liegt_ausserhalb(self):
        """`gemeinsam/gewebe.js` kennt weder DOM noch Three.js — nur so ist
        sie in node prüfbar."""
