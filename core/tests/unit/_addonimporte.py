# -*- coding: utf-8 -*-
"""Addonimporte — relative Importe des Blender-Addons ohne Blender pruefen.
...
class Addonimporte:
    """Loest relative Importe gegen den Dateibaum auf."""
    ...
    def relative_importe(baum):
        """Jeder `from .x import y` — absolute Importe zaehlen nicht."""
        return [k for k in ast.walk(baum) if isinstance(k, ast.ImportFrom) and k.level]

    # -------------------------------------------------------------- Aufloesen

    @staticmethod
    def ziel(pfad, knoten):
        """Auf welchen Pfad zeigt `from .x.y import …` aus dieser Datei?"""
        ordner = pfad.parent
    ...
    def _aus_dem_init(baum):
        """(Namen, Stern) aus einem `__init__.py`."""
    ...
    def paketnamen(cls, ordner):
        """Was ein `from <paket> import X` treffen kann — und ob ein
        Stern stoert.
    ...
    def ins_leere(cls, wurzel=None):
        """[(Datei, Zeile, Import)] — jeder relative Import ohne Ziel."""
        wurzel = cls.WURZEL if wurzel is None else wurzel
        schlecht = []
    ...
    def anzahl(cls, wurzel=None):
        """Wie viele relative Importe ueberhaupt geprueft werden.

        Sabotageschutz: Eine leere Menge bestuende jeden Test.
        """
        return sum(len(cls.relative_importe(baum)) for _pfad, baum in cls.baeume(wurzel))
