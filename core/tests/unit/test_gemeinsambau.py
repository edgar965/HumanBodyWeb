# -*- coding: utf-8 -*-
"""Die Bauregler im gemeinsamen Weg (11.09.2026).
...
class EndpunktBauTest(SimpleTestCase):
    ...
    def test_unsinn_im_bau_faellt_auf_die_vorgabe(self):
        gewaehlt = self._stuecke(
            [{"vorlage": "hose", "bau": "quatsch"}, {"vorlage": "hose", "bau": {"anliegen_mm": "x"}}]
        )
        for eintrag in gewaehlt:
            self.assertEqual(eintrag["fein"].hautabstand_mm, Baufeineinstellung.HAUTABSTAND_VORGABE)
...
class DienstBauTest(SimpleTestCase):
    ...
    def test_pruefen_reicht_fein_durch_und_setzt_sonst_die_vorgabe(self):
        ...
        gewaehlt = Garmentgemeinsam._pruefen([{"vorlage": "hose", "fein": fein}, {"vorlage": "t-shirt"}])
    ...
    def test_die_simulation_nimmt_die_werte_des_ersten_stuecks(self):
        ...
        gilt = Garmentgemeinsam.bauwerte(
            [{"vorlage": "a", "fein": erstes}, {"vorlage": "b", "fein": zweites}]
        )
...
class AblageBauTest(SimpleTestCase):
    ...
    def test_ohne_anliegen_bleibt_alles_wie_es_ist(self):
        ...
        neu, anlage, normalen = Gemeinsamablage._anlegen(schnitt, punkte, [[0, 0, 0]], (None, None))
...
def _lies_assets(name):
    return io.open(settings.ASSETS_ROOT / "GarmentCode" / name, encoding="utf-8").read()
