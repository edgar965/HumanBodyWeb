...
class QuellenBasis(TestCase):
    def setUp(self):
        ...
        for datei in list(self.ordner.glob("*.csv")) + list((self.ordner / "openpose_json").glob("*.json")):
            datei.unlink()
    ...
    def openpose_ablegen(self):
        ...
        datei.write_text(json.dumps({"people": [{"pose_keypoints_2d": punkte}]}), encoding="utf-8")
...
class OpenposeTest(QuellenBasis):
    ...
    def test_namen_werden_umgeschrieben(self):
        ...
        self.openpose_ablegen()
        for bilder in (
            self.quelle("openpose").aus_openpose(200, 100),
            self.quelle("openpose").aus_openpose(tupel=True, alle=True),
        ):
...
class DateiwahlTest(QuellenBasis):
    ...
    def test_eigene_erkenner_haben_eigene_datei(self):
        self.assertTrue(str(self.quelle("rtmpose").csv_pfad()).endswith("rtmpose_2d.csv"))

    def test_vorgabe_ist_die_mediapipe_datei(self):
        self.assertTrue(str(self.quelle("mocapnet").csv_pfad()).endswith("2dJoints_mediapipe.csv"))
