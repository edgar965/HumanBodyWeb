# -*- coding: utf-8 -*-
"""Die zwei Sitz-Kennzahlen eines drapierten Stuecks.
...
class ZweiZahlenStattEiner(SimpleTestCase):
    def test_das_engste_viertel_ist_das_untere_quartil(self):
        ...
        self.assertAlmostEqual(sitz.eng_mm, np.percentile(range(1, 11), 25), places=6)
...
class OhneAnkerGibtEsNichtsZuMessen(SimpleTestCase):
    """Ein Stueck ohne Verankerung ist kein Fehlerfall, sondern ein Stueck
    ohne Rig — es darf keine Ausnahme werfen."""

    def test_leere_ankerliste(self):
        sitz = Hautabstand.aus_rig(rig_mit([]))
        self.assertEqual((sitz.eng_mm, sitz.median_mm, sitz.punkte), (0.0, 0.0, 0))
...
class SitztOderSitztNicht(SimpleTestCase):
    """Beide Grenzen muessen halten — sie fangen verschiedene Fehler."""

    def test_ein_anliegendes_stueck_sitzt(self):
        """Zahlen des gemessenen „T-Shirt (anliegend)": 2,4 / 4,5 mm."""
        self.assertTrue(Hautabstand(2.4, 4.5).sitzt)

    def test_ein_weit_fallender_rock_sitzt_auch(self):
        """Kreisrock, gemessen: 8,2 mm im engsten Viertel, 27,4 Median.
    ...
    def test_auf_dem_falschen_koerper_sitzt_es_nicht(self):
        """Der dokumentierte Fall vom 06.09.2026: Median 27,4 mm mit 31 %
    ...
    def test_durchgehend_zu_weit_faellt_an_der_engen_grenze(self):
        """Ein Stueck, dessen Median die 40 mm haelt, das aber nirgends
...
class DerGemeinsameKernNimmtBeideEingaben(SimpleTestCase):
    """`aus_laengen` bekommt aus der Vorschau eine Liste und aus `aus_rig`
    ...
    def test_vorschau_und_bau_rechnen_gleich(self):
        """Dieselben Abstaende muessen dieselbe Kennzahl ergeben, egal auf
        ...
        self.assertAlmostEqual(ueber_liste.median_mm, ueber_rig.median_mm, places=6)
