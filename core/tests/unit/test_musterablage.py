...
class NamenspruefungTest(SimpleTestCase):
    ...
    def test_muster_braucht_flaechen(self):
        self.assertEqual(
            Musterablage({"name": "x", "pattern": {}}).fehler(), "Pattern with panels is required"
        )
        self.assertEqual(Musterablage({"name": "x"}).fehler(), "Pattern with panels is required")
...
class AblegenTest(SimpleTestCase):
    def setUp(self):
        ...
        umgebung = override_settings(HUMANBODY_GARMENT_LIBRARY_DIR=str(self.wurzel))
        umgebung.enable()
    ...
    def ablegen(self):
        ...
        ergebnis = {"vertices": np.zeros((3, 3), dtype=np.float32), "faces": np.array([[0, 1, 2]])}
    ...
    def test_muster_bleibt_bearbeitbar(self):
        ...
        gelesen = json.loads((Path(ablage.ordner()) / "specification.json").read_text(encoding="utf-8"))
