# -*- coding: utf-8 -*-
"""`Segmentierung`: welcher Vertex zu welchem Koerperteil zaehlt.
...
def netz(anzahl=4):
    """Ein Viereck je Vertex-Paar — Inhalt egal, nur die Form zaehlt."""
    faces = np.array([[0, 1, 2, 3]] * 2)
    return faces, np.array([0, 0])


class HilfsknochenTest(SimpleTestCase):
    """Ein Steuerknochen darf ein Koerperteil nicht ueberstimmen."""
    ...
    def test_steuerknochen_ueberstimmt_das_bein_nicht(self):
        """Vertex 0: Steuerknochen 0,7, Oberschenkel 0,3 -> linkes Bein.
...
class MaterialRueckfallTest(SimpleTestCase):
    """Ohne `materials.json` darf `face_internal` nicht leer bleiben."""
...
class EchteNetzeTest(SimpleTestCase):
    """Die Annahme hinter dem Rueckfall, an den echten Daten geprueft."""
    ...
    def test_materialreihenfolge_stimmt_in_beiden_netzen(self):
        """Fuer die Materialien 2 bis 8 — die inneren — muessen Flaechen-
        ...
        for geschlecht in ("female", "male"):
            ...
            zahlen[geschlecht] = {
                nr: (int((fm == nr).sum()), int(len(np.unique(faces[fm == nr])))) for nr in range(2, 9)
            }
        for nr in range(2, 9):
            self.assertEqual(
                zahlen["female"][nr],
    ...
    def test_maennlicher_schritt_liegt_wo_ein_schritt_liegt(self):
        """Der tiefste Rumpfpunkt muss ueber der Koerpermitte liegen.
...
class GrundnetzTest(SimpleTestCase):
    """Das Grundnetz muss zur Bauart passen, nicht zur Datei.
    ...
    def test_je_bauart_die_eigene_punktzahl(self):
        ...
        for bauart in ("Female_Caucasian", "Male_Caucasian"):
            ...
            netz = GarmentcodeDienst._grundnetz(Charakterdaten.geschlecht_zu(bauart), bauart)
    ...
    def test_maennlich_und_weiblich_sind_verschieden_lang(self):
        """Die Gegenprobe: Waeren beide gleich lang, koennte der Test
