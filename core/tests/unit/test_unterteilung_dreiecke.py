# -*- coding: utf-8 -*-
"""Dreiecke in der Catmull-Clark-Unterteilung (17.09.2026).
...
def _pyramide():
    punkte = np.array([[0, 0, 0], [1, 0, 0], [1, 0, 1], [0, 0, 1], [0.5, 1, 0.5]], dtype=np.float64)
    flaechen = np.array([[0, 1, 2, 3], [0, 4, 1, -1], [1, 4, 2, -1], [2, 4, 3, -1], [3, 4, 0, -1]])
...
class DreieckeInDerStufe(TestCase):
    ...
    def test_sabotage_einheitsgewicht_wuerde_auffallen(self):
        """Wer beim Dreieck weiter 0,25 je Ecke nimmt, summiert auf 0,75."""
        _, flaechen, _ = _pyramide()
        stufe = Unterteilungsstufe(flaechen, 5)
    ...
    def test_ganzer_unterteiler_mit_material_und_naehten(self):
        ...
        cc = CatmullClarkSubdivider(flaechen, face_materials=material, levels=1, uv_loops=uv_loops)
    ...
    def test_reines_vierecknetz_bleibt_bitgleich(self):
        """Kantenpunkt im Vierecknetz: 1/4 + 2/16 je Ende, 1/16 je andere Ecke."""
