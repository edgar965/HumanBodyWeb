# -*- coding: utf-8 -*-
"""Umareglerattrappe — ein winziges UMA3-Verzeichnis mit Regler-Dateien.
...
class Umareglerattrappe:
    ...
    def regler(cls, name, vorgabe, wirkungen):
        ...
        refs = "".join(
            ...
            % (100 + i, klasse, daten)
            for i, (klasse, daten) in enumerate(wirkungen)
        )
        return (
            cls.KOPF + "  m_Name: %s\n  m_EditorClassIdentifier: UMA_Core::UMA.DNA\n"
            "  defaultValue: %s\n  effects:\n%s  displayName: %s\n"
            "  references:\n    version: 2\n    RefIds:\n%s" % (name, vorgabe, rids, name, refs)
        )

    @classmethod
    def dateien(cls):
        skalieren = (
            cls.KURVE + "        minMapping: -1\n        maxMapping: 1\n"
            "        EffectName: Hips\n        BoneName: Hips\n"
            "        ScaleFactor: {x: 0.5, y: 0.5, z: 0.5}\n"
        )
        links = (
            cls.LEERE_KURVE + "        minMapping: -1\n        maxMapping: 1\n"
        ...
        posen = (
            cls.KOPF + "  m_Name: FemaleBody_BonePose\n"
        ...
        return {
            ...
            "DNA/Body/height.asset": cls.regler("height", 0.5, [("DNAEffect_BoneScale", skalieren)]),
            "DNA/Body/legSeparation.asset": cls.regler(
                "legSeparation",
                0.5,
                [("DNAEffect_BoneTranslate", links), ("DNAEffect_BoneTranslate", rechts)],
            ),
            "DNA/Face/noseCurve.asset": cls.regler(
                "noseCurve",
                0.5,
                [
                    ("DNAEffect_BoneRotate", drehen),
    ...
    def anlegen(cls, wurzel):
        """Das Verzeichnis unter `wurzel` (= UMA3) schreiben; gibt `wurzel` zurück."""
        wurzel = Path(wurzel)
