# -*- coding: utf-8 -*-
"""`Stoffhochziehen`: eine Hose hochziehen, bevor sie angelegt wird.
...
def _zylinder(x0, radius, z_von, z_bis, n=36, reihen=None):
    ...
    ringe = [
        np.column_stack([x0 + radius * np.cos(w), radius * np.sin(w), np.full(n, z)])
...
def _hose():
    """Zwei Stoffrohre um die Beine, Schritt 15 cm unter dem Koerperschritt,
...
class StoffhochziehenTest(SimpleTestCase):
    ...
    def test_der_bund_bleibt_wo_die_simulation_ihn_liess(self):
        """Bund 25 cm ueber dem Hosenschritt (rise 0,5): mit Maske bleibt der
        ...
        for hoehe, anteil in ((0.775, 0.5), (0.85, 0.2)):
            ...
            self.assertAlmostEqual(
                float(np.median(neu[dort, 2] - hose[dort, 2])), 0.15 * anteil, delta=0.004, msg=hoehe
            )
    ...
    def test_ohne_schritt_kein_absturz(self):
        """Ein Koerper ohne Beine (eine Saeule): unter dem tiefsten
