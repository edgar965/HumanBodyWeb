# -*- coding: utf-8 -*-
"""Fingergrenzen des SMPL-X-BVH: unmoegliche Fingerdrehungen werden geklemmt.
...
def _im_rahmen(wxyz, B):
    """(Beugung, Spreizung, Verdrehung) einer Drehung im Gelenkrahmen."""
...
class DerGelenkrahmen(unittest.TestCase):
    def test_beugung_fuehrt_zur_handflaeche(self):
        """+60 Grad um f bringt den Knochen nach -y — links, rechts, schraeger Kleinfinger."""
...
class DieFingergrenzen(unittest.TestCase):
    ...
    def test_grundgelenk_120_grad_wird_90_und_spreizt_gebeugt_nicht(self):
        ...
        feld = self._feld()
        feld[0, gelenk] = _wxyz(
            Rotation.from_rotvec(np.radians(120) * f) * Rotation.from_rotvec(np.radians(20) * n)
        )
...
class DasWerkzeug(unittest.TestCase):
    """`FingergrenzenBvh` auf einem kleinen SMPL-X-BVH mit einem Ausreisser."""
