# -*- coding: utf-8 -*-
u"""Einen Effektauftrag ueber die Seite starten, ohne dass ein Lauf beginnt.
...
class Effektstart:
    ...
    def senden(client, nutzlast):
        u"""`(antwort, start)` — `start` ist die Attrappe von `Effektlauf.starten`."""
        with mock.patch.object(Effektlauf, 'starten') as start:
            antwort = client.post(reverse('effekte_start'), data=json.dumps(nutzlast),
