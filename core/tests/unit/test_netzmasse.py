# -*- coding: utf-8 -*-
"""Netzmasse: Punkte, Vierecke, Dreiecke und Unterteilungen auf eine
...
class Ableitung(TestCase):
    ...
    def test_bezifferte_hoechste_stufe_schlaegt_die_rechnung(self):
        z = Netzmasse.ergaenzen({"stufen": 7, "hoechste": 69_000_000, "hoechste_einheit": "punkte"})
...
class Rangfolge(TestCase):
    def test_rang_folgt_der_dichte_und_ohne_zahl_gibt_es_keinen(self):
        zeilen = [
            {"name": "klein", "dreiecke": 100},
            {"name": "leer"},
            {"name": "gross", "vierecke": 100, "stufen": 1},
            {"name": "forschung", "dreiecke": 10**9, "ohne_rang": "nur Forschung"},
        ]
        aus = Netzmasse.rangfolge(zeilen)
        self.assertEqual(
            [(z["name"], z["rang"]) for z in aus],
            [("gross", 1), ("klein", 2), ("leer", None), ("forschung", None)],
        )

    def test_gleiche_dichte_behaelt_die_reihenfolge(self):
        aus = Netzmasse.rangfolge([{"name": "a", "dreiecke": 5}, {"name": "b", "dreiecke": 5}])
