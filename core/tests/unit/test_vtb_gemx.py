# -*- coding: utf-8 -*-
"""GEM-X im VideoToBVH-Baum — der Teil ohne Grafikkarte (12.09.2026).
...
class DerGemxBefehl(unittest.TestCase):
    ...
    def test_befehl_wie_das_demo(self):
        ...
        self.assertEqual(
            befehl[2:8], ["--video", "tanz.mp4", "--output_root", "aus", "--ckpt", Gemxlauf.CHECKPOINT]
        )
...
class DasSomaBvh(unittest.TestCase):
    """Vier Gelenke reichen: Hips -> Spine1 -> Head, Hips -> LeftLeg."""
    ...
    def _params(self, tpose, bilder=3):
        ...
        return {
            "names": np.asarray(self.NAMEN),
            "parents": np.asarray(self.ELTERN),
    ...
    def test_vorwaertskinematik_trifft_die_tpose_ohne_drehung(self):
        ...
        pos = Somabvh.positionen(
            np.zeros((2, 4, 3)), offsets, self.ELTERN, np.tile(tpose[0] * 100, (2, 1)), np
        )
