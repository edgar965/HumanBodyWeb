# -*- coding: utf-8 -*-
"""Hybridlauf mit GEM-SMPL als Rückgrat und GEM-X als Fingerquelle.
    ...
    def setUp(self):
        ...
        class Smplattrappe:
            ...
            def __init__(self, auftrag, video, ordner):
                ...
                self.welcher = 'haende' if auftrag.pipeline == Hybridhaende.QUELLE else 'koerper'
                pruefung.aufrufe[self.welcher] = auftrag
                pruefung.reihenfolge.append(self.welcher)

            def fahren(self):
                return getattr(pruefung, self.welcher)(self.auftrag, self.video, self.ordner)
...
class HybridFingerTest(HybridGemBasis):
    ...
    def test_die_finger_nehmen_kamera_und_geraet_des_koerpers(self):
        self.fahren('hybrid_gem', dict(self.PARAMS, body_device='cpu', static_cam=False))
