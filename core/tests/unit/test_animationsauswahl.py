...
class AnimationsBaum:
    ...
    def setUpClass(cls):
        ...
        (Path(cls.wurzel) / 'animations' / 'bvh' / 'KeineBvh' / 'liesmich.txt').write_text(
            'x', encoding='utf-8'
        )
...
class AnimationsauswahlTest(AnimationsBaum, TestCase):
    def test_kategorien_mit_anzahl(self):
        self.assertEqual(
            Animationsauswahl().kategorien(),
            [{'name': 'Aist', 'anzahl': 2}, {'name': 'Bandai 1', 'anzahl': 1}],
        )
    ...
    def test_eintraege_als_url(self):
        self.assertEqual(
            Animationsauswahl().eintraege('Aist'),
            [
    ...
    def test_eintraege_als_pfad(self):
        ...
        self.assertEqual(auswahl.eintraege('Bandai 1'), [{'value': 'Bandai 1/gehen', 'label': 'gehen'}])
    ...
    def test_unbekanntes_wertformat_faellt_auf_url_zurueck(self):
        self.assertEqual(Animationsauswahl('quatsch').wertformat, Animationsauswahl.ALS_URL)

    def test_zerlegen_kennt_beide_wertformate(self):
        self.assertEqual(Animationsauswahl.zerlegen('/api/character/bvh/Aist/tanz_01/'), ('Aist', 'tanz_01'))
    ...
    def test_fehlende_prueft_mehrere_werte(self):
        ...
        self.assertEqual(
            auswahl.fehlende(['/api/character/bvh/Aist/tanz_01/', '/api/character/bvh/Aist/weg/', '']),
    ...
    def test_seitenteil_liefert_beides(self):
        ...
        self.assertEqual([k['name'] for k in teil['anim_kategorien']], ['Aist', 'Bandai 1'])
...
class UmleitungGreiftTest(AnimationsBaum, TestCase):
    """Greift die Umleitung wirklich — oder lesen wir die echten Daten?
    ...
    def test_die_kategorien_kommen_aus_dem_testbaum(self):
        ...
        for echt in ('Mixamo', 'Walk', 'Bandai 2', 'MocapNET'):
            self.assertNotIn(
                echt, namen, '%s stammt aus den echten Daten — die Umleitung greift nicht.' % echt
            )

    def test_die_wurzel_liegt_im_testordner(self):
        """Die scharfe Probe: der gelesene Pfad selbst."""
        wurzel = Bvhverzeichnis().wurzel()
        self.assertIn('animauswahl_', wurzel, 'Gelesen wird %s — das ist nicht der Testbaum.' % wurzel)
