# -*- coding: utf-8 -*-
"""Das Schnittprogramm des Schuhs, gegen pygarment gebaut.
...
class SchuhschnittTest(SimpleTestCase):
    ...
    def setUpClass(cls):
        ...
        try:
            ...
            pfad = os.path.join(UPSTREAM, "assets", "design_params", "default.yaml")
            with open(pfad, "r", encoding="utf-8") as datei:
                cls.entwurf = yaml.safe_load(datei)["design"]
            Schuhentwurf.mischen(cls.entwurf)
            with open(os.path.join(UPSTREAM, "assets", "bodies", "mean_female.yaml"), "r") as datei:
    ...
    def test_die_sohle_liegt_unter_dem_fuss_und_das_blatt_darueber(self):
        ...
        self.assertAlmostEqual(sohle[1][2] - sohle[0][2], self.FUSS["foot_length"] * 1.04, delta=0.6)

    # ------------------------------------------------------ Aussenseiten

    def test_jedes_panel_zeigt_mit_der_aussenseite_nach_aussen(self):
        """Der Fehler Nr. 1 aus dem Modulkopf — hier für beide Schuhe."""
        ...
        for schuh, spiegel in ((g.links, 1.0), (g.rechts, -1.0)):
            for panel, richtung in schuh.aussenrichtungen():
                ...
                self.assertGreater(float(np.dot(panel.norm(), soll)), 0.0, panel.name)

    # --------------------------------------------------------------- Nähte

    def test_jede_naht_laeuft_in_beiden_panels_gleichgerichtet(self):
        """Der Fehler Nr. 2: Anfang an Anfang, nicht Anfang an Ende."""
        ...
        for schuh in (g.links, g.rechts):
            for regel in schuh.stitching_rules.rules:
                ...
                self.assertLessEqual(
                    gleich, gegen, "%s <-> %s" % (regel.int1.panel_names(), regel.int2.panel_names())
                )

    def test_die_nahtstuecke_passen_in_der_zahl(self):
        """Nach dem Unterteilen hat jede Naht auf beiden Seiten gleich
    ...
    def test_der_schaft_ist_nirgends_enger_als_das_bein(self):
        """Die Wade setzt früh an: Linear zwischen Knöchel und Wade war
        ...
        for schritt in range(0, 11):
            ...
            self.assertGreaterEqual(
                unten + t * (oben - unten) + 0.5, s.fuss.beinumfang(hoehe) * 1.2, "Höhe %.1f cm" % hoehe
            )

    def test_die_quartiere_schliessen_ueber_dem_rist(self):
        """Beim Stiefel ist der Einstieg des Blatts an die Ristkanten
        ...
        self.assertAlmostEqual(stiefel.ring_hinten, self.FUSS["ankle_height"], delta=0.5)
    ...
    def test_mit_absatz_ist_die_sohle_zweiteilig_und_die_ferse_oben(self):
        """Pumps mit 7 cm Absatz: die Vordersohle liegt flach am Boden, die
        ...
        self.assertAlmostEqual(vorn[1][1], -Halbschuh.SOHLE_UNTER_FUSS, delta=0.05)  # flach
    ...
    def test_die_sprengung_kippt_die_vordersohle_und_hebt_die_spitze(self):
        """`shoe.toe_spring`: der vordere Sohlenteil steigt zur Zehe hin,
    ...
    def test_ohne_fussmasse_wird_geschaetzt_und_gesagt(self):
        ...
        self.assertIn("geschätzt", g.hinweise[0])
        # Und die Schätzung liegt nahe an der Messung (Anteile der Höhe).
        self.assertAlmostEqual(g.links.fuss.foot_length, self.FUSS["foot_length"], delta=1.0)
