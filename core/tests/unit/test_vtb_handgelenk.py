# -*- coding: utf-8 -*-
u"""Das Handgelenk aus der Handquelle gilt nur, wo es anatomisch moeglich bleibt.
...
def _reihe(n, handgelenk_grad):
    u"""SMPLest-X-Reihe: Koerper in Ruhe, nur das linke Handgelenk gedreht."""
    reihe = Smplxreihe(30.0, np)
    koerper = np.zeros((21, 3))
    koerper[LINKS - 1] = np.radians(handgelenk_grad) * BEUGEACHSE
    for _ in range(n):
        reihe.dazu({'smplx_root_pose': np.zeros(3), 'smplx_body_pose': koerper.ravel(),
...
class DieGelenkgrenzen(unittest.TestCase):
    ...
    def test_ein_handgelenk_ueber_80_grad_wird_gekappt(self):
        ...
        self.assertAlmostEqual(np.degrees(np.arccos(np.clip(np.dot(gedreht, achse), -1, 1))),
                               80.0, places=1)
