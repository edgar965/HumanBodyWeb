# -*- coding: utf-8 -*-
"""`Bvhleser` — vor allem die Stetigkeitskorrektur ohne Schleife.
...
class Quaternionenbau:
    """Baut die Quaternionenfolgen fuer diese Faelle.
    ...
    def mit_schleife(quats):
        """Die Fassung von vor dem 31.08.2026, Zeichen für Zeichen."""
...
class StetigkeitTest(SimpleTestCase):
    """Die geschlossene Rechnung liefert exakt die Schleife."""
    ...
    def test_zufaellige_reihen(self):
        """200 Bilder, 12 Gelenke, zufällig gespiegelt."""
    ...
    def test_jedes_bild_gespiegelt(self):
        """Der Härtefall: abwechselnd q und -q."""
    ...
    def test_skalarprodukt_genau_null(self):
        """Bei dot == 0 wird NICHT gespiegelt — die Bedingung ist `< 0`.
...
class SpitzenTest(SimpleTestCase):
    """Ein einzelner Ausreißer wird durch die Nachbarn ersetzt."""
    ...
    def test_ein_ausreisser_wird_geglaettet(self):
        ...
        self.assertTrue(np.allclose(quats[2, 0], [0, 0, 0, 1], atol=1e-9), quats[2, 0])

    def test_eine_echte_bewegung_bleibt(self):
        """Wer sich stetig dreht, ist kein Ausreißer."""
