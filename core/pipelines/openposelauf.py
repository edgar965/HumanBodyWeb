...
class Openposelauf:
    ...
    def _starten(self):
        ...
        lauf = self.Prozess.starten(befehl, cwd=settings.OPENPOSE_ROOT, stdout_lesen=False)
    ...
    def _geschriebeneBilder(self, ersatz):
        try:
            return len([d for d in os.listdir(self.jsonordner) if d.endswith(self.JSON_ENDUNG)])
