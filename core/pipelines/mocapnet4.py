...
class V4Lauf(Pipelinelauf):
    ...
    def fahren(self):
        ...
        fortschritt = Erkennungsfortschritt(self.job, bilder, time.time(), anteil=self.ANTEIL)
        fortschritt.anfangsmeldung('v4_processing', 'MocapNET v4')
        prozess = PipelineProzess.starten(self._befehl(), cwd=settings.MOCAPNET_V4_ROOT)
    ...
    def _befehl(self):
        ...
        befehl = [
            ...
            str(settings.MOCAPNET_V4_SCRIPT),
            '--from',
            str(self.video_path),
            '--output',
            self.bvh,
            ...
            self.stoppmarke,
            '--hcd-iterations',
            str(p.get('hcd_iterations', s.v4_hcd_iterations)),
            '--hcd-epochs',
            str(p.get('hcd_epochs', s.v4_hcd_epochs)),
            '--hcd-lr',
            str(p.get('hcd_learning_rate', s.v4_hcd_learning_rate)),
            '--smooth-sampling',
            str(p.get('smoothing_sampling', s.v4_smoothing_sampling)),
            '--smooth-cutoff',
            str(p.get('smoothing_cutoff', s.v4_smoothing_cutoff)),
            '--mp-detection-conf',
            str(p.get('mp_detection', s.mp_min_detection_confidence)),
            '--mp-tracking-conf',
            str(p.get('mp_tracking', s.mp_min_tracking_confidence)),
            '--mp-model-complexity',
            str(s.mp_model_complexity),
            '--flipHorizontal',
        ]
    ...
    def _bauteile(self):
        return [
            schalter
            for schalter, schluessel, feld in self.BAUTEILE
            if self.params.get(schluessel, getattr(self.einstellungen, feld))
        ]
    ...
    def _ergebnis(self, prozess):
        ...
        if code != 0:
            ...
            raise RuntimeError(
                'MocapNET v4 failed (exit code %s):\n%s' % (code, self.fehlerausschnitt(fehler))
            )
    ...
    def _brauchbar(self):
        return os.path.exists(self.bvh) and os.path.getsize(self.bvh) > self.MINDESTGROESSE
