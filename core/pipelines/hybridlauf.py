...
class Hybridlauf(Pipelinelauf):
    ...
    RUECKGRAT = {'hybrid_gvhmr': 'gvhmr', 'hybrid_prompthmr': 'prompthmr', 'hybrid_gem': 'gem'}
    ...
    def __init__(self, job, video_path, output_dir):
        ...
        self.fehler: dict[str, str | None] = {'koerper': None, 'gesicht': None, 'haende': None}
    ...
    def fahren(self):
        ...
        self.job.progress_detail = (
            'Done (partial: ' + '; '.join(meldungen)[:200] + ')' if meldungen else 'Done'
        )
    ...
    def _koerperauftrag(self):
        ...
        params['device'] = self.params.get('body_device', self.einstellungen.smpl_device)
        return Teilauftrag(
            self.koerper_rueckgrat,
            params,
            self.job.name,
            '%s_body' % self.job.id,
    ...
    def _starten(self, koerper_ordner, gesicht_ordner):
        laeufe = [
            threading.Thread(target=self._koerper_dann_haende, daemon=True, args=(koerper_ordner,)),
            threading.Thread(
                target=self._lauf, daemon=True, args=('gesicht', V4Lauf, self.gesicht, gesicht_ordner)
            ),
        ]
        for lauf in laeufe:
            lauf.start()
    ...
    def _koerper_dann_haende(self, koerper_ordner):
        ...
        if self.haende:
            self._lauf('haende', Smpllauf, self.haende, self.output_dir / Hybridhaende.ORDNER)
    ...
    def _lauf(self, welcher, pipelineklasse, auftrag, ordner):
        ...
        """
        try:
            self.ergebnis[welcher] = pipelineklasse(auftrag, self.video_path, ordner).fahren()
    ...
    def _verfolgen(self, laeufe):
        ...
        while any(lauf.is_alive() for lauf in laeufe):
            ...
            for welcher, _, auftrag in teile:
                hoechststand[welcher] = max(hoechststand[welcher], auftrag.progress or 0)
            self._melden(
                min(hoechststand.values()),
                ' | '.join(
                    '%s: %s' % (name, auftrag.progress_detail or '%d%%' % hoechststand[welcher])
    ...
    def _pruefen(self):
        ...
        if self.fehler['koerper']:
            meldungen.append('Body (%s): %s' % (self.koerper_rueckgrat, self.fehler['koerper']))
    ...
    def _gesichtsausdruecke(self):
        ...
        skript = os.path.join(str(settings.TOOLS_ROOT), 'VideoToBVH', 'wrappers', '_run_smplest_x_video.py')
        try:
            self._melden(self.job.progress, 'Extracting face expressions (SMPLest-X batch)...')
            lauf = subprocess.run(
                [settings.PIPELINE_PYTHON, skript, str(self.video_path), ziel],
                capture_output=True,
                text=True,
                encoding='utf-8',
                errors='replace',
