# -*- coding: utf-8 -*-
"""Slamstatus — steht die Kamerabahn-Schätzung (DPVO, DROID-SLAM) bereit?
...
class Slamstatus:
    """Dateien, die DPVO und DROID-SLAM in der Pipeline-Umgebung brauchen."""
    ...
    PAKETE = (('dpvo', 'dpvo.py'), ('lietorch', 'groups.py'), ('torch_scatter', '__init__.py'))

    @classmethod
    def site_packages(cls, python_exe):
        """`.../python10/Scripts/python.exe` -> `.../python10/Lib/site-packages`."""
    ...
    def fehlend(cls, python_exe, gvhmr_root, prompthmr_root):
        """Was fehlt, als Liste von Pfaden — leer heisst: alles da."""
        sp = cls.site_packages(python_exe)
        fehlt = [
            os.path.join(sp, m + '*.pyd') for m in cls.MODULE if not glob.glob(os.path.join(sp, m + '*.pyd'))
        ]
        pfade = [os.path.join(sp, paket, datei) for paket, datei in cls.PAKETE]
        pfade.append(os.path.join(str(gvhmr_root), 'inputs', 'checkpoints', 'dpvo', 'dpvo.pth'))
        pfade.append(os.path.join(str(prompthmr_root), 'data', 'pretrain', 'droidcalib.pth'))
