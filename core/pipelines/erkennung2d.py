...
class Erkennung2d(Pipelinelauf):
    ...
    def mediapipe(self):
        ...
        pp = self._fahren(
            befehl, settings.MOCAPNET_ROOT, "mediapipe", Erkennung2d.STILLE_MEDIAPIPE_S, warten=600
        )
        ...
        csv_datei = os.path.join(str(csv_ausgabe) + "-mpdata", "2dJoints_mediapipe.csv")
    ...
    def neuer_erkenner(self):
        ...
        groesse = getattr(self.einstellungen, Erkennung2d.MODELLFELD.get(self.job.pipeline, ""), "l")
        befehl = [
            settings.PIPELINE_PYTHON,
        ...
        pp = self._fahren(
            befehl,
            settings.WRAPPERS_DIR.parent,
            "detecting_2d",
            Erkennung2d.STILLE_NEUE_S,
            warten=3600,
    ...
    def _fahren(self, befehl, arbeitsordner, status, stille, warten, name=""):
        ...
        """
        fortschritt = Erkennungsfortschritt(self.job, Videolaenge.bilder(self.video_path), time.time(), name)
