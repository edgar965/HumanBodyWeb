...
class Kleidungsregler:
    ...
    def __init__(
        ...
        anheben_mm=None,
        schritttiefe_mm=None,
    ):
        self.abstand = abstand
        self.steifigkeit = steifigkeit
        self.farbe = farbe
        self.verfahren = verfahren
        self.mindestabstand_mm = self.MINDESTABSTAND_MM if mindestabstand_mm is None else mindestabstand_mm
        self.schrittboden_mm = self.SCHRITTBODEN_MM if schrittboden_mm is None else schrittboden_mm
        self.anheben_mm = self.ANHEBEN_MM if anheben_mm is None else anheben_mm
        self.schritttiefe_mm = self.SCHRITTTIEFE_MM if schritttiefe_mm is None else schritttiefe_mm
