...
class Garmentabsatz:
    ...
    def ordner(cls, stueck):
        ...
        treffer = [
            os.path.join(wurzel, d)
            for d in os.listdir(wurzel)
            if muster.match(d) and os.path.isdir(os.path.join(wurzel, d))
        ]
        if not treffer:
            return None
    ...
    def lesen(cls, stueck):
        ...
        vermerk = Schuhentwurf.vermerk(os.path.join(ordner, "%s_specification.json" % name))
        antwort["name"] = name
