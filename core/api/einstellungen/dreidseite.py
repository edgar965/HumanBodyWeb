...
class DreiDEinstellungen(Einstellungsseite):
    ...
    def uebernehmen(self, s, post):
        for name, vorgabe, unten, oben, ganz in self.ZAHLEN:
            setattr(s, name, F.zahl(post, name, vorgabe, mini=unten, maxi=oben, ganz=ganz))
        ...
        s.default_model_result = F.text(post, "default_model_result", "femaleWithClothes")
        s.video_output_dir = F.text(post, "video_output_dir", str(Path(settings.MEDIA_ROOT) / "output"))
