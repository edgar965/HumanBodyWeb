# -*- coding: utf-8 -*-
u"""Bauplan eines dForce-Stuecks fuer den Stoffschwung im Browser.
...
class G9stoffapi:
    u"""Lesender Endpunkt auf Kanten, Unterteilungsmatrix und Kaefighaut."""
    ...
    def bauplan(request, kennung, nummer):
        ...
        if not getattr(folger, 'dynamisch', False):
            return JsonResponse({'fehler': u'Kein dForce-Stück'}, status=404)
        try:
        ...
        return JsonResponse({
            'kennung': kennung, 'nummer': int(nummer), 'stufen': stufen,
            'punkte': plan['punkte'], 'zeilen': plan['zeilen'],
