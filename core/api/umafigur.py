# -*- coding: utf-8 -*-
"""UMA-Figur aus dem Figurkatalog: Dateien, Beipackzettel, Form-Regler.
...
class Umafigur:
    ...
    def _pfad_oder_antwort(cls, name) -> tuple[str, JsonResponse | None]:
        """`(pfad, None)` — oder `('', JsonResponse)` mit 400/404.
    ...
    def bauer_stand(request):
        """Lebt der Unity-Bauer, startet er, oder ist er aus? — für die Anzeige."""
    ...
    def vorwaermen(request):
        """Den Bauer starten und einmal ins Leere bauen lassen (06.09.2026)."""
        return Umafigur._lauf(Umabauer.vorwaermen)

    @staticmethod
    def _lauf(start, *args):
        """Einen Auftrag an den Unity-Bauer geben; 202 mit dem Stand, sonst 400/503."""
        try:
