...
from humanbody_core.cloth import BUILDER_REGIONS, PRIMITIVE_TYPES, TEMPLATE_TYPES
...
class Kleidungsvorlagen:
    ...
    def bereiche(request):
        ...
        return JsonResponse(
            {
                'templates': [
                    {'key': k, 'label': v['label'], 'color': list(v['color'])}
                ...
                'builder_regions': [
                    {'key': k, 'label': k.replace('_', ' ').title(), 'color': list(v['color'])}
    ...
    def _ordner(cls, kategorie):
        ...
        pfad = settings.HUMANBODY_ASSETS_INSTANCE_DIR / kategorie / 'clothFromTemplate'
    ...
    def liste(request):
        ...
        for datei in sorted(Kleidungsvorlagen._ordner(kategorie).glob('*.json')):
            try:
                ...
                logger.warning(
                    'Kleidervorlage %s nicht lesbar — Dateiname als Bezeichnung', datei, exc_info=True
                )
    ...
    def sichern(request):
        ...
        if not kategorie:
            return JsonResponse({'error': 'Unknown template: %s' % vorlage}, status=400)
        ...
        return JsonResponse({'ok': True, 'filename': '%s.json' % sauber, 'category': kategorie})
    ...
    def vorlage(request, category, name):
        ...
        if not os.path.isfile(pfad):
            return HttpResponseNotFound('Preset not found: %s/%s' % (category, name))
