# -*- coding: utf-8 -*-
"""MakeHuman-Figur fuer die Szene: Basisnetz, Garderobe, Texturen.
...
from MakeHuman.kleidnetz import MhkleidFehler, Mhkleidnetz
...
class Mhfigur:
    """Lesende Endpunkte auf das MakeHuman-Basisnetz und seine Kleidung."""
    ...
    BILDARTEN = {'.png': 'image/png', '.jpg': 'image/jpeg', '.jpeg': 'image/jpeg'}
    ...
    def liste(request):
        if not Mhbasisnetz.vorhanden():
            return JsonResponse({'figuren': [], 'fehler': 'MakeHuman/base.obj fehlt'})
    ...
    def netz(request, name):
        """Das Netz — mit GET in der Vorgabeform, mit POST samt Reglern.
        ...
        """
        if name != 'basis':
            return JsonResponse({'fehler': 'Unbekanntes MakeHuman-Modell'}, status=404)
        if not Mhbasisnetz.vorhanden():
            return JsonResponse({'fehler': 'MakeHuman/base.obj fehlt'}, status=404)
        ...
        antwort = Netzantwort.aus(netz['punkte'], faces=netz['dreiecke'], normals=netz['normalen'])
        antwort.update(
            {
                'name': name,
                'teile': list(teile),
                'glatt': glatt,
                'verdeckt': list(getragen),
                'hoehe': netz['hoehe'],
    ...
    def _skelett(formung):
        """`default.mhskel`, gerechnet auf dieser Reglerstellung — oder `None`.
    ...
    @staticmethod
    def _rumpf(request):
        """Der JSON-Rumpf eines POST — bei GET ein leeres Woerterbuch."""
    ...
    def _teile(cls, request, rumpf):
        """`teile=koerper,helfer` — Unbekanntes faellt weg, Leeres gilt nicht."""
        aus_rumpf = rumpf.get('teile')
        gewuenscht = (
            aus_rumpf
            if isinstance(aus_rumpf, list)
            else [t.strip() for t in request.GET.get('teile', '').split(',')]
        )
    ...
    def _getragen(request, rumpf):
        """Die Stuecke, deren Haut ausgeblendet wird.
        ...
        roh = (
            aus_rumpf
            if isinstance(aus_rumpf, list)
            else [k.strip() for k in request.GET.get('verdeckt', '').split(',')]
        )
    ...
    @require_GET
    def regler(request):
        """MakeHumans Modellierregler: Seiten, Abschnitte, Grenzen, Vorgaben."""
            ...
            return JsonResponse(
                {
                    ...
                    'ziele_bereit': False,
                    'fehler': 'MakeHuman-Upstream fehlt — siehe MakeHuman/HERKUNFT.md',
                }
            )
        return JsonResponse(
            {
    ...
    def garderobe(request):
        ...
        for stueck in stuecke:
            eintrag = {'schluessel': stueck['kategorie'], 'name': stueck['kategoriename']}
            if eintrag not in kategorien:
                kategorien.append(eintrag)
        return JsonResponse({'stuecke': stuecke, 'kategorien': kategorien, 'anzahl': len(stuecke)})

    @staticmethod
    @csrf_exempt
    @require_http_methods(['GET', 'POST'])
    def kleidnetz(request, kategorie, stueck):
        """Ein Kleidungsstueck — mit POST auf dem GEFORMTEN Traeger.
        ...
        try:
            ...
            logger.warning('MakeHuman-Kleid %s nicht ladbar: %s', kennung, fehler)
            return JsonResponse({'fehler': str(fehler)}, status=500)
    ...
    def textur(request, kategorie, stueck, datei):
        """Eine Texturdatei aus dem Ordner des Kleidungsstuecks."""
