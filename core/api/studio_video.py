...
class Theatrevideo:
    ...
    def umwandeln(request):
        ...
        try:
            ...
            Videokodierer.ausfuehren(Videokodierer.umwandeln(webm, mp4), zeitgrenze=Theatrevideo.UMWANDLUNG_S)
            antwort = FileResponse(
                open(mp4, 'rb'), content_type='video/mp4', as_attachment=True, filename='theatre-export.mp4'
            )
    ...
    def aufnehmen(request):
        ...
        try:
            ...
            return Theatrevideo._ausliefern(daten.get('format', 'mp4'), daten, bilder, arbeitsordner)
        except (RenderFehler, VideoFehler) as fehler:
            return Theatrevideo._abbrechen(
                arbeitsordner, fehler, 'theatre_render_video: RenderFehler/VideoFehler'
            )
        except Exception as fehler:  # noqa: BLE001
            return Theatrevideo._abbrechen(arbeitsordner, fehler, 'Serverseitige Aufnahme fehlgeschlagen')

    @staticmethod
    def _aufnahme(request, daten, bilder):
        render = BildfolgenRender(
            bilder,
        ...
        render.aufnehmen(
            render.vollstaendige_url(daten['scene_url'], request.META.get('SERVER_PORT', 8081)),
            start=daten.get('start_time', 0),
            ende=daten.get('end_time', 0),
            ausschnitt=render.ausschnitt(
                daten.get('crop_x', 0), daten.get('crop_y', 0), daten.get('crop_w', 0), daten.get('crop_h', 0)
            ),
        )
    ...
    def _ausliefern(format_, daten, bilder, arbeitsordner):
        if format_ == 'png':
            ...
            return Theatrevideo._datei_und_aufraeumen(
                ziel + '.zip', 'application/zip', 'theatre_frames.zip', arbeitsordner
            )
        endung = Videokodierer.endung(format_)
        ausgabe = os.path.join(arbeitsordner, 'output.' + endung)
        Videokodierer.ausfuehren(
            Videokodierer.aus_bildfolge(
                bilder,
                ausgabe,
                daten.get('fps', Theatrevideo.VORGABE_FPS),
                daten.get('crf', Theatrevideo.VORGABE_CRF),
                format_,
            )
        )
        return Theatrevideo._datei_und_aufraeumen(
    ...
    def bilder_kodieren(request):
        ...
        try:
            for platz, hochgeladen in enumerate(bilder):
                Hochgeladen.ablegen(os.path.join(ordner, '%06d.png' % platz), hochgeladen)
            return Theatrevideo._kodieren(request.POST, ordner, arbeitsordner, zielpfad, len(bilder))
        except VideoFehler as fehler:
            return Theatrevideo._abbrechen(arbeitsordner, fehler, 'theatre_encode_frames: VideoFehler')
        except Exception as fehler:  # noqa: BLE001
            return Theatrevideo._abbrechen(arbeitsordner, fehler, 'Kodieren der Bildfolge fehlgeschlagen')
    ...
    def _zielpfad(request):
        ...
        try:
            ...
            return None, JsonResponse({'error': 'save_path abgelehnt: %s' % fehler}, status=403)
    ...
    def _kodieren(werte, ordner, arbeitsordner, zielpfad, anzahl):
        ...
        Videokodierer.ausfuehren(
            Videokodierer.aus_bildfolge(
                ...
                ausgabe,
                fps,
                int(werte.get('crf', Theatrevideo.VORGABE_CRF)),
                format_,
    ...
    def _datei_und_aufraeumen(pfad, inhaltstyp, dateiname, arbeitsordner):
        ...
        antwort = FileResponse(
            open(pfad, 'rb'), content_type=inhaltstyp, as_attachment=True, filename=dateiname
        )
        antwort._resource_closers.append(lambda: shutil.rmtree(arbeitsordner, ignore_errors=True))
        return antwort
