...
class CharmorphBestand:
    ...
    @staticmethod
    def wurzel():
        return os.path.join(str(settings.TOOLS_ROOT), 'tools', 'CharMorphPlugin', 'data')

    @classmethod
    def figurordner(cls, unterordner):
        """`…/data/characters/mb_female/<unterordner>`."""
        return os.path.join(cls.wurzel(), 'characters', cls.FIGUR, unterordner)
    ...
    def _vorgabe(ordner, dateiname):
        name = dateiname.replace('.json', '').replace('type_', '').replace('specialtype_', 'special_')
        try:
    ...
    def frisuren(request):
        ...
        return JsonResponse({'hairstyles': frisuren, 'colors': CharmorphBestand._haarfarben()})
        ...
        try:
            ...
            for name, werte in (roh or {}).items():
                if isinstance(werte, dict):
                    farben[name] = {
                        'viewport_color': werte.get('viewport_color', CharmorphBestand.FARBE_VORGABE),
                        'melanin': werte.get('melanin', CharmorphBestand.MELANIN_VORGABE),
                    }
        except Exception:
            logger.debug('optionaler Schritt fehlgeschlagen', exc_info=True)
        return farben
