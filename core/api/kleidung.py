...
class Kleidung:
    ...
    KATEGORIEN = frozenset(
        {'Tops', 'Bottoms', 'Skirts', 'Full', 'Underwear', 'Shoes', 'Accessories', 'Other'}
    )
    ...
    def garderobe(request):
        ...
        if os.path.isfile(manifest):
            with open(manifest, encoding='utf-8') as datei:
    ...
    def _aus_verzeichnis(glb_ordner):
        ...
        for kategorie in sorted(os.listdir(wurzel)):
            ...
            if not (os.path.isdir(ordner) and kategorie in Kleidung.KATEGORIEN):
                continue
                ...
                stuecke.append(
                    {
                        ...
                        'has_glb': os.path.isfile(os.path.join(glb_ordner, '%s.glb' % name)),
                    }
                )
        return stuecke
    ...
    def stoff(request):
        ...
        if art == 'builder':
            ...
            if netz.faces is not None and netz.faces.ndim == 2:
                flaechen = netz.faces
        try:
            ergebnis = generate_cloth(
                koerper.vertices, faces=flaechen, **Kleidung._bauwerte(request.GET, art)
            )
    ...
    def _bauwerte(werte, art):
        return {
            ...
            'tightness': (float(werte['tightness']) if 'tightness' in werte else None),
    ...
    def anpassen(request):
        ...
        if vorlage is None or vorlage.vertices is None:
            return JsonResponse({'error': 'Garment not found: %s' % kennung}, status=404)
        koerper = Charakterdaten.koerper_aus(request.GET)
        if koerper.vertices is None:
