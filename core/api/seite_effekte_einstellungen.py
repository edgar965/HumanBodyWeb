# -*- coding: utf-8 -*-
"""Einstellungsseite fuer Effekte — Pipeline, Vorgabemodell, Animation, Video, Wind.
...
class EffekteEinstellungenSeite(View):
    ...
    def get(self, request):
        s = AppSettings.load()
        anim_teil = Animationsauswahl(Animationsauswahl.ALS_URL).seitenteil([s.effekte_default_animation])
        return render(
            request,
            self.VORLAGE,
            {
    ...
    def _zahl(roh, typ, name, bisher):
        """Zahl im Rahmen des Registers; Unlesbares laesst den Wert stehen."""
        try:
            wert = typ(float(roh))
