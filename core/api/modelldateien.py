...
class Modelldateien:
    ...
    def _eintrag(pfad, dateiname):
        ...
        name = dateiname[: -len(Modelldateien.SZENE)] if szene else dateiname[:-5]
        ...
        try:
            ...
            logger.warning("%s nicht lesbar — Dateiname als Bezeichnung", pfad, exc_info=True)
            eintrag["label"] = name
        return eintrag
    ...
    def modellliste(request):
        ...
        return JsonResponse(
            {"presets": Modellkatalog.gespeicherte(), "koerpertypen": Modellkatalog.koerpertypen()}
        )
    ...
    def modell(request, name):
        ...
        pfad = Modellpfad.geprueft(Modelldateien._modellordner(), name, ".json")
        if pfad is None:
    ...
    def frisuren(request):
        ...
        return JsonResponse(
            {
                "hairstyles": stile,
                "colors": {name: wert["viewport"] for name, wert in Modelldateien.HAARFARBEN.items()},
            }
        )
    ...
    def frisur_glb(request, name):
        ...
        pfad = Modellpfad.geprueft(Modelldateien._frisurordner(), name, ".glb")
        if pfad is None:
            return JsonResponse({"error": "Invalid name"}, status=400)
        if not os.path.isfile(pfad):
