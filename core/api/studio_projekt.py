...
class Studioprojekte:
    ...
    def _szeneneintrag(ordner, dateiname):
        ...
        try:
            ...
            log.warning('Szene %s nicht lesbar — ohne Figurenzahl gelistet', pfad, exc_info=True)
            return {'name': name, 'label': name, 'character_count': 0}
        return {
            'name': name,
            'label': daten.get('name', name),
    ...
    def szene(request, name):
        ...
        pfad = Modellpfad.geprueft(Studioprojekte._modellordner(), name, Studioprojekte.SZENE)
        if pfad is None:
    ...
    def szene_sichern(request):
        ...
        ordner = Studioprojekte._modellordner()
        pfad = Modellpfad.geprueft(ordner, sauber, Studioprojekte.SZENE) if sauber else None
        if pfad is None:
            return JsonResponse({'error': 'Invalid name'}, status=400)
        ...
        return JsonResponse({'ok': True, 'filename': '%s%s' % (sauber, Studioprojekte.SZENE)})
    ...
    def projekt_laden(request):
        ...
        """
        try:
            pfad = SafePath.fuer_studio_projekte().pruefe(request.GET.get('path'))
        except PfadAbgelehnt as fehler:
            return JsonResponse({'error': str(fehler)}, status=403)
        if not pfad.is_file():
