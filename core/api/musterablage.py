...
class Musterablage:
    ...
    def ordner(self):
        pfad = os.path.join(str(settings.HUMANBODY_GARMENT_LIBRARY_DIR), self.kategorie, self.name)
    ...
    def ablegen(self, ergebnis):
        ...
        ordner = self.ordner()
        Objdatei(
            ergebnis['vertices'], ergebnis['faces'], kopfzeile='Pattern Editor export: %s' % self.name
        ...
        logger.info(
            'Saved pattern garment to library: %s (%d verts, %d tris)',
            kennung,
            len(ergebnis['vertices']),
            len(ergebnis['faces']),
        )
        return kennung
    ...
    def beschreibung(self):
        ...
        return {
            ...
            'default_params': {'offset': self.VORGABE_ABSTAND, 'stiffness': self.VORGABE_STEIFE},
