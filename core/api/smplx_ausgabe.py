...
class SmplxAusgabe:
    ...
    NETZFELDER = ('vertices', 'faces', 'joints', 'skin_indices', 'skin_weights')
    #: Felder, die nur mitgehen, wenn das Modell UV-Daten liefert.
    UV_FELDER = ('uv_vertices', 'uv_coords', 'uv_faces', 'uv_skin_indices', 'uv_skin_weights')
    ...
    def netz(request):
        ...
        ergebnis = generate_mesh(
            rumpf.get('betas', [0.0] * SmplxAusgabe.BETAS), rumpf.get('gender', 'neutral')
        )
        if ergebnis is None:
    ...
    def textur(request, job_id):
        ...
        try:
            vertices, faces, _netz = SmplxNetz.erzeugen(
                daten.get('betas', [0.0] * SmplxAusgabe.BETAS), daten.get('gender', 'neutral')
            )
        ...
        return SmplxAusgabe._backen(cv2, request, job, daten, bild, vertices, faces)
    ...
    def _backen(cv2, request, job, daten, foto, vertices, faces):
        ...
        try:
            textur = backen.backen(
                request.GET.get('backend', 'orthographic'), vertices, faces, region, hintergrund
            )
        ...
        try:
            ...
            logger.error('Texturpfad fuer %s nicht speicherbar', job.id, exc_info=True)
