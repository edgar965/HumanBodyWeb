# -*- coding: utf-8 -*-
u"""Modellkatalog — was der Dialog „Charakter hinzufügen" je Reiter anbietet.
...
class Modellkatalog:
    u"""Körpertypen, gespeicherte Modelle und ihre `quelle`."""
    ...
    @staticmethod
    def koerpertypen():
        u"""Die Standardfiguren von HumanBody, wie MB-Lab sie nennt."""
        return [{'name': name, 'anzeige': name.replace('_', ' '),
                 'geschlecht': ('weiblich' if name.startswith('Female')
                                else 'männlich')}
                for name in MorphData.BODY_TYPES]

    @classmethod
    def koerpertyp(cls, name):
        u"""Die Modellvorgabe eines Körpertyps (ohne Morphs) — oder None."""
        if name not in MorphData.BODY_TYPES:
            return None
        return {'name': name, 'body_type': name, 'morphs': {}, 'meta': {},
                'cloth': [], 'hair_style': None, 'garments': [],
                'mh_proxy': [], 'garmentcode': [], 'koerpertyp': True}

    # ---------------------------------------------------------- Gespeichert

    @classmethod
    def gespeicherte(cls):
        u"""`[{name, label, quelle}]` aller Modelldateien, nach Namen."""
        ordner = cls.ordner()
        aus = []
        ...
        for dateiname in sorted(os.listdir(ordner)):
            if (not dateiname.endswith('.json')
                    or dateiname.endswith(cls.SZENE)):
                continue
            name = dateiname[:-5]
            daten = cls._lesen(os.path.join(ordner, dateiname))
            aus.append({'name': name, 'label': name,
                        'quelle': cls.quelle(daten)})
        return aus

    @classmethod
    def gespeichert(cls, name, quelle):
        u"""Die Figurdaten (`figur`) eines gespeicherten Modells DIESER
    ...
    def quelle(cls, daten):
        u"""Die Figurart einer Modelldatei; ohne Angabe HumanBody."""
