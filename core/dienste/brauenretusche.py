# -*- coding: utf-8 -*-
"""Brauenretusche — die gemalten Augenbrauen aus einer MB-Lab-Hauttextur nehmen.
...
class Brauenretusche:
    ...
    def kern(cls, uvs, form, radius):
        """Bool-Bild (H, W): Umkreis `radius` um die UV-Pixel der Punkte."""
    ...
    def maske(cls, bild, zone, kern):
        """Die gemalte Braue: das Dunkle der Zone, das die Brauenpunkte berührt
        ...
        am_bogen[0] = False
        return ndimage.binary_dilation(bogen | am_bogen[marken], iterations=cls.AUSDEHNUNG_PX)

    @classmethod
    def fuellen(cls, bild, maske):
        """Maskenpixel aus der Umgebung füllen — Front vom Rand nach innen,
        ...
        while offen.any():
            ...
            offen &= ~rand
        weich = np.stack(
            [ndimage.gaussian_filter(aus[..., k], cls.GLAETTUNG) for k in range(aus.shape[-1])], axis=-1
        )
    ...
    @classmethod
    def fuer(cls, quelle):
        """Die retuschierte Fassung einer Albedo — mit den Zutaten des Projekts."""
        einheiten, uvs = cls.zutaten()
        return cls.ohne_brauen(quelle, einheiten, uvs)

    @classmethod
    def brauenmaske(cls, quelle):
        """`(maske, zone)` der gemalten Braue einer Albedo — für `Brauenbogen`."""
    ...
    def ohne_brauen(cls, quelle, einheiten, uvs):
        """Pfad der retuschierten Fassung — aus der Ablage oder frisch."""
        from PIL import Image
