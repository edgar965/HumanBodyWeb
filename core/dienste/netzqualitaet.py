# -*- coding: utf-8 -*-
u"""Netzqualität — wie fein die HumanBody-Figur gerechnet wird.
...
class Netzqualitaet:
    ...
    @classmethod
    def werte(cls):
        u"""`{unterteilung_browser, unterteilung_film, haut_verschiebung}`.
    ...
    def _lesen(cls):
        ...
        try:
            ...
            logger.warning('Netzqualitaet nicht lesbar (%s) — Vorgaben von '
                           'MB-Lab', fehler)
            return None

    @classmethod
    def _aus_db(cls, eigener_faden=False):
        u"""Nur der Hilfsfaden schliesst seine Verbindung — im Hauptfaden
    ...
    def aus_einstellungen(cls, s):
        u"""Die drei Werte aus einem `AppSettings`-Objekt, begrenzt."""
        aus = {}
    ...
    @classmethod
    def merken(cls):
        u"""Den Speicher füllen — im WebSocket über `database_sync_to_async`."""
        return cls.werte()
    ...
    def stufen_browser(cls):
        u"""Die Stufe im Browser — was der Browser mit dem Keks `netzstufen`
    ...
    def uebernehmen(cls, s, post):
        u"""Formularwerte auf das Einstellungsobjekt (Seite Modell)."""
        from ..api.einstellungen.formularwert import Formularwert as F
        for name, vorgabe, mini, maxi in (cls.STUFEN_BROWSER, cls.STUFEN_FILM):
            setattr(s, name, F.zahl(post, name, vorgabe, mini=mini, maxi=maxi,
                                    ganz=True))
