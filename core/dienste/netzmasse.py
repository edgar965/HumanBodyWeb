# -*- coding: utf-8 -*-
u"""Netzmasse — Punkte, Vierecke und Dreiecke auf EINE Vergleichsgröße bringen.
...
class Netzmasse:
    ...
    @staticmethod
    def zahl(n):
        u"""1234567 → „1.234.567" (deutsche Tausenderpunkte)."""
        return '{:,}'.format(int(n)).replace(',', '.')

    @classmethod
    def basis_dreiecke(cls, z):
        u"""Dreiecke des Basisnetzes: gegeben, sonst aus Vierecken, sonst aus
    ...
    def hoechst_dreiecke(cls, z):
        u"""Dreiecke der höchsten Stufe: vom Hersteller beziffert, sonst
    ...
    @classmethod
    def ergaenzen(cls, z):
        u"""Die Zeile um Texte und Sortierschlüssel ergänzt (neue Kopie)."""
        ...
        return dict(
            z,
            stufen=z.get('stufen') or 0,
            basis_punkte_text=cls.text(
                punkte if punkte is not None else vierecke, punkte is None),
            basis_vierecke_text=cls.text(
                vierecke if vierecke is not None else
                (punkte if z.get('dreiecke') is None else None),
                vierecke is None),
            basis_dreiecke=basis, basis_dreiecke_text=cls.text(basis, basis_ab),
            hoechst_dreiecke=hoechst,
            hoechst_text=cls.text(hoechst, hoechst_ab),
            stufen_text=z.get('stufen_text') or (
                '%d Unterteilung%s' % (z['stufen'], '' if z['stufen'] == 1 else 'en')
                if z.get('stufen') else 'keine'),
        )

    @classmethod
    def rangfolge(cls, zeilen):
        u"""Alle Zeilen ergänzt, absteigend nach `hoechst_dreiecke`; Rang 1..n
        ...
        mit = [z for z in fertig
               if z['hoechst_dreiecke'] is not None and not z.get('ohne_rang')]
