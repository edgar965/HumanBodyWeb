# -*- coding: utf-8 -*-
u"""Mimiksmplx — `static/mimik/smplx_basis.json` aus MB-Lab und dem SMPL-X-Modell.
...
class Mimiksmplx:
    ...
    def uebertrag(cls, einheiten, daten):
        u"""Der fertige `Smplxmimik` — `daten` ist der HumanBody-Datenordner."""
        ...
        quelle = Kopflandmarken(drei, haut, skelett, einheiten,
                                np.load(str(daten / 'faces.npy')))
    ...
    def schreiben(cls, einheiten, daten, ziel):
        u"""Die Datei anlegen; Rueckgabe: Bericht des Uebertrags plus Zahlen."""
        ...
        bericht['punkte'] = max((len(r['i']) for e in felder.values()
                                 for r in e.values()), default=0)
        return bericht
