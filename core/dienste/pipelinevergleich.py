# -*- coding: utf-8 -*-
"""Pipelinevergleich — alle Video-nach-BVH-Pipelines mit Messwerten und Rang.
...
class Pipelinevergleich:
    """Alle Pipelines, nach Rang sortiert, mit Anzeigenamen aus dem Modell."""
    ...
    SPALTEN = (
        (
            'Art',
            '2D + Lifter: ein 2D-Erkenner liefert Bildpunkte, MocapNET v2.1 hebt sie '
        ...
        (
            'Dauer (s)',
            'Wanduhrzeit des Auftrags vom Anlegen bis „complete" oder „failed", '
            'auf diesem Rechner (RTX PRO 4500), Video 298 Bilder.',
        ),
        (
            'Δ ViTPose (px)',
            'Mittlerer Pixelabstand des projizierten BVH-Skeletts zu den '
        ...
        (
            'Ruhe Wurzel',
            'Mittlere Beschleunigung der Wurzel (Hüfte) in cm je Bild², aus der '
            'BVH-Datei. Hohe Werte sind Zittern der Kamerabahn, nicht Bewegung.',
        ),
        (
            'Ruhe Pose',
            'Dasselbe für alle Gelenke relativ zur Hüfte, cm je Bild² — das Zittern der Pose selbst.',
        ),
        (
            'Boden (cm)',
            'Mittlere Höhe des tiefsten Gelenks. 0 heißt: die Füße stehen auf y = 0. '
            'Stark negativ heißt: die Figur steht im Kameraraum, nicht auf einem Boden.',
        ),
        (
            'Zustand',
            'läuft: BVH fertig. teilweise: nur ein Teil (etwa nur das Gesicht). '
    ...
    @classmethod
    def alle(cls):
        """Alle Eintraege, mit `name` aus dem Modell und `zustand_text`."""
    ...
    @classmethod
    def rangfolge(cls):
        """Nach Rang; wer keinen hat (kein Ergebnis), steht am Ende.
    ...
    def dreid_rangfolge(cls):
        """Die 3D- und Hybrid-Pipelines mit Rang, in Rangfolge — der
        ...
        return [e for e in cls.rangfolge() if e['rang'] is not None and e['art'] in cls.ARTEN_3D]
    ...
    @classmethod
    def schluessel(cls):
        """Jede Pipeline einmal, Varianten nicht doppelt."""
        return list(dict.fromkeys(e['schluessel'] for e in cls.alle()))

    @classmethod
    def grundeintraege(cls):
        """Die Eintraege ohne Variante — genau einer je Pipeline."""
