# -*- coding: utf-8 -*-
u"""Brauendecal — die Augenbraue als Zeichnung fürs Brauenfenster der Haut.
...
class Brauendecal:
    ...
    VORGABE = {'farbe': '#3a2a1e', 'hoehe_innen': 0.0, 'hoehe_aussen': 0.0,
               'woelbung': 0.0, 'lage': 0.0, 'laenge': 1.0, 'dicke': 1.0,
               'dichte': 1.0, 'haar_laenge': 1.0, 'deckkraft': 1.0}
    GRENZEN = {'hoehe_innen': (-15, 15), 'hoehe_aussen': (-15, 15),
               'woelbung': (-8, 10), 'lage': (-20, 20), 'laenge': (0.25, 3.0),
               'dicke': (0.25, 3.0), 'dichte': (0.25, 3.0),
               'haar_laenge': (0.25, 3.0), 'deckkraft': (0.0, 1.0)}
    ...
    @classmethod
    def regler(cls, roh):
        u"""Regler aus einer Abfrage (Zeichenketten erlaubt), geklemmt."""
    ...
    def kennung(cls, geschlecht, regler):
        text = json.dumps([cls.FASSUNG, Brauenbogen.FASSUNG, geschlecht,
                           {k: (round(v, 2) if isinstance(v, float) else v)
                            for k, v in sorted(regler.items())}])
        return hashlib.sha1(text.encode()).hexdigest()[:12]

    @classmethod
    def bild(cls, geschlecht, regler):
        u"""Pfad der PNG für diesen Reglerstand — aus der Ablage oder frisch."""
    ...
    def zeichnen(cls, bogen, regler):
        u"""Das Fensterbild: je Braue ein kleiner Ausschnitt in `S`-facher
        ...
        for nummer, seite in enumerate(('links', 'rechts')):
            if seite not in bogen:
                continue
            linie = np.array([[(u - u0) * px_je_uv, (v1 - v) * px_je_uv]
                              for u, v in bogen[seite]])
            striche = cls.haare(linie, px_je_mm, regler, farbe,
                                np.random.default_rng(1000 + nummer))
    ...
    def ausschnitt(cls, striche, farbe):
        u"""Eine Braue als kleines RGBA-Bild plus seine linke obere Ecke."""
        ...
        for segment in range(cls.SEGMENTE - 1, -1, -1):
            for punkte, breiten, rgba in striche:
                ...
                zeichner.line([tuple(a), tuple(b)], fill=rgba[segment],
    ...
    def haare(cls, linie, px_je_mm, regler, farbe, rng):
        u"""Die Striche einer Braue: `[(punkte (SEGMENTE+1, 2), breiten, rgba je
        Segment)]` — `linie` in Bildpixeln von innen nach außen."""
        laengen = np.concatenate([[0], np.cumsum(np.linalg.norm(np.diff(linie, axis=0),
                                                                axis=1))])
        gesamt = laengen[-1]
        ...
        def punkt(s):
            if s <= 1:
                return np.array([np.interp(s, t_linie, linie[:, 0]),
                                 np.interp(s, t_linie, linie[:, 1])])
            return linie[-1] + tangente(1.0) * (s - 1) * gesamt

        def tangente(s):
            i = min(len(linie) - 2,
                    max(0, int(np.searchsorted(t_linie, min(s, 1.0)) - 1)))
            d = linie[i + 1] - linie[i]
        ...
        for k in range(anzahl):
            ...
            punkte = ((1 - tt) ** 2 * wurzel + 2 * (1 - tt) * tt * steuer
                      + tt ** 2 * ende)
            breiten = np.interp(np.linspace(0, 1, cls.SEGMENTE), [0, 1],
                                cls.STRICH_MM) * px_je_mm
            hell = 1 + rng.normal(0, 0.10)
            rgb = tuple(int(min(255, max(0, c * hell))) for c in farbe)
            rgba = [rgb + (int(round(255 * regler['deckkraft'] * cls.AUSBLENDEN[i])),)
    ...
    def _drehen(richtung, normale, grad):
        u"""`richtung` um `grad` zur `normale` hin drehen (positiv = zur Normalen)."""
        a = math.radians(grad)
