# -*- coding: utf-8 -*-
"""Der Stand eines UMA-Baulaufs — aus Auftragsordner, Ergebnisdatei und Log.
...
class Umalaufstand:
    ...
    def merken(bauer, lauf):
        """Den Lauf neben dem Log ablegen: der Dev-Server lädt sich bei jeder
    ...
    def stand(cls, bauer, name):
        """`None`, wenn kein Lauf dieses Namens bekannt ist."""
        ...
        fertig = exit_code == 0 and os.path.isfile(datei) and os.path.getmtime(datei) >= lauf["start"] - 1
        return {
            "name": name,
            "rasse": lauf["rasse"],
            "laeuft": laeuft,
    ...
    def _lage(cls, bauer, lauf, ergebnis, wartet):
        """(läuft, exit, Meldung) — fertig, unterwegs, oder Unity ist weg."""
        ...
        return (False, -1, "Unity läuft nicht (mehr) — " + (cls.letzte_meldung(lauf["log"]) or "kein Log"))

    @staticmethod
    def ergebnis(pfad, start):
        """Das Ergebnis dieses Laufs — ein älteres gleichen Namens zählt nicht."""
    ...
    def letzte_meldung(log):
        """Die letzte Roomguest-Zeile aus Unitys Log — was der Exporter zuletzt
        sagte."""
        try:
