# -*- coding: utf-8 -*-
"""Eigenepipeline — der Abschnitt „Eigene Pipeline: SMPL-X als Rückgrat" der
...
class Eigenepipeline:
    ...
    IST = [
        {
            'teil': 'YOLO, ViTPose-H, HMR2-Backbone',
            'herkunft': 'drei andere Labs, eingefroren',
            'groesse': '0,1 + 2,4 + 2,6 GB',
            'tut': 'Person finden, 2D-Punkte, Bildmerkmale je Bild — die eigentliche Güte',
        },
        {
            'teil': 'GVHMR-Bewegungsmodell',
            'herkunft': 'ZJU (README: 2× RTX 4090, 420 Epochen)',
            'groesse': '156 MB, 2.253 Zeilen Modellcode',
            'tut': 'aus den Merkmalen SMPL-Parameter über die Zeit; GEM ist dieselbe Familie',
        },
        {
            'teil': 'Demo-Skripte, hydra, Renderer, Dateihandling',
            'herkunft': 'dieselben Unis',
            'groesse': '—',
            'tut': 'das, was nervt: Video dreimal laden, vergessene Dateien, Checkpoints auf Google Drive',
        },
        {
            'teil': 'Vorstufe, BVH-Schreiber, Hybrid, Retarget',
            'herkunft': 'wir (seit 11.09.2026)',
            'groesse': '—',
            'tut': 'bildweise, ein venv, Fehler mit Grund',
        },
    ]

    STUFEN = [
        {
            'name': 'Eigene Pipeline auf ihren Gewichten',
            'aufwand': 'Wochen — begonnen (unten)',
    ...
    PLAN = [
        (
            'Kanonisches Format `Smplxbahn`',
            'SMPL-X je Bild (Körperpose, Handposen, Kiefer, '
            'Ausdruck, Betas, Verschiebung, Kamera) als Klasse mit Datei `<lauf>_smplx.npz` — '
            'jede Quelle schreibt hinein, nichts anderes verlässt die Pipeline.',
            'gebaut',
        ),
        (
            'Quellen als Module',
            'Körper aus GEM (`Gemlauf`, ohne dessen BVH), Hände und Gesicht aus '
            'SMPLest-X (`_run_smplx_reihe.py`, der ganze SMPL-X-Satz je Bild als npz). Dieselbe '
            'Vorstufe für alle.',
            'gebaut',
        ),
        (
            'Mischung im Parameterraum',
            '`Smplxmischung`: Lücken gefüllt, Finger und Kiefer als '
            'Quaternionen geglättet, Ausdruck als Zahlen; Gegenprobe Körper gegen Körper '
            '(mittlerer Winkel GEM ↔ SMPLest-X).',
            'gebaut',
        ),
        (
            'EIN Retarget SMPL-X → DEF',
            'BVH mit 54 Gelenken (`Smplxbvh`: SMPL-Körper mit '
            ...
            'Ausdrucksdatei auch beim Einzel-Retarget.',
            'gebaut',
        ),
        (
            'Quellen wählbar',
            'Körper GEM-SMPL, GVHMR oder DuoMo (`Smplxkoerper`); Hände '
            'SMPLest-X oder GEM-X; Gesicht SMPLest-X oder keins; Handgelenk aus der Hand oder '
            'aus dem Körper — auf der Karte, im Auftrag, in `Smplbefehl`.',
            'gebaut',
        ),
        (
            'Handgelenk-Übergabe',
            'GEMs Handgelenk liegt im Median 35 (links) und 46 Grad '
        ...
        (
            'Fußkontakt',
            '`Bodenkontakt`: Boden = 5-%-Perzentil des tiefsten Fußgelenks, Stand = '
        ...
        (
            'Netz-Video',
            '`Smplxvideo`: das SMPL-X-Netz aus der Bahn (Körper grau, Hände blau, '
        ...
        (
            'Stufe 2: Training mit SMPL-X-Ausgabe',
            'AMASS und BEDLAM liegen in SMPL-X vor; ein '
            'Bewegungsmodell mit Körper + Händen in einem Zug (wie PromptHMR) ist auf der eigenen '
            'Karte trainierbar, die Merkmale kommen aus derselben Vorstufe.',
            'offen',
        ),
    ]

    GRENZE = (
        'Was damit nicht versprochen ist: bessere Hände als der beste Handschätzer, '
