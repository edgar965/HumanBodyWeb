# -*- coding: utf-8 -*-
"""Auftragstabelle — die Liste der Video-Aufträge als djangoBase-Tabelle.
...
class Auftragstabelle:
    """Baut die Tabellen-Struktur für `djangobase/_tabelle.html`."""
    ...
    def _kaestchen(job):
        return format_html(
            '<td class="kaestchen"><input type="checkbox" class="job-check" value="{}"></td>', job.id
        )
    ...
    def _pipeline(job):
        return format_html(
            '<td><span class="badge badge-{}">{}</span></td>', job.pipeline, job.get_pipeline_display()
        )

    def _status(self, job):
        """Sortiert wird nach dem Text — „Fehlgeschlagen" vor „Fertig" vor
        „Wartet", und die laufenden nach ihrem Fortschritt."""
        ende = self.ZUSTAENDE.get(job.status)
        if ende:
            klasse, symbol, text = ende
            inhalt = format_html('<span class="{}"><i class="fas {}"></i> {}</span>', klasse, symbol, text)
            return format_html('<td id="status-{}" data-sort="{}">{}</td>', job.id, text, inhalt)
        return format_html(
    ...
    @staticmethod
    def _laeuft(job):
        """Fortschritt in der Zeile — dieselbe Form schreibt
