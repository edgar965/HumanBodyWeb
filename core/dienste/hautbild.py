# -*- coding: utf-8 -*-
u"""Hautbild — die Hauttextur einer Figur als EIN Bild, mit gezeichneter Braue.
...
class Hautbild:
    u"""`Hautbild.bild(details, body_type)` → (H, W, 3) uint8, sRGB."""
    ...
    BRAUENFELDER = {
        'brauen': ('farbe', 1), 'brauen_staerke': ('haar_laenge', 1),
        'brauen_dicke': ('dicke', 1), 'brauen_dichte': ('dichte', 1),
        'brauen_bogen_laenge': ('laenge', 1), 'brauen_deckkraft': ('deckkraft', 1),
        'brauen_lage': ('lage', 1000), 'brauen_hoehe_innen': ('hoehe_innen', 1000),
    ...
    def regler(cls, details):
        u"""Die Zeichnerregler aus den Detailfeldern — wie `Brauenhaut.abfrage`."""
        roh = {}
        for feld, (name, faktor) in cls.BRAUENFELDER.items():
            wert = (details or {}).get(feld)
            if wert is None or wert == '':
                continue
            roh[name] = (round(float(wert) * faktor, 2)
                         if isinstance(wert, (int, float)) else wert)
    ...
    def hautfarbe(cls, details, body_type):
        u"""(r, g, b) 0..255: die gesetzte Hautfarbe, sonst die der Körperart."""
    ...
    def albedo(cls, details):
        u"""Pfad der MB-Lab-Albedo zum Detail `haut_textur`, oder None."""
    ...
    def toenen(cls, bild, farbe):
        u"""Albedo mal Hautfarbe, linear wie in Three.js (`color` × `map`)."""
    ...
    def grund(cls, details, body_type):
        u"""Albedo ohne gemalte Brauen (getönt, wenn eine Hautfarbe gesetzt
    ...
    def mit_brauen(cls, grund, geschlecht, regler):
        u"""Das Brauenfenster in den Grund gemischt — Bildzeile 0 ist oben,
        ...
        with Image.open(Brauendecal.bild(geschlecht, regler)) as decal:
            braue = np.asarray(decal.convert('RGBA').resize(
                (x1 - x0, y1 - y0), Image.LANCZOS), dtype=np.float32)
        alpha = braue[..., 3:4] / 255.0
        fenster = grund[y0:y1, x0:x1].astype(np.float32)
        aus = np.array(grund)
        aus[y0:y1, x0:x1] = np.clip(
            fenster * (1 - alpha) + braue[..., :3] * alpha, 0, 255).astype(np.uint8)
        return aus

    @classmethod
    def bild(cls, details, body_type):
        u"""Grund plus Braue für diese Figur."""
        return cls.mit_brauen(cls.grund(details, body_type),
                              cls.geschlecht(body_type), cls.regler(details))
