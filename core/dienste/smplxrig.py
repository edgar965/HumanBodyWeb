# -*- coding: utf-8 -*-
"""Smplxrig — Skelett und Hautgewichte des SMPL-X-Modells fuer einen Koerper.
...
class Smplxrig:
    """Lesender Zugang zum SMPL-X-Modell, je Geschlecht einmal geladen."""
    ...
    def koerper(cls, geschlecht):
        """Das formbare Modell (`Smplxkoerper`) — 75 MB, deshalb einmal."""
        from SMPL.xkoerper import Smplxkoerper
    ...
    @classmethod
    def vergessen(cls):
        """Den Speicher leeren — fuer Tests."""
    ...
    def kette(cls, name, punkte, geschlecht, armwinkel):
        """Das `Gelenkskelett` zu diesem Netz — oder `None`, wenn das Modell
    ...
    def skelett(cls, name, punkte, geschlecht, armwinkel):
        """`{name, knochen}` — die Antwort an den Browser, oder `None`."""
        from SMPL.xgelenke import Smplxgelenke
    ...
    def hautgewichte(cls, name, punkte, geschlecht, armwinkel):
        """`{knochen, index, gewicht}` (numpy) — oder `None`."""
    ...
    def referenz(cls, geschlecht, armwinkel):
        """Der SMPL-X-Durchschnittskoerper in der Haltung des fremden Netzes."""
