# -*- coding: utf-8 -*-
u"""Lippensync — Mundformen mit Zeiten aus einer Tondatei (Rhubarb Lip Sync).
...
class LipsyncFehler(RuntimeError):
    u"""Rhubarb fehlt, bricht ab oder die Tondatei ist nicht lesbar."""


class Lippensync:
    u"""Tondatei -> `{dauer, cues: [{start, end, form}]}`."""
    ...
    @staticmethod
    def flags():
        u"""Kein Konsolenfenster fuer ffmpeg und Rhubarb (Windows)."""
    ...
    def cues(cls, tonpfad, neu=False):
        u"""Die Mundformen einer Tondatei — aus der Ablage daneben, sonst
        gerechnet. `neu` erzwingt die Rechnung."""
        tonpfad = str(tonpfad)
        if not os.path.isfile(tonpfad):
            raise LipsyncFehler(u'Tondatei fehlt: %s' % os.path.basename(tonpfad))
        ablage = os.path.splitext(tonpfad)[0] + cls.ENDUNG
        if not neu and os.path.isfile(ablage):
            try:
                with open(ablage, 'r', encoding='utf-8') as datei:
    ...
    def rechnen(cls, tonpfad, ziel):
        u"""Rhubarb laufen lassen; das JSON geht ueber `-o` nach `ziel` (mit
        `-q` schweigt Rhubarb auch auf stdout, gemessen 18.09.2026)."""
        programm = cls.programm()
        if programm is None:
            raise LipsyncFehler(u'Rhubarb Lip Sync fehlt (RHUBARB_EXE)')
        wav = cls.als_wav(tonpfad)
        befehl = [programm, '-r', cls.ERKENNER, '-f', 'json', '-q', '-o', str(ziel),
                  wav]
        try:
            lauf = subprocess.run(befehl, capture_output=True, text=True,
                                  timeout=cls.ZEITGRENZE, encoding='utf-8',
                                  errors='replace', creationflags=cls.flags())
        except subprocess.TimeoutExpired:
            raise LipsyncFehler(u'Rhubarb: Zeitgrenze von %d s überschritten'
                                % cls.ZEITGRENZE)
        except OSError as fehler:
            raise LipsyncFehler(u'Rhubarb startet nicht: %s' % fehler)
        if lauf.returncode != 0 or not os.path.isfile(ziel):
            raise LipsyncFehler(u'Rhubarb bricht ab (%d): %s'
                                % (lauf.returncode, (lauf.stderr or '')[-400:]))
        with open(ziel, 'r', encoding='utf-8') as datei:
            return cls.lesen(datei.read())

    @classmethod
    def lesen(cls, text):
        u"""Rhubarbs JSON -> `{dauer, cues}`; unbekannte Formen werden X."""
        try:
            roh = json.loads(text)
        except ValueError:
            raise LipsyncFehler(u'Rhubarb: Antwort nicht lesbar')
        cues = []
        for c in roh.get('mouthCues') or []:
            form = str(c.get('value') or 'X').upper()
            cues.append({'start': round(float(c.get('start', 0.0)), 3),
    ...
    def als_wav(cls, tonpfad):
        u"""Die WAV-Fassung neben der Quelle (mono, 16 kHz) — einmal gewandelt."""
        ...
        befehl = [Videokodierer.programm(), '-y', '-loglevel', 'error', '-i', tonpfad,
                  '-ac', '1', '-ar', '16000', ziel]
        try:
            lauf = subprocess.run(befehl, capture_output=True, text=True,
                                  timeout=cls.ZEITGRENZE, encoding='utf-8',
            ...
            raise LipsyncFehler(u'ffmpeg wandelt nicht: %s' % fehler)
        if lauf.returncode != 0 or not os.path.isfile(ziel):
            raise LipsyncFehler(u'ffmpeg bricht ab: %s' % (lauf.stderr or '')[-400:])
        return ziel
    ...
    def pfad_aus_url(cls, url):
        u"""`/media/studio_audio/<name>` -> Pfad im Medienordner; alles andere None.
