# -*- coding: utf-8 -*-
"""Auftragsarbeiter — ein Auftrag läuft in einem eigenen Prozess, nicht im Server.
...
class Auftragsarbeiter:
    ...
    @staticmethod
    def flags():
        """Abgelöst vom Server: eigene Gruppe, kein Fenster, wenn möglich
    ...
    def _popen(cls, befehl, protokoll):
        """`CREATE_BREAKAWAY_FROM_JOB` scheitert, wenn das Job-Objekt des
    ...
    @classmethod
    def pid(cls, job_id):
        """Die eingetragene PID oder None."""
        try:
    ...
    @classmethod
    def lebt(cls, job_id):
        """Läuft der Arbeitsprozess dieses Auftrags noch?"""
        return Prozesspruefung.lebt(cls.pid(job_id))

    @classmethod
    def eintragen(cls, job_id):
        """Vom Arbeitsprozess selbst: die eigene PID hinterlegen (der Server
    ...
    def austragen(cls, job_id):
        """Am Ende des Arbeitsprozesses: die PID-Datei ist eine Notiz."""
        try:
