...
class Figurvideo:
    ...
    @classmethod
    def _skript(cls):
        return os.path.join(str(settings.BASE_DIR), 'TheatreJS', 'ModelPhysik', 'filmlauf.py')
    ...
    def starten(cls, daten, dateien=None):
        ...
        auftrag['stuecke'] = Figurvideostuecke.ablegen(
            dateien or {}, daten.get('stuecke') or [], daten.get('knochen') or [], ordner
        )
        ...
        with open(os.path.join(ordner, 'lauf.log'), 'w', encoding='utf-8') as protokoll:
            prozess = subprocess.Popen(
                ...
                cwd=os.path.dirname(cls._skript()),
                creationflags=flags,
            )
    ...
    def _auftrag(cls, daten, ordner):
        sekunden = min(float(daten.get('sekunden') or 5.0), cls.HOECHSTE_SEKUNDEN)
        physik = min(max(float(daten.get('physik_mm') or 0.0), 0.0), cls.HOECHSTE_PHYSIK_MM)
        return {
            'body_type': daten.get('body_type') or 'Female_Caucasian',
            'morphs': dict(daten.get('morphs') or {}),
            # Alter/Masse/Tonus: Koerperform und Hautverschiebung (17.09.2026).
            'meta': dict(daten.get('meta') or {}),
            ...
            'ablage': {
                ...
                'name': Figurvideoablage.dateiname(
                    daten.get('dateiname'), daten.get('figur'), daten.get('animation')
                ),
            },
        }
    ...
    def _bvh_pfad(url):
        ...
        pfad = os.path.abspath(os.path.join(wurzel, teile[-2], '%s.bvh' % teile[-1]))
        if not pfad.startswith(os.path.abspath(wurzel) + os.sep):
            raise ValueError('Animation ausserhalb der Bibliothek.')
        if not os.path.isfile(pfad):
            raise ValueError('Animation nicht gefunden: %s/%s' % (teile[-2], teile[-1]))
        return pfad
    ...
    def stand(cls, kennung):
        ...
        if not os.path.isfile(pfad):
            return {'phase': 'Wartet', 'anteil': 0.0, 'fertig': False, 'fehler': None}
        with open(pfad, encoding='utf-8') as datei:
            stand = json.load(datei)
        prozess = LaufendeProzesse.holen('figurvideo_' + kennung)
        if (
            prozess is not None
            and prozess.poll() is not None
            and not stand.get('fertig')
            and not stand.get('fehler')
        ):
            ...
            stand['fehler'] = 'Der Lauf endete ohne Ergebnis (Exit %s). Siehe lauf.log.' % prozess.returncode
        if stand.get('fertig'):
            stand['video_url'] = '%sfigurvideos/%s/video.mp4' % (settings.MEDIA_URL, kennung)
    ...
    def aus_bildfolge(cls, bilder, fps=24, physik_mm=0.0, ablage=None):
        ...
        try:
            for platz, datei in enumerate(bilder):
                Hochgeladen.ablegen(os.path.join(bilderordner, '%06d.png' % platz), datei)
            ziel = os.path.join(ordner, 'video.mp4')
            Videokodierer.ausfuehren(
                Videokodierer.aus_bildfolge(bilderordner, ziel, fps=int(fps), crf=20), zeitgrenze=300
            )
            ...
            if ablage:
                pfad = Figurvideoablage.ablegen(
                    ...
                    Figurvideoablage.dateiname(
                        ablage.get('name'), ablage.get('figur'), ablage.get('animation')
                    ),
                )
        finally:
        ...
        return (kennung, '%sfigurvideos/%s/video.mp4' % (settings.MEDIA_URL, kennung), pfad)
