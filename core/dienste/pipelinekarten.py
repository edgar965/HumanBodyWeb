# -*- coding: utf-8 -*-
"""Pipelinekarten — die Karten der 3D-Uploadseite in der Folge des Rangs.
...
class Pipelinekarten:
    ...
    @classmethod
    def reihenfolge(cls):
        """Kartennamen nach Rang; jede Karte einmal, ohne Rang am Ende."""
        dreid = set(cls.dreid())
        karten = []
    ...
    @classmethod
    def vorlagen(cls):
        """Die Vorlagennamen in Kartenfolge."""
        return [cls.VORLAGE % karte for karte in cls.reihenfolge()]

    @classmethod
    def rang_von(cls):
        """Wie viele Pipelines im Vergleich einen Rang haben — der Nenner
        im Abzeichen („Rang 2 von 10")."""
        return len(Pipelinevergleich.mit_rang())

    @classmethod
    def eintraege(cls):
        """Karte, Vorlage und Rang je Karte in Kartenfolge — so geht es in
