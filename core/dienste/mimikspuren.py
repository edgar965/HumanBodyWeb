# -*- coding: utf-8 -*-
"""Mimikspuren — die eingerechnete Mimik neben einer BVH, als Gesichtsspuren.
...
class Mimikspuren:
    ...
    def laden(cls, bvh_pfad):
        """`Bewegungsspuren` (Deltas je Gesichtsknochen) oder None ohne Datei."""
        pfad = cls.pfad(bvh_pfad)
    ...
    def mischen(cls, ergebnis, bvh_pfad):
        """Die Mimik neben `bvh_pfad` auf ein DEF-Ergebnis legen (wie das Video-Gesicht)."""
    ...
    def spuren(daten, basis):
        ...
        tracks = {k: Rotation.from_rotvec(v).as_quat().reshape(-1).tolist() for k, v in drehung.items()}
        return Bewegungsspuren(
            duration=anzahl / fps,
            times=[i / fps for i in range(anzahl)],
            tracks=tracks,
            frame_count=anzahl,
            mapped_bones=knochen,
        )

    @staticmethod
    def _knochen(einheiten, basis):
        """Alle Knochen, die eine der Einheiten bewegt — sortiert."""
        return sorted({k for e in einheiten if e in basis for r in basis[e].values() for k in r})

    @staticmethod
    def _einheit(drehung, eintrag, g):
        """Die Drehvektoren EINER Einheit (Gewichte `g` je Bild, positiv →
