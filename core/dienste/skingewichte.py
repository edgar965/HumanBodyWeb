...
class Skingewichte:
    ...
    def _nicht_def_entfernen(cls, daten, verzeichnis, geschlecht):
        ...
        if not os.path.isfile(skelettpfad):
            return
        with open(skelettpfad, encoding='utf-8') as f:
            skelett = json.load(f)
        ...
        daten['weights'] = [
            cls._paare_umnummerieren(paare, umnummerierung, weg) for paare in daten['weights']
        ]
        logger.info('Basisgewichte (%s): %d Nicht-DEF-Knochen entfernt', geschlecht, len(weg))
    ...
    def _paare_umnummerieren(paare, umnummerierung, weg):
        gefiltert = [(umnummerierung[bi], w) for bi, w in paare if bi not in weg and bi in umnummerierung]
    ...
    def arrays(cls, geschlecht='female'):
        ...
        logger.info(
            'Basisgewichte (%s) aufbereitet: %d Vertices, %d Einfluesse',
            geschlecht,
            len(indices),
            cls.EINFLUESSE,
        )
    ...
    def _propagieren(cls, geschlecht, unterteiler):
        ...
        if basis is not None and unterteiler is not None:
            logger.info(
                'Gewichte (%s) durch die Unterteilung reichen: %d Basis- -> %d Untervertices',
                geschlecht,
                basis['vertex_count'],
                unterteiler.sub_vertex_count,
            )
            return unterteiler.propagate_skin_weights(basis['weights'], basis['bone_names'])
    ...
    def _notfassung(cls, geschlecht):
        ...
        with open(pfad, encoding='utf-8') as f:
            daten = json.load(f)
        logger.error(
            'Rohe skin_weights.json fuer %s benutzt — die '
