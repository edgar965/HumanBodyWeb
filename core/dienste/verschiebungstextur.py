# -*- coding: utf-8 -*-
u"""Verschiebungstextur — MB-Labs Displacement-Textur aus Alter, Tonus, Masse.
...
class Verschiebungstextur:
    ...
    def datenbild(cls, geschlecht):
        u"""(H, W, 4) float32 in 0..1 — je Geschlecht einmal geladen."""
    ...
    def feld(cls, geschlecht, alter=0.0, tonus=0.0, masse=0.0):
        u"""Die Textur als (H, W) float32 — gemerkt je Geschlecht und Werten."""
        schluessel = (cls.geschlecht(geschlecht), cls.runden(alter),
                      cls.runden(tonus), cls.runden(masse))
        if schluessel not in cls._felder:
            if len(cls._felder) >= 8:
                cls._felder.clear()
            cls._felder[schluessel] = Hautverschiebung.textur(
                cls.datenbild(schluessel[0]), *schluessel[1:])
        return cls._felder[schluessel]

    @classmethod
    def dateiname(cls, geschlecht, alter, tonus, masse):
        return 'verschiebung_%s_a%+.2f_t%+.2f_m%+.2f.png' % (
            cls.geschlecht(geschlecht), cls.runden(alter), cls.runden(tonus),
            cls.runden(masse))

    @classmethod
    def png(cls, geschlecht, alter=0.0, tonus=0.0, masse=0.0):
        u"""Pfad des 8-Bit-Graustufen-PNG; geschrieben, wenn es fehlt."""
        ...
        Image.fromarray(np.clip(werte * 255.0 + 0.5, 0, 255).astype(np.uint8),
                        'L').save(vorlaeufig, 'PNG')
