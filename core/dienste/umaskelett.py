# -*- coding: utf-8 -*-
"""Umaskelett — das Skelett der UMA-Figur aus dem Figurkatalog.
...
class UmaskelettFehlt(Exception):
    """Keine UMA-GLB im Katalog."""


class Umaskelett:
    """UMA-Skelett aus `Figuren/uma/`, je Datei einmal gelesen."""
    ...
    def glb_pfad(cls, name=None):
        """Die gueltige GLB — `name` aus dem Katalog, sonst aus dem Zeiger, sonst
        die juengste."""
        ...
        if name:
            logger.warning(
                'Umaskelett: %s nennt %s, die Datei fehlt — nehme %s',
                cls.ZEIGER,
                name,
    ...
    def _juengste(ordner):
        if not os.path.isdir(ordner):
            return None
        dateien = [os.path.join(ordner, n) for n in os.listdir(ordner) if n.lower().endswith('.glb')]
    ...
    def knochen(cls, name=None):
        """Die Knochen in Three.js-Form — siehe `Gltfskelett.knochen`."""
        return cls._laden(name)[1]

    @classmethod
    def geometrie(cls, name=None):
        """`SkeletonGeometry` fuer den Retarget-Motor, Achse +Y — der Figur `name`,
        sonst der gueltigen."""
        return cls._laden(name)[2]

    @classmethod
    def beschreibung(cls):
        """Quelle, Dateiname und Stand — fuer die Antwort an den Browser."""
        pfad = cls.glb_pfad()
        return {
            'quelle': cls.QUELLE,
            'datei': os.path.basename(pfad),
            'stand': time.strftime(cls.ZEITFORMAT, time.localtime(os.path.getmtime(pfad))),
        }

    @classmethod
    def _laden(cls, name=None):
        """`(mtime, knochen, geometrie)` der Datei — aus dem Bestand oder frisch
        gelesen."""
        ...
        with cls._schloss:
            ...
            knochen = Skelettausrichtung(roh, Umazuordnung.LINKS, Umazuordnung.RECHTS).ausgerichtet()
    ...
    @classmethod
    def vergessen(cls):
        """Den Speicher leeren — fuer Tests und nach einem Katalogwechsel."""
        cls._bestand = {}
