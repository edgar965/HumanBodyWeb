# -*- coding: utf-8 -*-
u"""Figurbilder — die Vorschaubilder der Figur-Kandidaten, nur lesend.
...
class Figurbilder:
    ...
    TYPEN = {'.jpg': 'image/jpeg', '.jpeg': 'image/jpeg',
    ...
    def ordner_liste(cls):
        u"""Die Kandidatenordner, sortiert (`00_…`, `01_…`)."""
    ...
    def dateien(cls, ordner):
        u"""Die Bilddateien eines Ordners, sortiert; leer, wenn er fehlt."""
        pfad = cls.ORDNER / ordner
        if not cls.NAME.match(ordner) or not pfad.is_dir():
            return []
        return sorted(p.name for p in pfad.iterdir()
                      if p.is_file() and p.suffix.lower() in cls.ENDUNGEN)

    @classmethod
    def quelle(cls, ordner, datei):
        u"""Die Originaldatei — oder None, wenn Ordner/Name nicht zulässig sind."""
    ...
    def vorschau(cls, ordner, datei):
        u"""Die verkleinerte Fassung als JPEG; gebaut, wenn sie fehlt oder
