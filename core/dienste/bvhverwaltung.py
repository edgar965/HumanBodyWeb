...
class Bvhverwaltung:
    ...
    def ausfuehren(daten):
        ...
        logger.info(
            '[bvh-manage] action=%s, category=%s, name=%s',
            aktion,
            daten.get('category', ''),
            daten.get('name', ''),
        )
    ...
    def _datei(kategorie, name):
        ...
        pfad = Bvhablage.pfad_pruefen(Bvhablage.wurzel() / kategorie / f'{name}.bvh')
    ...
    def _umbenennen(daten):
        kategorie, name, neu = Bvhverwaltung._pflicht(daten, 'category', 'name', 'new_name')
        alt = Bvhverwaltung._datei(kategorie, name)
        ziel = Bvhverwaltung._freies_ziel(
            Bvhablage.wurzel() / kategorie / f'{neu}.bvh', f'{neu}.bvh exists already'
        )
    ...
    def _verschieben(daten):
        kategorie, name, neue_kategorie = Bvhverwaltung._pflicht(daten, 'category', 'name', 'new_category')
        alt = Bvhverwaltung._datei(kategorie, name)
    ...
    def _kopieren(daten):
        ...
        ordner.mkdir(parents=True, exist_ok=True)
        ziel = Bvhverwaltung._freies_ziel(ordner / f'{neu}.bvh', f'{neu}.bvh already exists')
    ...
    def _ordner_umbenennen(daten):
        ...
        ziel = Bvhverwaltung._freies_ziel(Bvhablage.wurzel() / neu, f'Folder {neu} already exists')
        alt.rename(ziel)
