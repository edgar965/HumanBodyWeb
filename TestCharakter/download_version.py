...
def extract_new(commit_hash: str):
    ...
    try:
        subprocess.run(
            ['git', '-C', str(HUMANBODY_REPO), 'archive', '--format=tar', '-o', tmp_path, commit_hash],
            check=True,
        )
        with tarfile.open(tmp_path, 'r') as tar:
            for member in tar.getmembers():
                if any(
                    member.name == p.rstrip('/') or member.name.startswith(p) for p in EXTRACT_PREFIXES_NEW
                ):
...
def extract_old(commit_hash: str):
    ...
    try:
        subprocess.run(
            ['git', '-C', str(PARENT_REPO), 'archive', '--format=tar', '-o', tmp_path, commit_hash],
            check=True,
        )
        with tarfile.open(tmp_path, 'r') as tar:
            for member in tar.getmembers():
                for src_prefix, dest_prefix in EXTRACT_MAP_OLD.items():
                    if member.name == src_prefix.rstrip('/') or member.name.startswith(src_prefix):
                        ...
                        dest_path = (
                            os.path.join(str(DEST), dest_prefix.rstrip('/'), rel)
                            if rel
                            else os.path.join(str(DEST), dest_prefix.rstrip('/'))
                        )
                        if member.isdir():
...
def main():
    ...
    if layout == 'new':
        repo = HUMANBODY_REPO
        print('  Layout: new (humanbody_core inside HumanBody repo)')
    elif layout == 'old':
        repo = PARENT_REPO
        print('  Layout: old (humanbody_core in parent repo A:\\3DTools)')
    else:
