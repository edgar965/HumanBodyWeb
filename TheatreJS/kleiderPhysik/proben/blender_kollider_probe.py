...
kugel.location = (0, 0, 0.5); kugel.keyframe_insert('location', frame=1)
kugel.location = (0.6, 0, 0.5); kugel.keyframe_insert('location', frame=40)
# Verformung: Shape-Key streckt die Kugel in z
kugel.shape_key_add(name='Basis')
sk = kugel.shape_key_add(name='hoch')
for v in sk.data: v.co.z = v.co.z * 1.8
sk.value = 0.0; sk.keyframe_insert('value', frame=1)
sk.value = 1.0; sk.keyframe_insert('value', frame=40)
...
for f in range(1, 31): sc.frame_set(f)
...
kz = max(q.z for q in kw); kx = sum(q.x for q in kw) / len(kw)
...
innen = 0
for q in p:
    l = kugel.matrix_world.inverted() @ q
    sz = (kz - 0.5) / 0.5
    if (l.x / 0.5) ** 2 + (l.y / 0.5) ** 2 + (l.z / (0.5 * sz)) ** 2 < 1.0: innen += 1
print('PROBE kugel_mitte_x=%.2f kugel_oben=%.3f tuch_ueber_kugel_min=%.3f (n=%d) tuch_am_rand_min=%.3f (n=%d) im_kollider=%d von %d' % (kx, kz, min(nah) if nah else -9, len(nah), min(weit) if weit else -9, len(weit), innen, len(p)))
