# -*- coding: utf-8 -*-
"""Freie Inseln im Verdeckten schliessen (Achselfalte).
...
class Maskeninseln:
    ...
    def schliessen(maske, dreiecke, hoechstens):
        ...
        kanten = kanten[frei]
        graph = coo_matrix((np.ones(len(kanten), dtype=np.int8), (kanten[:, 0], kanten[:, 1])), shape=(n, n))
        anzahl, marke = connected_components(graph, directed=False)
        groesse = np.bincount(marke, minlength=anzahl)
        groesse_frei = np.where(np.bincount(marke, weights=~maske, minlength=anzahl) > 0, groesse, 0)
        groesste = groesse_frei.max() if len(groesse_frei) else 0
        insel = (groesse_frei <= hoechstens) & (groesse_frei < groesste) & (groesse_frei > 0)
