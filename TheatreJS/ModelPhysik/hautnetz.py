# -*- coding: utf-8 -*-
"""Die Aussenhaut der HumanBody-Figur als geschlossenes Dreiecksnetz.
...
class Hautnetz:
    """Die Aussenhaut als Dreiecksnetz, ohne Zaehne, Augen und Zunge.
    ...
    def _basis(self, wurzel):
        """Das L1-Netz der Bauart — dieselbe Quelle wie `CharacterState`.
    ...
    def aussenhaut(self):
        """Dreiecke der Aussenhaut und die dazu gehoerenden Punkte."""
    ...
    def _kantenlaengen(punkte, dreiecke):
        """Alle Kantenlaengen eines Dreiecksnetzes (mit Wiederholung)."""
    ...
    def guete(punkte, dreiecke):
        """Radienquotient je Dreieck: 1 = gleichseitig, 0 = entartet.
    ...
    def reparieren(punkte, dreiecke, ziel_dreiecke=10000):
        """Loecher schliessen und Winzkanten verschmelzen — mit trimesh.
        ...
        if ziel_dreiecke:
            satz.meshing_decimation_quadric_edge_collapse(
                targetfacenum=int(ziel_dreiecke),
            ...
            m = satz.current_mesh()
            kanten = Hautnetz._kantenlaengen(np.asarray(m.vertex_matrix()), np.asarray(m.face_matrix()))
    ...
    def randkanten(dreiecke):
        """Kanten, an denen nur EIN Dreieck haengt — also Loecher."""
