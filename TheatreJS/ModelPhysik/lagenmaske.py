# -*- coding: utf-8 -*-
"""Stoff unter Stoff: welche Punkte eines Stuecks unter einem anderen liegen.
...
class Lagenmaske:
    """Welche Punkte eines Stuecks unter einem anderen liegen."""

    MINDEST_M = 0.001

    @classmethod
    def verdeckt(cls, koerper, koerperdreiecke, stoffe, **optionen):
        """`stoffe`: Liste von (name, punkte, dreiecke). Liefert
        ...
        normalen = [cls.normalen_von_haut(np.asarray(p, dtype=np.float64), N, baum) for _n, p, _t in stoffe]
        aus = {name: (np.zeros(len(p), dtype=bool), []) for name, p, _t in stoffe}
        for a in range(len(stoffe)):
            for b in range(a + 1, len(stoffe)):
                lage = cls.lage(stoffe[a], normalen[a], stoffe[b], normalen[b], **optionen)
                if lage == 0:
                    continue
                innen, aussen, n_innen = (
                    (stoffe[a], stoffe[b], normalen[a]) if lage > 0 else (stoffe[b], stoffe[a], normalen[b])
                )
                m = Hautmaske.verdeckt(
                    innen[1], innen[2], [(aussen[1], aussen[2])], normalen=n_innen, **optionen
                )
                aus[innen[0]][0][:] |= m
