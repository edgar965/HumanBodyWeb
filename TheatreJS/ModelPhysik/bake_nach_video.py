# -*- coding: utf-8 -*-
"""Eine `FPSBAKE1`-Ablage als MP4 — schattiert, nicht als Punktwolke.
...
class Bakevideo:
    """Rendert die Punkte je Bild und schreibt ein MP4."""
    ...
    def _in_metern(self):
        """Auf Meter und auf den Boden (`Bakedatei.auf_dem_boden`)."""
        return self.bake.auf_dem_boden(self.FIGURHOEHE)

    # ------------------------------------------------------------ Rendern

    def _kamera(self, figurhoehe):
        """Blick von vorn, leicht ueber Huefthoehe."""
        import trimesh
    ...
    def bilder(self):
        ...
        try:
            for nummer in range(self.bake.bilder):
                ...
                netz = trimesh.Trimesh(vertices=punkte, faces=self.dreiecke, process=False)
                ...
                material = pyrender.MetallicRoughnessMaterial(
                    baseColorFactor=[0.84, 0.77, 0.70, 1.0], metallicFactor=0.0, roughnessFactor=0.75
                )
                szene = pyrender.Scene(
                    bg_color=list(self.HINTERGRUND) + [1.0], ambient_light=[0.35, 0.35, 0.38]
                )
                szene.add(pyrender.Mesh.from_trimesh(netz, smooth=True, material=material))
    ...
    def schreiben(self, ziel, fps=20, schleifen=1):
        """MP4 schreiben. Gibt (Pfad, Bilderzahl) zurueck."""
