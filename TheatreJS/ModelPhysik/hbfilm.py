# -*- coding: utf-8 -*-
"""Die angezogene HumanBody-Figur in Bewegung — ein Video.
...
AUSGABE = os.path.join("A:", os.sep, "3DTools", "Assets", "GarmentCode", "ausgabe")
...
BVH = os.path.join("A:", os.sep, "3DTools", "3DObjects", "animations", "bvh", "Walk", "136_28.bvh")
AB_BILD = 278
...
class Hbfilm:
    """Koerper und Kleider durch dieselbe Rechnung, dann ins Bild."""
    ...
    def __init__(
        ...
        """`figurpunkte` sind ALLE Basispunkte der Figur (mit Morphs);
        ...
        self.melder("Bewegung laden", 0.05)
        ...
        if details is not None:
            self.melder("Haut und Augen", 0.06)
        ...
        for eintrag in stuecke if stuecke is not None else STUECKE:
            if isinstance(eintrag, dict):
                self._stueck(
                    eintrag["name"], eintrag.get("pfad"), tuple(eintrag.get("farbe") or (0.3, 0.45, 0.7))
                )
            else:
                name, ordner, farbe = eintrag
                self._stueck(name, os.path.join(AUSGABE, ordner, ordner + "_sim_rig.json"), farbe)
    ...
    def _koerper(self, fein=None, alle=None):
        ...
        if fein is not None:
            ...
            self.melder("Netz: " + feinheit.beschreibung(), 0.055)
        else:
        ...
        self.teile.append(
            {
                ...
                "dreiecke": dreiecke,
                "farbe": farbe,
                "sitz": Figurnetze.sitzprobe(punkte, Feinkoerper.ruhe(self.teile[0])),
            }
        )
    ...
    def proben(self):
        """Ruheprobe je Teil — muss null sein, sonst stimmt die Lage nicht."""
        zeilen = []
        for teil in self.teile:
            zeilen.append(
                (
                    teil["name"],
                    len(teil["haut"].punkte),
    ...
    def rechnen(self):
        for nummer, teil in enumerate(self.teile):
            self.melder("Bewegung: %s" % teil["name"], 0.1 + 0.2 * nummer / max(len(self.teile), 1))
            teil["haut"].rechnen()
        if self.physik > 0.0:
            Filmphysik(self.teile, self.bahn, self.bildzeit, self.physik, self.melder).anwenden()

    def gleichlauf(self):
        """DIE ENTSCHEIDENDE PROBE: Bleibt der Stoff beim Koerper?
    ...
    def bilder_rendern(self, breite=720, hoehe=900):
        return Filmrender(self.teile, self.bahn, self.melder).bilder_rendern(breite, hoehe)

    def schreiben(self, ziel, fps=24.0, schleifen=2):
        return Filmrender(self.teile, self.bahn, self.melder).schreiben(ziel, fps, schleifen)
...
def main():
    ...
    zerleger.add_argument(
        "--physik", type=float, default=0.0, help="Hoechster Weichgewebe-Zuschlag in mm, 0 = aus"
    )
    zerleger.add_argument("--ab", type=int, default=AB_BILD)
    zerleger.add_argument(
        "--aus", default=os.path.join(r"A:\3DTools\Docu\KörperPhysik", "hb_angezogen_gehen.mp4")
    )
    werte = zerleger.parse_args()

    film = Hbfilm(werte.bvh, werte.bilder, werte.fps, werte.ab, physik=werte.physik)
    print("Bewegung  %s, %d Bilder ab %d" % (os.path.basename(werte.bvh), werte.bilder, werte.ab))
    print(
        "          Quelle %.0f fps, Video %.0f fps, Zeitschritt %d"
        % (film.bahn.quell_fps, werte.fps, film.bahn.schritt)
    )
    print(
        "          Wurzelweg %.2f m, Fuss-Spanne %.0f mm"
        % (film.bahn.wurzelweg(), (film.bahn.knochenspanne("DEF-foot.L") or 0) * 1000)
    )
    print("\n%-10s %8s %14s %10s %8s" % ("Teil", "Punkte", "RUHEPROBE", "Sitz", "ohne W."))
    for name, punkte, ruhe, sitz, leer in film.proben():
        print(
            "%-10s %8d %11.6f mm %7s %8d"
            % (name, punkte, ruhe, "%.0f mm" % sitz if sitz is not None else "-", leer)
        )
    ...
    for name, werte_bahn in film.gleichlauf():
        print(
            ...
            "Mitte %.0f mm, Ende %.0f mm" % (name, werte_bahn[0], werte_bahn[1], werte_bahn[2])
        )
    for teil in film.teile:
        mittel, groesster = teil["haut"].bewegung()
        print(
            "%-10s bewegt sich im Mittel %.0f mm, groesster Weg %.0f mm"
    ...
    for teil in film.teile:
        ...
        if not b:
            continue
        print(
            "PHYSIK %-8s Ziel %.0f mm -> Staerke %.4f, erreicht %.1f mm "
            "(Mittel %.1f, Bild 0 %.2f)"
            % (
                teil["name"],
                b["ziel_mm"],
        ...
        print(
            "       Weichheit Median %.2f, Knochen je Punkt %.1f "
            "(hoechstens %d), Tempo bis %.2f m/s und %.2f rad/s"
            % (
                b["weichheit_median"],
                b["ahnen_je_punkt"][0],
                b["ahnen_je_punkt"][1],
        ...
        if "durchstich" in b:
            print(
                "       STOFF IM KOERPER %.2f %% (ohne Grenze waeren es "
                "%.2f %%), %d Punkte gekuerzt" % (b["durchstich"], b["durchstich_ohne_grenze"], b["gekuerzt"])
            )
    pfad, zahl = film.schreiben(werte.aus, fps=werte.fps)
    print("\nVideo     %s (%d Bilder, %.1f KB)" % (pfad, zahl, os.path.getsize(pfad) / 1024.0))
    return 0
