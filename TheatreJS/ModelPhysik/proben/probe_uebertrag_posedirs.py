# -*- coding: utf-8 -*-
"""Machbarkeit: SMPLs Pose-Korrektur auf UNSEREM Netz.
...
class Posedirsuebertrag:
    """SMPLs `posedirs` ueber den naechsten Punkt auf das HumanBody-Netz."""

    GRADE = (30, 60, 90, 120)

    @staticmethod
    def nach_smpl_raum(punkte):
        """Blender (Z oben) -> SMPL (Y oben): (x, y, z) -> (x, z, -y)."""
        return np.stack([punkte[:, 0], punkte[:, 2], -punkte[:, 1]], axis=1)

    @staticmethod
    def zurueck(vektoren):
        """Die Gegenrichtung, fuer Verschiebungen."""
    ...
    def zuordnen(self):
        """Beide in denselben Raum und auf dieselbe Hoehe; je Punkt der
        naechste SMPL-Punkt."""
    ...
    def korrektur(self, w):
        """SMPLs Korrektur fuer diesen Ellbogenwinkel, auf unser Netz gebracht:
    ...
    def zeile(self, grad):
        ...
        mit = Armmass.haeuten(self.punkte + self.korrektur(w), self.gewichte, self.ruhe, pose)
