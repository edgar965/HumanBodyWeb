# -*- coding: utf-8 -*-
"""Was macht das reine Linear Blend Skinning mit unserem Netz?
...
class Armmass:
    """Die Rechenbausteine der Probe: Drehungen, LBS, Scheibe, Umfang."""

    @staticmethod
    def quat_matrix(q):
        """[w, x, y, z] wie in `def_skeleton.json` -> 3x3."""
        w, x, y, z = q
        return np.array(
            [
    ...
    def haeuten(punkte, gewichte, ruhe, pose):
        """LBS wie Three.js: v' = sum_i w_i * (P_i * R_i^-1) * v."""
    ...
    def scheibe(punkte, auswahl, ursprung, achse, hoehe, dicke=0.010):
        """Punktnummern in einer Scheibe senkrecht zur Achse (Ruhelage)."""
    ...
    def umfang(punkte, nummern, achse):
        """Umfang der konvexen Huelle DIESER Punkte, projiziert auf die Ebene
        senkrecht zur Achse.
...
class Skelett:
    ...
    def welt(self, zusatz=None):
        """Weltmatrizen; `zusatz` = {knochenname: lokale 3x3-Drehung}.
...
class LbsProbe:
    """Beugen und Verdrehen des linken Unterarms, Umfang je Messscheibe."""
