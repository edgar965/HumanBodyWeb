# -*- coding: utf-8 -*-
"""Machbarkeit, zweiter Anlauf: erst die Skelette zur Deckung bringen.
...
class Smplraum:
    """Achsen, Ausrichtung und Armrichtung — die Rechenbausteine."""

    @staticmethod
    def hin(punkte):
        """Blender (Z oben) -> SMPL (Y oben)."""
    ...
    def procrustes(quelle, ziel):
        """Aehnlichkeitstransformation quelle -> ziel (Drehung, Massstab, Versatz)."""
...
class Uebertragprobe:
    """Vier Schritte: Haltung angleichen, ausrichten, zuordnen, Wirkung messen."""
    ...
    def __init__(self):
        ...
        unsere_gelenke_roh = {n: self.ruhe[self.skelett.nummer[n]][:3, 3] for n in PAARE.values()}
        self.unsere = Smplraum.hin(np.array([unsere_gelenke_roh[PAARE[i]] for i in self.nummern]))

    def haltung_angleichen(self):
        """1. SMPL-Arme in unsere T-Pose drehen."""
        smpl_gelenke = self.k.J_regressor @ self.smpl_rest
        unser_arm = Smplraum.armwinkel(
            {0: self.unsere[self.nummern.index(16)], 1: self.unsere[self.nummern.index(18)]}, 0, 1
        )
        smpl_arm = Smplraum.armwinkel(smpl_gelenke, SCHULTER_L, ELLBOGEN_SMPL)
        # Winkel in der Frontalebene (x nach links, y nach oben)
        diff = np.degrees(np.arctan2(unser_arm[1], unser_arm[0])) - np.degrees(
            np.arctan2(smpl_arm[1], smpl_arm[0])
        )
        print(
            "Armrichtung: unsere %s, SMPL %s -> %.1f Grad angleichen"
            % (np.round(unser_arm, 3), np.round(smpl_arm, 3), diff)
        )
        w = np.radians(diff)
        self.smpl_rest_t = self.k.posieren(
            self.smpl_rest,
            {
    ...
    def ausrichten(self):
        ...
        print(
            "Gelenkpassung: %d Paare, Median %.1f mm, max %.1f mm, Massstab %.4f"
            % (len(ziel), np.median(rest) * 1000, rest.max() * 1000, self.massstab)
        )

    def zuordnen(self):
        """3. Jeden unserer Punkte an den naechsten SMPL-Punkt."""
        unser_im_smpl = self.massstab * (self.r @ Smplraum.hin(self.punkte).T).T + self.versatz
    ...
    def _korrektur(self, wk):
        """SMPLs posedirs-Korrektur fuer die Ellbogenbeugung, in unseren Raum."""
    ...
    def wirkung(self):
        """4. Armumfang 3 cm ueber dem Ellbogen: LBS gegen LBS + Korrektur."""
        ...
        print("%-10s %9s %24s %26s" % ("Beugung", "Ruhe", "nur LBS (heute)", "LBS + SMPL-Korrektur"))
        ...
        for grad in (30, 60, 90, 120):
            ...
            mit = Armmass.haeuten(self.punkte + self._korrektur(wk), self.gewichte, self.ruhe, pose)
            ...
            print(
                "%-10s %6.2f cm   %7.2f cm (%+6.1f %%)   %8.2f cm (%+6.1f %%)"
                % ("%d Grad" % grad, u0, u_lbs, 100 * (u_lbs - u0) / u0, u_mit, 100 * (u_mit - u0) / u0)
            )
