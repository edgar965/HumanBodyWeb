...
def bauen(dim, zelle, verankert=True):
    ...
    builder.add_soft_grid(
        pos=wp.vec3(0.0, 1.0, 0.0), rot=wp.quat_identity(),
    ...
    if verankert:
        ...
        for i, m in enumerate(builder.particle_mass):
            if builder.particle_q[i][0] < 1e-6:
                builder.particle_mass[i] = 0.0
    return builder


def messen(dim, zelle, bilder=20, teilschritte=10, iterationen=10):
    builder = bauen(dim, zelle)
...
def main():
    ...
    messen((8, 8, 8), 0.05, bilder=2)

    print('')
    print('--- ueber die Netzgroesse (10 Teilschritte x 10 Iterationen) ---')
    print('%-12s %8s %10s %11s' % ('Gitter', 'Punkte', 'Tetraeder', 'ms je Bild'))
    for dim, zelle in (((8, 8, 8), 0.05), ((12, 12, 12), 0.04),
                       ((16, 16, 16), 0.03), ((20, 20, 20), 0.03),
                       ((26, 26, 26), 0.02), ((34, 34, 34), 0.015)):
        punkte, tets, ms = messen(dim, zelle)
    ...
    for teil, iters in ((10, 10), (10, 5), (5, 10), (5, 5), (2, 5), (1, 5)):
        _, _, ms = messen((20, 20, 20), 0.03, teilschritte=teil, iterationen=iters)
