...
BEREICHE = [
    ...
    {'slug': 'djangobase_infra',
     'name': 'djangoBase: Einstellungs-Speicher und Log-Einordnung',
     'wurzel': r'A:\shared\djangoBase',
    ...
    {'slug': 'db_logs', 'name': 'djangoBase: Logs-Seite',
     ...
     'hinweis': ('Zeigt rotierende Logdateien in zwei Reitern (Ausnahmen/Allgemeines), '
                 'mit Quellenauswahl und Filtern. Die Dateien koennen hunderte '
                 'Megabyte gross sein und WACHSEN, waehrend die Seite sie liest. In '
                 'sechs Projekten im Einsatz. Die Einordnung selbst (log_classifier) '
                 'wurde am 13.08.2026 schon geprueft und nachgebessert — hier geht es '
                 'um die SEITE.'),
     'fragen': [
         'Wie viel Speicher braucht ein Aufruf bei einer 500-MB-Datei? Wird von hinten '
         'gelesen oder alles eingelesen?',
         'Was passiert bei einer Datei, die gerade rotiert wird, und bei einer mit '
         'kaputtem Zeichensatz?',
         'Der Modus "alle Quellen chronologisch": Wie wird sortiert, was kostet das, '
         'und was passiert mit Zeilen ohne Zeitstempel?',
     ]},
    {'slug': 'db_versions', 'name': 'djangoBase: Versionen-Seite (GitHub)',
     'wurzel': r'A:\shared\djangoBase',
     'dateien': ['djangobase/views/versions.py'],
     'hinweis': ('Holt Commit-Listen aus der GitHub-API und zeigt sie je Repo — ein '
                 'fremder Dienst im Anfrage-Weg: langsam, gedrosselt (403 Ratelimit), '
                 'fehlerhaft oder stumm. Der Repo-Slug wird aus dem lokalen git-origin '
                 'abgeleitet, wenn er nicht konfiguriert ist. In sechs Projekten im '
                 'Einsatz.'),
     'fragen': [
         'Was passiert bei Drosselung, ohne Netz und bei einer Antwort, die kein JSON '
         'ist? Bleibt die Seite benutzbar?',
         'Gibt es eine Zeitgrenze fuer den Abruf, und blockiert das Warten die '
         'Anfrage?',
         'Wird gecacht? Was passiert beim Cache-Miss unter Last?',
         'Kommt ein Token zum Einsatz, und kann es in einer Fehlermeldung oder im '
         'Protokoll landen?',
     ]},
    {'slug': 'db_stats', 'name': 'djangoBase: Auslastungsanzeige, Jobs, Verkehr',
     'wurzel': r'A:\shared\djangoBase',
     'dateien': ['djangobase/system_stats.py', 'djangobase/jobs.py',
                 'djangobase/traffic.py'],
     'hinweis': ('system_stats liefert die Zahlen der Auslastungsleiste (GPU ueber '
                 'nvidia-smi als Subprozess, CPU/RAM/Netz/Platten ueber psutil) und '
                 'wird im Sekundentakt von mehreren Tabs abgefragt; dafuer gibt es '
                 'HintergrundCache (stale-while-revalidate). jobs.py ist eine Registry '
                 'wiederkehrender Aufgaben, traffic.py zaehlt Besuche (Middleware, '
                 'opt-in).'),
     'fragen': [
         'Wo kostet ein Abruf trotz Zwischenspeicher Zeit IN der Anfrage?',
         'nvidia-smi als Subprozess: Was ohne NVIDIA-Karte, was bei einem haengenden '
         'Aufruf, und wie oft wird er wirklich gestartet?',
         'traffic: Was schreibt die Middleware je Anfrage, was kostet das bei vielen '
         'Aufrufen, und werden IP-Adressen gespeichert?',
    ...
    {'slug': 'db_benutzer', 'name': 'djangoBase: Benutzerverwaltung und Rollen',
     ...
     'dateien': ['djangobase/views/benutzer.py', 'djangobase/forms.py',
                 'djangobase/mixins.py'],
     'hinweis': ('GETEILTER CODE — dieselbe Datei laeuft in sechs Projekten, ein '
                 'Fehler wirkt '
                 'ueberall. Die Benutzerliste zeigt Rollen, Online-Zustand und '
                 'Sitzungen; '
                 '_sitzungen_map liest die Sitzungstabelle und dekodiert jede Sitzung, '
                 'um den '
                 'Benutzer zu finden. BenutzerStatusView schaltet Benutzer '
                 'aktiv/inaktiv. '
                 'ZugriffMixin in mixins.py entscheidet, WER diese Seiten sehen darf — '
                 'das ist '
    ...
    {'slug': 'db_uebersetzung', 'name': 'djangoBase: Übersetzungs-Katalog',
     'wurzel': 'A:\\shared\\djangoBase',
     'dateien': ['djangobase/uebersetzung.py',
                 'djangobase/templatetags/uebersetzung.py',
                 'djangobase/views/uebersetzung.py'],
     'hinweis': ('GETEILTER CODE. Texte werden zur Laufzeit uebersetzt und in einem '
                 'Katalog in '
                 'der Datenbank gehalten; _katalog_laden haelt ihn im Modul-Speicher, '
                 'text_holen liefert aus dem Katalog oder registriert einen neuen '
                 'Text. Der '
                 'Template-Tag laeuft OFT — je Seitenaufbau hunderte Male. '
                 'lauf_starten '
                 'startet einen Hintergrundlauf, der fehlende Uebersetzungen holt. '
                 'Zwei Fallen '
                 'liegen hier nebeneinander: ein Modul-Cache, den mehrere Faeden '
                 'benutzen, und '
                 'ein Hintergrundlauf, der in die Datenbank schreibt.'),
     'fragen': [
         ('Der Modul-Cache: Was passiert bei mehreren Arbeiter-Faeden (Daphne) und '
          'wann '
    ...
    {'slug': 'db_cache_signale',
     'name': 'djangoBase: Hintergrund-Cache, Signale, Online-Zustand',
     'wurzel': 'A:\\shared\\djangoBase',
     'dateien': ['djangobase/hintergrund_cache.py', 'djangobase/signals.py',
                 'djangobase/online.py'],
     'hinweis': ('GETEILTER CODE. HintergrundCache liefert einen alten Wert sofort und '
                 'erneuert in einem Faden (dasselbe Muster habe ich am 13.08.2026 in '
                 'die '
                 'Versionen-Seite eingebaut, nachdem sie 4,9 s in der Anfrage '
                 'rechnete). '
                 'signals.py haengt an Login/Logout/Signup und schreibt dabei in die '
                 'Datenbank; online.py haelt "wer ist online" im Django-Cache und hat '
                 'eine '
                 'Middleware, die bei JEDER Anfrage laeuft.'),
     'fragen': [
         ('HintergrundCache: Kann derselbe Bau zweimal gleichzeitig laufen, und was '
             'passiert bei einer Ausnahme im Faden — steht der alte Wert noch, oder '
             'ist '
