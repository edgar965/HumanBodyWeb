...
BEREICHE = [
    {'slug': 'fe_studio_tracks', 'name': 'Frontend: BVH-Studio Spuren',
     ...
     'hinweis': ('Verwaltet die Spuren des BVH-Studios: Ausschnitte, Verschiebungen, '
                 'Ueberblendungen. Rechnet mit Frame-Indizes UND Sekunden; die '
                 'Bildrate steckt in den Daten. Fehler zeigen sich als ruckelnde oder '
                 'um Frames verschobene Wiedergabe.'),
     'fragen': [
         'Wo werden Frames und Sekunden vermischt, und wo wird gerundet statt '
         'abgeschnitten (oder umgekehrt)?',
         'Was passiert am Rand: erster Frame, letzter Frame, Ausschnitt der Laenge 0, '
         'Ueberblendung laenger als der Ausschnitt?',
    ...
    {'slug': 'studio_timeline', 'name': 'BVH-Studio: Zeitleiste (Zeichnen und Maus)',
     ...
     'hinweis': ('1.176 Zeilen: zeichnet Spuren, Clips, Schlüsselbilder auf ein Canvas '
                 'und '
                 'behandelt Ziehen, Zuschneiden, Auswahl. renderTimeline laeuft bei '
                 'jeder '
                 'Mausbewegung. Die Zeilen 21-56 rechnen zwischen Zeile-auf-dem-Schirm '
                 'und '
                 'Spur-im-Modell um; wenn diese Umrechnung und das Zeichnen '
                 'verschiedene '
                 'Annahmen haben, greift man neben den Clip. Ich kann im Browser '
                 'messen '
                 '(Chrome-Fernsteuerung) und die Seite bedienen.'),
     'fragen': [
         ('_rowAtY und _rowYForTrackIdx sind Hin- und Rueckrechnung. Sind sie '
             'zueinander invers, auch bei zusammengeklappten Spuren und beim '
             'Scrollen?'),
    ...
    {'slug': 'studio_props', 'name': 'BVH-Studio: Eigenschaften-Bedienfeld',
     ...
     'hinweis': ('updateProperties (Zeile 54) ist eine einzige Funktion von ueber 700 '
                 'Zeilen: '
                 'Sie baut das rechte Bedienfeld je nach ausgewaehlter Spur neu auf — '
                 'Lichter, '
                 'Kameras, Charaktere, Boden. _changeLightType tauscht den Lichttyp im '
                 'Modell '
                 'UND in der Szene. Solche Funktionen halten Zustand doppelt: im '
                 'Modell und im '
                 'DOM.'),
     'fragen': [
         ('Wo wird im DOM etwas gesetzt, ohne dass das Modell mitgeht (oder umgekehrt) '
             '— also nach welcher Bedienung zeigt das Feld etwas anderes als die '
             'Szene?'),
         ('_changeLightType: Was bleibt von der alten Lichtquelle uebrig (Objekt in '
          'der '
    ...
    {'slug': 'studio_tools', 'name': 'BVH-Studio: Werkzeuge (Glättung, Bodenfix)',
     ...
     'hinweis': ('Die Werkzeugleiste: Gauss-Glaettung ueber alle Clips, feste '
                 'Position, '
                 'Bodenfix. _gaussSmooth.origClips haelt die Originaldaten in einer '
                 'Map, damit '
                 'man die Glaettung zurueckdrehen kann — der Schluessel ist '
                 '`trackIdx_clipIdx`. Beim Verschieben oder Loeschen von Spuren '
                 'aendern sich '
                 'diese Indizes. _gaussFilter glaettet Quaternion-Komponenten einzeln '
                 'und '
     ...
     'fragen': [
         ...
         ('applyGaussToAllClips und reloadAllClipAnimations: Wo wird doppelt '
          'geglaettet '
    ...
    {'slug': 'studio_project',
     ...
     'hinweis': ('buildProjectData sammelt den Zustand, restoreProjectData baut ihn '
                 'wieder '
                 'auf, saveSessionState/restoreSessionState machen dasselbe fuer '
                 'localStorage. '
                 'Solche Paare veralten getrennt: Was neu gespeichert wird, wird beim '
                 'Laden '
                 'vergessen — und umgekehrt. previewAnimation legt eine zweite '
                 'Three.js-Szene '
