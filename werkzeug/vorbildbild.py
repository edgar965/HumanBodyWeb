# -*- coding: utf-8 -*-
"""Vorbildbild — das Icon eines eigenen Vorbilds der Kleiderbibliothek.
...
class Vorbildbild:
    """Rendert Figur + Stueck als quadratisches Icon mit Alphakanal."""
    ...
    def __init__(self, modell, geschlecht='female'):
        ...
        self.koerper, self.flaechen = Garmentdrapierung.sichtbares_netz(figur, geschlecht, netz)
    ...
    def rendern(self, rig, ziel, farbe=(0.13, 0.13, 0.13), groesse=256):
        ...
        stoffwerk = pyrender.MetallicRoughnessMaterial(
            baseColorFactor=list(farbe) + [1.0], metallicFactor=0.0, roughnessFactor=0.7, doubleSided=True
        )
        koerpernetz = trimesh.Trimesh(
            np.asarray(self.koerper) @ d.T, np.asarray(self.flaechen), process=False
        )
