...
from ._bundle_basis import _PNG_CONTENT, Bundelruf, Mtlbezug
...
class BundleMtlTests(TestCategory):
    ...
    @classmethod
    def _mtl_und_textur(cls, bundle, mtl_name, mtl_inhalt, textur='swan.png'):
    ...
    def test_10_mtl_options_parser_strips_flags(cls):
        ...
        mtl = b'newmtl Swan\nKd 1.0 1.0 1.0\nmap_Kd -s 1 1 -o 0 0 0 swan_tex.png\n'
        fehler, mtl_daten = cls._mtl_und_textur(bundle, 'swan.mtl', mtl, textur='swan_tex.png')
        if fehler:
            return False, fehler
        ...
        if name != 'swan_tex.png':
            return False, (f'Parser extrahierte "{name}" (erwartet: "swan_tex.png")')
        basis = mtl_daten['url'].rsplit('/', 1)[0]
        status, _ = Bundelruf.abrufen(f'{basis}/{name}')
        if status != 200:
            return False, (f'Referenzierte Textur (mit Options-Flags davor) HTTP {status}')
    ...
    def test_11_auto_discover_mtl_from_obj(cls):
        ...
        status1, obj_data = Bundelruf.hochladen(
            '/api/studio/scene-object-upload/',
            [
