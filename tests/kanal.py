...
class Kanal:
    ...
    def _mehrteilig(files, grenze='----TestBoundary9876xyz'):
        ...
        eintraege = (
            ...
            else [(name, wert[0], wert[1], wert[2]) for name, wert in files.items()]
        )
        rumpf = b''
        for name, dateiname, inhalt, typ in eintraege:
            ...
            rumpf += (inhalt if isinstance(inhalt, bytes) else str(inhalt).encode()) + b'\r\n'
        rumpf += ('--%s--\r\n' % grenze).encode()
    ...
    def _lesen(status, text):
        ...
        try:
            ...
            logging.getLogger('core').debug('Antwort ist kein JSON (%d Zeichen)', len(text or ''))
