...
from ._bundle_basis import _MTL_CONTENT, _OBJ_CONTENT, _PNG_CONTENT, Bundelruf, Mtlbezug
...
class BundleUploadTests(TestCategory):
    ...
    def test_01_obj_upload_in_bundle(cls):
        ...
        status, data = Bundelruf.hochladen(
    ...
    def test_08_without_bundle_id_uses_random(cls):
        ...
        status1, data1 = Bundelruf.hochladen(
            '/api/studio/scene-object-upload/',
            [
                ('object', 'solo.obj', b'# a\nv 0 0 0\n', 'text/plain'),
            ],
        )
        status2, data2 = Bundelruf.hochladen(
            '/api/studio/scene-object-upload/',
            [
                ('object', 'solo.obj', b'# b\nv 1 1 1\n', 'text/plain'),
            ],
        ...
        if data1.get('url') == data2.get('url'):
            return False, 'Beide Uploads lieferten identische URL — kein Bundle-Schutz'
    ...
    def test_09_mtl_kd_references_texture_in_bundle(cls):
        ...
        if not all(hasattr(cls, name) for name in ('_obj_url', '_mtl_url', '_tex_url')):
        ...
        rohangabe, wortteile = Mtlbezug.aus_text(mtl_content.decode('utf-8', errors='ignore'))
        if not rohangabe:
        ...
        if erwartet != cls._tex_url:
            return False, (
                f'MTL zeigt auf "{name}" -> URL "{erwartet}", aber Textur liegt unter "{cls._tex_url}"'
            )
