"""
Test animation dropdown with two-level selection
"""
import asyncio
import sys
from playwright.async_api import async_playwright

sys.stdout.reconfigure(encoding='utf-8')

async def main():
    async with async_playwright() as p:
        browser = await p.chromium.launch(headless=False, slow_mo=500)
        page = await browser.new_page()

        print("Loading Settings-Theatre page...")
        await page.goto('http://localhost:8081/settings/theatre/')
        await page.wait_for_selector('.settings-card', timeout=5000)
        await asyncio.sleep(1)

        print("\nClicking animation dropdown...")
        await page.click('select[name="theatre_default_animation"]')
        await asyncio.sleep(1)

        await page.screenshot(path='settings_theatre_anim_open.png', full_page=True)
        print("✓ Screenshot: settings_theatre_anim_open.png")

        print("\nBrowser stays open for 5 seconds...")
        await asyncio.sleep(5)
        await browser.close()

if __name__ == '__main__':
    asyncio.run(main())
